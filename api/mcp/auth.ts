 ...
 *
 *   - env_key → `X-Meridian-Key: <apiKey>` (existing, unchanged).
...
export async function buildAuthHeaders(
  ...
  if (context.kind === 'env_key' || context.kind === 'user_key') {
    ...
    return { 'X-Meridian-Key': context.apiKey };
  }
  // context.kind === 'pro'
...
export function wwwAuthHeader(resourceMetadataUrl: string, errorParam = ''): string {
  ...
  return `Bearer realm="meridian"${errSegment}, resource_metadata="${resourceMetadataUrl}"`;
}
...
export async function resolveAuthContext(
  ...
  const candidateKey = req.headers.get('X-Meridian-Key') ?? '';
  if (!candidateKey) {
    return {
      ok: false,
      response: new Response(
        JSON.stringify({ jsonrpc: '2.0', id: null, error: { code: -32001, message: 'Authentication required. Use OAuth (/oauth/token) or pass your API key via X-Meridian-Key header.' } }),
...
export async function runProPreChecks(
  ...
  if (!validation || validation.userId !== context.userId) {
    return { ok: false, response: new Response(
      JSON.stringify({ jsonrpc: '2.0', id: null, error: { code: -32001, message: 'MCP authorization revoked. Re-authorize at https://meridian.app/mcp-grant.' } }),
      { status: 401, headers: withMcpNoStore({ 'Content-Type': 'application/json', 'WWW-Authenticate': wwwAuthHeader(resourceMetadataUrl, 'invalid_token'), ...corsHeaders }) },
    ) };
  }
