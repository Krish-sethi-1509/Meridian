...
function classifyMcpInboundHost(hostname: string): McpToolExecutionContext['inboundHostClass'] {
  ...
  if (hostname === 'meridian.app') return 'apex';
  if (hostname === 'www.meridian.app') return 'www';
  if (VARIANT_HOSTS.has(hostname)) return 'variant';
  if (hostname.endsWith('.meridian.app')) return 'meridian_subdomain';
...
export function createMcpToolExecutionContext(requestUrl: string): McpToolExecutionContext {
  ...
  const isProductionMeridianHost = (
    inbound.hostname === 'meridian.app'
    || inbound.hostname.endsWith('.meridian.app')
  );
  const downstreamOrigin = isProductionMeridianHost
