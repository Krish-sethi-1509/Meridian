...
export const PUBLIC_RESOURCE_REGISTRY: PublicResourceDef[] = [
  {
    uri: 'meridian://seed-meta/freshness',
...
export const TEMPLATE_RESOURCE_REGISTRY: TemplateResourceDef[] = [
  {
    uriTemplate: 'meridian://countries/{iso2}/risk',
    name: 'Country Risk',
    ...
    paramExtractor: (uri: string) => {
      if (!uri.startsWith('meridian://countries/')) return null;
      const m = /^meridian:\/\/countries\/([a-z]{2})\/risk$/.exec(uri);
      const iso2 = m?.[1];
      if (!iso2) {
        return {
          ok: false,
          reason: 'Expected meridian://countries/{iso2}/risk where {iso2} is lowercase ISO 3166-1 alpha-2.',
        };
      }
  ...
  {
    uriTemplate: 'meridian://chokepoints/{slug}/status',
    ...
    paramExtractor: (uri: string) => {
      if (!uri.startsWith('meridian://chokepoints/')) return null;
      const m = /^meridian:\/\/chokepoints\/([a-z][a-z0-9-]*)\/status$/.exec(uri);
      const slug = m?.[1];
      if (!slug) {
        return {
          ok: false,
          reason: 'Expected meridian://chokepoints/{slug}/status where {slug} is a hand-curated kebab-case identifier.',
        };
      }
  ...
  {
    uriTemplate: 'meridian://markets/{symbol}/quote',
    name: 'Market Quote',
    ...
    paramExtractor: (uri: string) => {
      if (!uri.startsWith('meridian://markets/')) return null;
      ...
      const m = /^meridian:\/\/markets\/([A-Z][A-Z0-9.=-]{0,15})\/quote$/.exec(uri);
      const symbol = m?.[1];
      if (!symbol) {
        return {
          ok: false,
          reason: 'Expected meridian://markets/{symbol}/quote where {symbol} is an uppercase ticker (e.g. "AAPL", "GC=F", "BTC-USD").',
        };
      }
