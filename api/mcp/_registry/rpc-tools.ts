...
import { COUNTRY_BRIEF_UI_URI, COUNTRY_RISK_UI_URI, WORLD_BRIEF_UI_URI } from '../_ui/registry';
...
const INTEL_HISTORY_RECORD_SCHEMA = {
  ...
  properties: {
    ...
    ingestedAt: { type: 'number', description: 'When Meridian stored the event, Unix epoch milliseconds. Differs from occurredAt for backfills.' },
...
export const RPC_TOOLS: ToolDef[] = [
  {
    ...
    _execute: async (_params, base, context, execution) => {
      ...
      const response = await fetch(url, {
        headers: { ...auth, 'User-Agent': 'meridian-mcp-edge/1.0' },
  ...
  },
  {
    ...
    _execute: async (params, base, context, execution) => {
      const UA = 'meridian-mcp-edge/1.0';
  ...
  },
  {
    ...
    _execute: async (params, base, context) => {
      ...
      const res = await fetch(url, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json', ...auth, 'User-Agent': 'meridian-mcp-edge/1.0' },
        body,
  ...
  ...ANALYSIS_TOOLS,
  {
    ...
    _execute: async (params, base, context, execution) => {
      ...
      const response = await fetch(url, {
        method: 'POST', headers: { 'Content-Type': 'application/json', ...auth, 'User-Agent': 'meridian-mcp-edge/1.0' }, body,
        signal: AbortSignal.timeout(12_000),
      });
      await assertMcpToolFetchOk(response, {
