...
test('CORS allow headers include MCP transport headers', () => {
  const privateCors = getCorsHeaders(makeRequest('https://meridian.app'));
  ...
  for (const cors of [privateCors, publicCors]) {
    ...
    assert.match(exposed, /\bX-Meridian-Bbox\b/);
    assert.match(exposed, /\bX-Meridian-Bbox-Missing\b/);
    assert.match(exposed, /\bX-Meridian-Bbox-Invalid\b/);
