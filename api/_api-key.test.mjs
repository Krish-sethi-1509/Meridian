...
function makeReq({ origin, referer, secFetchSite, key, cookie } = {}) {
  ...
  if (key) headers.set('x-meridian-key', key);
...
test('#3541: forged Referer alone is rejected', async () => {
  const r = await validateApiKey(makeReq({ referer: 'https://meridian.app/' }));
...
test('#3541: forged Origin: https://meridian.app alone is rejected (no key, no session)', async () => {
  const r = await validateApiKey(makeReq({ origin: 'https://meridian.app' }));
  assert.equal(r.valid, false);
});
...
test('getHeaderApiKey centralizes X-Meridian-Key and X-Api-Key aliases', () => {
