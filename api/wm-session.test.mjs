...
test('POST sets a valid HttpOnly cookie and returns the anonymous-only fallback token', async () => {
  const resp = await handler(makeReq('POST', { origin: 'https://meridian.app' }));
  ...
  assert.match(cookies.join('\n'), /wm-session=.*Domain=\.meridian\.app/);
});
...
test('POST emits one anonymous mint usage event without exposing cookie material', async () => {
  ...
  const request = makeReq('POST', {
    ...
    referer: 'https://meridian.app/reset-password?token=must-not-be-logged#also-not-logged',
  });
  ...
  assert.deepEqual(
    ...
    },
    {
      ...
      ip: null,
      referer: 'https://meridian.app/reset-password',
      reason: 'ok',
    },
  );
...
test('session usage telemetry records verified Cloudflare client attribution and rejects forged headers', async () => {
  ...
  const verified = makeReq('POST', { origin: 'https://meridian.app' });
  ...
  const forged = makeReq('POST', { origin: 'https://meridian.app' });
  ...
  const tor = makeReq('POST', { origin: 'https://meridian.app' });
...
test('OPTIONS preflight returns 204 with CORS', async () => {
  ...
  __resetRateLimitForTest();
  const resp = await handler(makeReq('OPTIONS', { origin: 'https://meridian.app' }));
...
test('GET method is rejected with 405', async () => {
  const resp = await handler(makeReq('GET', { origin: 'https://meridian.app' }));
...
test('POST returns degraded 503 without issuing a token when Redis limiter config is missing', async () => {
  ...
  assert.equal(resp.headers.get('access-control-allow-origin'), 'https://meridian.app');
...
test('failed mint outcomes emit their terminal status', async () => {
  ...
  const resp = await handler(makeReq('POST', { origin: 'https://meridian.app' }), ctx);
  assert.equal(resp.status, 429);
  await settle();
...
test('telemetry stops delivery attempts when the Axiom sink is repeatedly unavailable', async () => {
  ...
  for (let index = 0; index < 20; index += 1) {
    const { ctx, settle } = makeWaitUntilCtx();
    const response = await handler(makeReq('POST', { origin: 'https://meridian.app' }), ctx);
...
test('telemetry probes and closes the circuit after the outage window elapses', async () => {
  ...
  try {
    for (let index = 0; index < 20; index += 1) {
      const { ctx, settle } = makeWaitUntilCtx();
      await handler(makeReq('POST', { origin: 'https://meridian.app' }), ctx);
      await settle();
    }
    now += 5 * 60 * 1000 + 1;
    axiomAvailable = true;
    ...
    await handler(makeReq('POST', { origin: 'https://meridian.app' }), recovered.ctx);
    ...
    await handler(makeReq('POST', { origin: 'https://meridian.app' }), resumed.ctx);
...
test('legacy widget/pro keys are moved into short-lived HttpOnly cookies', async () => {
  ...
  assert.match(joined, /wm-widget-key=widget-secret;.*Domain=\.meridian\.app/);
  assert.match(joined, /wm-pro-key=pro-secret;.*Domain=\.meridian\.app/);
...
test('legacy cookie tombstones do not delete replacement HttpOnly key cookies', async () => {
  ...
  assert.equal(jar.get('wm-widget-key;.meridian.app;/'), 'widget-secret');
  assert.equal(jar.get('wm-pro-key;.meridian.app;/'), 'pro-secret');
});
...
function makeReqWithCookie(cookie) {
  const headers = new Headers({ origin: 'https://meridian.app' });
