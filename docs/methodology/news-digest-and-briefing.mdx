...
description: "How Meridian collects RSS/Atom feeds, classifies news, scores importance, tracks stories, filters briefs, uses LLMs, and controls bias."
---

## Scope

This page is the canonical public methodology for the news-feed digest,
daily digest notifications, and Meridian Brief. It covers the automated
...
Feed inventory lives in `server/meridian/news/v1/_feeds.ts`.
...
`energy` is a site and client-feed variant (`energy.meridian.app`), but it
...
`server/meridian/news/v1/_classifier.ts`.
...
Meridian Brief uses two LLM surfaces:
...
- Feed inventory and digest build: `server/meridian/news/v1/_feeds.ts`,
  `server/meridian/news/v1/list-feed-digest.ts`
- Keyword classifier: `server/meridian/news/v1/_classifier.ts`
- API contract: `proto/meridian/news/v1/list_feed_digest.proto`
