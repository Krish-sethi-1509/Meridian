...
- `https://www.meridian.app/api/resilience/v1/get-runtime-manifest`
  returned HTTP 200 with `formulaTag: "pc"` and
  `constructVersions.energy: "v2"` when requested with a browser-like
  user agent.
- `https://www.meridian.app/api/health` returned HTTP 200. The overall
...
```bash
API_BASE=https://www.meridian.app \
...
```bash
export API_BASE=https://www.meridian.app
...
3. **Bump the score-cache prefix.** Add a new commit to this branch
   ...
   `server/meridian/resilience/v1/_shared.ts`. This guarantees the
...
4. **Flip the flag in production.**
   ...
   ```bash
   curl -s https://meridian.app/api/resilience/v1/get-runtime-manifest \
