...
description: "Read, share, render, and paginate the AI-composed Meridian intelligence brief — endpoints for public shares, carousels, and share URLs."
---

Meridian composes a per-user intelligence brief on Railway, stores each
...
Single-shot completion endpoint used by embedded widget iframes. Auth via `X-Meridian-Key` (partner keys). Rate-limited per key.
