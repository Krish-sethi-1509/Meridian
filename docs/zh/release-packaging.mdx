...
description: "本指南提供两种桌面变体：full（Meridian）和 tech（Tech Monitor）的可复现本地打包步骤，涵盖构建脚本、代码签名、公证、图标与安装包生成，帮助维护者与贡献者在 macOS、Windows 与 Linux 上产出与官方发布一致的桌面二进制，并对每一步进行确定性校验。"
---
...
- Full 变体：`Meridian` / `meridian`
- Tech 变体：`Tech Monitor` / `tech-monitor`

不同的名称在 Tauri 中配置：

- `src-tauri/tauri.conf.json` → `Meridian` / `meridian`
