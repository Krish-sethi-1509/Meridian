---
title: "健康检查端点"
description: "Meridian 提供两个健康检查端点用于持续监控数据管道完整性与实时性，服务在 Vercel Edge Runtime 上以低延迟运行并查询 Redis (Upstash) 评估各数据源状态，帮助 SRE 与运维团队快速定位新闻、市场、海事等采集任务的中断与滞后。"
---
...
Edge 健康注册表随 PR 合并即刻部署，但它读取的键只能在 consumer-prices-core 的下一次每日抓取（02:00 UTC）→ 聚合（02:15）→ 发布（02:30）窗口之后才会出现。[#6059](https://github.com/koala73/worldmonitor/issues/6059) 用两道彼此独立、缺一不可的关卡关闭这个窗口：

1. **激活（持久、单向）。** 每日发布器（`consumer-prices-core/src/jobs/publish.ts`，且仅限它）为某市场写入「确实尝试过页面」的覆盖率快照后，才会以**无 TTL** 的方式 `SET` `seed-activated:consumer-prices:coverage:v1:<market>`。手动兜底脚本 `scripts/seed-consumer-prices.mjs` 刻意**不会**写入该标记：它的覆盖率键只有 30 分钟 TTL，用一个 30 分钟的产物换取永久激活，会在数据过期后把该市场永久卡在无法恢复的 `EMPTY`（严重）状态。该标记一旦存在，该市场即永久转为严格判定——缺失、为空、过期或低于阈值的覆盖率，其分类与没有本握手时完全一致。真实但为空的快照（无启用零售商，或零售商从未运行）**不会**激活。键名中的 `v1` 是覆盖率 schema 版本，因此未来的 schema 变更会开启各自独立评审的新窗口，而不会继承旧形态已获得的激活。
2. **截止时间（有界）。** 无论生产者是否运行过，软化都会在编译进 `api/health.js` 的每市场墙钟时间戳处停止。因此错过或失败的首次运行会在一个完整的抓取/聚合/发布周期内自行升级为 `EMPTY`（严重）。

八个覆盖率检查在**所有**状态下都会附带 `activated` 字段（持久标记存在时为 true），因此上线进度可直接从响应体审计，无需手动探测 Redis 即可区分「已激活后损坏」与「从未运行」。

仅当覆盖率键**完全缺失**时才会被软化。键一旦存在即说明生产者已运行，因此 `EMPTY_DATA`、`COVERAGE_DEGRADED`、`COVERAGE_PARTIAL` 与 `STALE_SEED` 均保持严格——覆盖率计数绝不合成，成功状态也绝不从无关的规范键推断。`scripts/check-seed-freshness.mjs` 会用墙钟时间重新校验每个 `rolloutPendingUntil`，而非信任状态字符串，因此已过期或无法解析的截止时间（以及缓存过旧的 compact 快照）都会照常上报问题。

在窗口期内 `overall` 会变为 `WARNING`（绝不会是 `UNHEALTHY`）。
...
| `ROLLOUT_PENDING` | 警告 | 新部署的 schema，其生产者尚未到达首次计划运行。**有界**：该条目携带编译在 `api/health.js` 中的 `rolloutPendingUntil` 截止时间；一旦该时间过去，或生产者写入其持久激活标记，状态即变为 `EMPTY`（严重）。计入 `summary.rolloutPending` |
...
生产者声明的集合只要覆盖固定集合，额外多出的国家会被接受。72 小时预算同样由健康
...
seeder 会为决策关键国家保留冷取槽位，使其在每次成为缓存未命中的运行中都被刷新。若不
保留，轮换周期（12 小时节奏下 `ceil(174 / 30)` 次运行 ≈ 72 小时）恰好等于内容预算，
告警将只是在测量轮换而非事故。

该保留仅对处于缓存**未命中**的国家生效。只要某国已持有的上游 `max(date)` 未变，它
就保持缓存命中；因此当上游订阅源自身冻结时，决策关键国家根本不会进入刷新队列，再多
的槽位保留也无法让其内容变新。在这种状态下，检查保持 `STALE_CONTENT` 恰恰是它在正确
工作：内容确实已过时，seeder 无法修复，告警报告的是上游中断而非内部故障。全局范围的内容过时在设计上属于常态：seeder 每次
12 小时运行最多刷新 30 个国家，因此完整轮换约需 72 小时，轮换队尾常规性地超出
预算。对此告警只会产生一个长亮且无法处置的警告。全局计数（`coveredCount`、
`freshCount`、`staleCount`、`unknownCount`、有界的 `staleCountries` 列表，以及
最旧观测及其年龄）仍会发布以供查看；只有关键子集会改变状态。

该 72 小时预算刻意与 `china-corridor-source-adapters.ts` 对 PortWatch 观测所用的
...
两个面只在一类输入上有意地不同。本端点无法区分“标记读取失败”与“标记确实缺失”——
两者落入同一分支——因此读取错误仍会授予放宽。MCP 则把无法读取的标记视为*未知*并故障
关闭：因为基于“缺乏证据”授予的放宽永不过期，被逐出或改名的标记会就此永久停用该告警。
因此对于一个尚未激活、且标记读取出错的键，本端点报告 `OK`，而 MCP 报告 `stale: true`。
分歧只会朝这一个方向发生——MCP 过度告警——绝不会朝隐藏过时内容的方向。让本端点对齐
的工作跟踪于 [#6095](https://github.com/koala73/worldmonitor/issues/6095)。
...
curl -s https://api.worldmonitor.app/api/health \
  -H "X-Meridian-Key: $WORLDMONITOR_API_KEY" | jq .
...
curl -s https://api.worldmonitor.app/api/seed-health \
  -H "Origin: https://meridian.app" | jq .
```
