---
title: "高级金融"
description: "高级金融是 Meridian 中提供股票分析能力的金融变体层，最初灵感来源于一个独立的 daily_stock_analysis 项目，现已完全集成 —— 在同一面板内提供股票代码、目标价、历史回测、分析师信号与新闻情绪聚合，帮助投资者与研究员在宏观情报之上叠加个股层面的量化视角。"
---
高级金融是 Meridian 中提供股票分析能力的金融变体层。这些功能最初灵感来源于一个独立的 `daily_stock_analysis` 项目，现已完全集成到 Meridian 代码库中。
...
高级股票分析引擎最初是独立项目核心逻辑的 TypeScript 移植版本，现已完全原生于 Meridian。
...
Meridian 仍缺少通用的关系型后端，因此高级金融当前使用 Redis 作为后端拥有的数据来源。
...
Meridian 当前的回测只验证技术信号模型。Yahoo 日线历史不包含时点化的基本面数据，因此回测不能被解释为实时基本面复合评级的历史回放。响应会公开 `ratingBasis: "technical_only"`，并使用独立的技术面引擎版本。
...
这是 Meridian 原生功能，并非原始项目调度器/自动化系统的直接移植。
...
- Finnhub，提供 Meridian 其他地方已使用的更广泛市场数据
...
该功能在 Meridian 当前架构内有效且可用于生产环境，但一些边界仍然明确：
