---
title: "新闻摘要与简报方法论"
description: "Meridian 如何采集 RSS 与 Atom 信息流、对新闻条目进行主题分类、按重要性评分、追踪话题报道趋势、过滤每日简报内容、调用 LLM 生成结构化摘要，并通过多源均衡与提示工程严格控制模型偏见与幻觉，构建一条透明、可审计的编辑管线与研究流程。"
---

## 范围

本页是新闻信息流摘要、每日摘要通知和 Meridian Brief 的规范公开
方法论。它涵盖了将 RSS/Atom 信息流转化为排序后的面板条目、持久化的
报道轨迹、摘要通知和个性化编辑简报的自动化管道。

该管道是自动化的。正常路径中没有人工审阅队列。质量来自来源策展、
日期门控、分类器上限、评分公式、读取路径过滤器、冷却遥测和 LLM
回退护栏。

## 信息流清单

信息流清单位于 `server/meridian/news/v1/_feeds.ts`。
接受的摘要变体为：
...
`energy` 是一个站点和客户端信息流变体（`energy.meridian.app`），但它
...
每个已解析条目都从 `server/meridian/news/v1/_classifier.ts` 中的
关键词分类器开始。
...
`isEphemeralLiveCoverage`，以及 `category`。
...
报道阶段由首次所见时间、提及计数和分数趋势推导：

| 阶段 | 规则 |
| --- | --- |
| `breaking` | 提及计数为 1。 |
| `developing` | 提及计数为 2-5 且报道产生不足 2 小时。 |
| `fading` | 为评分历史支持保留：信息流 API 分支要求 `currentScore` 和 `peakScore` 均为正，且当前分数降至峰值的一半以下。当前的摘要写入器仍为这些字段存储零占位符，因此该分支在信息流面板报道阶段上处于惰性状态。 |
| `sustained` | 持续追踪报道的回退。 |

通知 cron 有自己的用于摘要投递的读取路径阶段辅助函数。在那里，
沉默超过 24 小时的报道被视为 `fading` 并从延迟摘要/简报池中丢弃。
...
Meridian Brief 使用两个 LLM 界面：

- 摘要文稿（`brief:llm:digest:v8`）从可见报道池生成 JSON 导语、线索列表、
...
使用 Gemini 2.5 Flash（`google/gemini-2.5-flash`）。摘要文稿和报道描述调用
使用温度 `0.4`。区域每周简报有意与那些摘要文稿和 `whyMatters` 界面
不同：`scripts/regional-snapshot/weekly-brief.mjs`
首先用 `deepseek/deepseek-v4-flash` 尝试 OpenRouter，然后回退到 Groq
`llama-3.3-70b-versatile`，并向两个提供商都发送温度
...
合并两个报道之前要求实质性关联，并通过相同的形态和专有名词锚定门控来
验证缓存命中和新输出。按报道的提示也尽可能要求指明行为者，并可使用
RSS 描述作为锚定上下文。
...
- 信息流清单与摘要构建：`server/meridian/news/v1/_feeds.ts`，
  `server/meridian/news/v1/list-feed-digest.ts`
- 关键词分类器：`server/meridian/news/v1/_classifier.ts`
- API 契约：`proto/meridian/news/v1/list_feed_digest.proto`
