---
title: "国家韧性指数"
description: "面向 196 个国家的实时韧性评分体系，覆盖 6 大领域与 20 个细分维度 —— 一个每 6 小时刷新一次的 0-100 综合指数，采用 OECD、JRC 等国际对等标准进行同侪基准比较，用于衡量国家治理、经济、社会与安全韧性的相对强弱、动态变化与结构性差距。"
---

Meridian 国家韧性指数（CRI）以 0-100 分对 196 国公开可排名范围进行评分，将长期结构能力与当前运营压力结合为可操作的韧性指标。上游静态/来源源可以包含更广泛的领土集合，但公开的 CRI 排名在分数发布前过滤到可排名范围。CRI 不依赖静态国家风险评级，而是每 6 小时从官方和权威来源更新，并公开完整的来源溯源、覆盖率和插补上下文，让分析师能准确看到分数*为什么*变动以及其中多少是真实数据与插补数据。

本文档描述指数**当前发布**的行为。版本化有两个独立维度：

- **响应形状**：`schemaVersion: "2.0"` 是当前默认值。每个响应携带一个真实的 `pillars[]` 数组，将六个领域重新分组为结构准备度/实时冲击暴露/恢复能力。支柱分数使用每个成员领域的已发布设计权重乘以该领域的平均维度覆盖率。遗留的 `schemaVersion: "1.0"` 形状（支柱为空）仍可通过 `RESILIENCE_SCHEMA_V2_ENABLED=false` 环境标志获取，保留一个发布周期。
- **评分公式**：顶层 `overall_score` 是 v2 非补偿性支柱组合公式，带最小支柱惩罚。遗留的六领域加权聚合仍保留在代码中作为 `RESILIENCE_PILLAR_COMBINE_ENABLED=false` 时的回退路径，但生产和验证 cron 已在支柱组合公式上激活。年度参考版是冻结的、引用级质量的工件；它有意与实时部署清单分离。

对于实时部署状态，请使用[运行时清单](https://api.worldmonitor.app/resilience/v1/get-runtime-manifest)。它报告活动公式标签、静态 `dataVersion`、排名缓存元数据、安全派生构造版本如 `constructVersions.energy`（`legacy` 或 `v2`），以及公共 `scoreInterval`/`rankStable` 支持路径的 `intervals` 可用性元数据，而不暴露部署标识符、原始 `RESILIENCE_*` 标志状态或内部缓存键。参考版仍是冻结的已发布包；不应将其视为当前生产中活动运行时状态的证据。

以下文档描述**当前发布**状态：schemaVersion `"2.0"` 形状，6 领域 × 20 活跃维度 × 3 支柱（加上 2 个结构性退役维度为模式连续性在注册表中保留，`coverage=0`），支柱组合惩罚 `overall_score`，以及能源 v2 构造。[支柱组合分数激活](#pillar-combined-score-activation-active) 小节记录了激活证据和回退机制。2026-06-02 实时运行时清单报告 `formulaTag="pc"` 和 `constructVersions.energy="v2"`，`/api/health` 对三个必需的能源 v2 种子检查（`lowCarbonGeneration`、`fossilElectricityShare` 和 `powerLosses`）报告 `OK`。
...
项目 1-3 是现已活跃的能源 v2 修复；项目 4-6 是已落地的修复，此处保留作为活跃注册表有 20 个评分维度加 2 个为模式连续性保留的退役维度的历史记录。
...
三个仪表盘面均可免费查看。直接分数和排名 API 调用需要正常的 Pro/API 认证路径，而运行时清单在 `/api/resilience/v1/get-runtime-manifest` 公开；请参见 [Resilience service](/api/ResilienceService.openapi.yaml) 了解 HTTP 合约。

## 概览

Meridian 国家韧性指数以 0-100 分对 196 国公开可排名范围评分，覆盖 6 个领域和 20 个活跃维度（加上 2 个结构性退役维度为模式连续性在注册表中以 `coverage=0` 保留）。排名处理器仍可将低置信度或标题不合格的国家路由到 `greyedOut[]`，但可排名范围本身由提交的 UN 成员 + SAR 白名单固定。它将结构基线指标（治理质量、健康基础设施、财政能力）与实时压力信号（网络威胁、冲突事件、航运中断）和恢复能力指标（财政空间、储备、进口集中度）结合，产出每 6 小时更新的单一韧性分数。

数据来源于官方和权威提供商：世界银行、IMF、WHO、WTO、UNHCR、UCDP、BIS、IEA、FAO、无国界记者和经济学与和平研究所等。
...
| 能源 | `energy` | 0.11 | 能源 |
| 社会与治理 | `social-governance` | 0.19 | 治理、社会凝聚力、冲突与流离失所、信息 |
| 健康与食品 | `health-food` | 0.13 | 健康与公共服务、食品与水 |
| 恢复 | `recovery` | 0.25 | 财政空间、外债覆盖、进口集中度、国家连续性、流动储备充足性、主权财政缓冲 |

权重总和为 1.00。权威值位于 `server/meridian/resilience/v1/_dimension-scorers.ts` 的 `RESILIENCE_DOMAIN_WEIGHTS`；如果此表与代码不一致，以代码为准。在活跃的 `pc` 公式中，绝对顶层影响力还取决于外层支柱权重。
...
| bisLbsXborderPctGdp | BIS CBS（`WS_CBS_PUB`）按母国外国债权总和（美国/英国/主要欧盟/瑞士/日本/加拿大/澳大利亚/新加坡）占 GDP 百分比；U 形区间 — 隔离（低于 5%）和过度暴露（高于 60%）均低分 | 越低越好（U 形） | 60 - 25 | 0.30 | BIS CBS | 季度 |
| fatfListingStatus | FATF AML/CFT 列表状态 — 黑名单（呼吁行动）→ 0，灰名单（加强监控）→ 30，合规 → 100 | 越高越好 | 0 - 100 | 0.20 | FATF | 月度 |
| financialCenterRedundancy | 拥有非平凡（>1% GDP）外国债权的不同 BIS CBS 按母国报告者计数；奖励多对手方金融中心，平衡组件 2 过度暴露惩罚 | 越高越好 | 1 - 10 | 0.15 | BIS CBS | 季度 |

**覆盖率**：WB IDS 仅发布约 125 个 LMIC；HIC 落入 BIS CBS 结构性暴露组件（覆盖约 200 国）。FATF + BIS CBS 一起有效覆盖所有清单国家。
...
两个新维度的 `0.5` 权重将其对恢复分数的综合贡献限制在约 20%，符合计划中主权财富信号补充而非主导经典流动储备和财政空间信号的方向。权重通过 `server/meridian/resilience/v1/_dimension-scorers.ts` 中的 `RESILIENCE_DIMENSION_WEIGHTS` 应用；`_shared.ts` 中的 `coverageWeightedMean` 在计算领域均值前将每个维度的覆盖率乘以其权重，因此 `coverage=0`（退役）的维度无论权重如何仍贡献零。
...
当数据缺失时，系统用四类之一标记，使下游消费者可区分"没有发生"与"我们不知道"与"上游中断"与"维度不适用于此国家"。分类法定义在 `server/meridian/resilience/v1/_dimension-scorers.ts` 中作为导出的 `ImputationClass` 类型。
...
| 键 | 类型 | TTL | 写入方 | 读取方 |
|---|---|---|---|---|
| `resilience:score:v25:{countryCode}` | JSON | 6 小时 | `server/meridian/resilience/v1/_shared.ts` 中的 `buildResilienceScore` | `getResilienceScore` 处理器 |
| `resilience:ranking:v25` | JSON | 12 小时 | `getResilienceRanking` 暖路径，仅当至少 90% 国家已评分时（`RANKING_CACHE_MIN_COVERAGE = 0.90`） | `getResilienceRanking` 处理器 |
| `resilience:history:v20:{countryCode}` | 有序集合 | 无限期，修剪至 30 天 | 评分期间 `appendHistory` | 趋势和 `change30d` 计算 |
| `resilience:intervals:v9:{countryCode}` | JSON | 7 天；通过 `seed-meta`/健康频率监控新鲜度 | `scripts/seed-resilience-scores.mjs` | `getResilienceScore`（可选 `scoreInterval` 字段）和 `getResilienceRanking`（`rankStable`） |
...
公共运行时清单有意不回显这些 Redis 键名。其 `intervals` 对象报告使用固定公共样本国家、当前区间方法论标签和从区间新鲜度元数据或样本区间载荷得到的最新安全观测时间戳的派生可用性检查。如果区间数据缺失、对活动公式过期或由不同方法论产出，`intervals.available` 为 `false` 而非抛出；这是用户面 `scoreInterval` 和 `rankStable` 支持当前未由可读区间数据支撑的公共审计信号。
...
4. 对 20 个活跃维度中的每一个，应用评分公式小节中的公式和维度与指标表中的目标阈值。对缺失信号，查阅本文档中的插补分类法表。
...
| **来源质量** | 7.0 | 世界银行、IMF、WHO、IEA、UNHCR、UCDP、IPC、BIS、FAO、RSF、GPI：均权威。间隙：精选列表来源（BIS 约 40 经济体、WTO）不覆盖完整 Meridian 国家集，这是 `unmonitored` 插补类存在的原因。Phase 2 T2.9 添加语言归一化信息信号以减少英语媒体偏差。 |
...
**当前发布版本**。参考级升级计划 Phase 2（`docs/internal/country-resilience-upgrade-plan.md`）。响应形状重建已上线：每个响应现在携带真实领域加权、覆盖率缩放的 `pillars[]` 数组，将六个领域重新分组为结构准备度、实时冲击暴露和恢复能力。恢复领域添加六个新维度，验证套件（跨指数基准、结果回测、敏感度分析）门控激活的支柱组合公式。顶层 `overall_score` 现在是部分非补偿支柱组合分数（见[支柱组合分数激活](#pillar-combined-score-activation-active)）；六领域加权聚合在 `RESILIENCE_PILLAR_COMBINE_ENABLED=false` 时仅作为回退路径保留。
...
- **T2.3**（#2990/#3954）：三支柱聚合形状发布并激活。每个响应现在在 `pillars[]` 携带真实领域加权、覆盖率缩放支柱分数和支柱覆盖率。支柱权重：结构准备度 0.40、实时冲击暴露 0.35、恢复能力 0.25。惩罚因子 `(1 − α × (1 − min_pillar / 100))`，α = 0.5，作为 `penalizedPillarScore` 在 `server/meridian/resilience/v1/_shared.ts` 中实现，并由敏感度套件行使。**顶层 `overall_score` 是生产中的惩罚支柱组合形式**；六领域加权聚合保留为标志关闭回退路径。
...
5. **投影和权威快照**：`docs/snapshots/resilience-ranking-pillar-combined-projected-2026-04-21.json` 携带激活前使用的 52 国预览表。`docs/snapshots/resilience-ranking-2026-05-28.json` 携带标志激活后的权威全范围实时捕获；它早于 P1-1 领域加权支柱聚合修复，作为 P1-1 前历史捕获保留，直到生成下一次实时全范围捕获。

回退：设 `RESILIENCE_PILLAR_COMBINE_ENABLED=false`，刷新当前 `resilience:score:v25:*`、`resilience:ranking:v25` 和 `resilience:history:v20:*` 键（或等待 TTL 过期）。6 领域公式与支柱组合在 `_shared.ts` 中并存，回退无需代码变更。
...
| **数据** | 9.0 | 6 领域 20 活跃维度（加上 2 个结构性退役维度保留在注册表中），47+ 指标。恢复能力在 Comtrade 配额/回填限制允许处使用真实进口 HHI 数据；`fuelStockDays` 从核心分数退役，仅作为实验注册表面保留。信号分层注册表标记每个指标 Core/Enrichment/Experimental 含覆盖率 + 许可审计。间隙：储备保证金集成、外部专家审查、正式刷新 SLA 和归属/解释面。 |
