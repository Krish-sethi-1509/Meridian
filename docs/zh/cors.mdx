---
title: "CORS"
description: "Meridian 跨域请求保护机制的完整技术说明，以及在添加新 API 端点时需要注意的处理方式：涵盖域名允许列表配置、预检 OPTIONS 请求处理、Cloudflare Worker 门控层、凭据传递策略、公开与私有端点区分以及常见 CORS 错误排查。"
---
...
| 模式 | 匹配 |
| --- | --- |
| `(*.)?meridian.app` | 生产环境 + 子域名（`tech.`、`finance.` 等） |
| `meridian-*-elie-*.vercel.app` | Vercel 预览部署 |
...
- `Content-Type`
- `Authorization`
- `X-Meridian-Key`（用于桌面/第三方访问的 API key）。密钥管理详情请参见 [API Key 门控](/zh/api-key-deployment)。
- `X-Api-Key`
- `X-Widget-Key`
- `X-Pro-Key`
...
通过 `Access-Control-Expose-Headers`，浏览器可见的响应头包含 `Mcp-Session-Id`、`WWW-Authenticate`、`Retry-After` 与 `X-Billing-Verification`，以便 MCP 客户端可以继续会话、重新认证、遵循退避提示，并在无需解析响应体的情况下区分「可重试的计费校验故障」与「终态的订阅失效」。参见[错误处理](/zh/usage-errors)。
