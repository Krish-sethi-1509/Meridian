---
title: "沙盒与测试环境"
description: "无需 API 密钥或配额即可测试 Meridian API：使用确定性的沙盒样例响应、匿名 MCP 发现接口和只读数据面，验证智能体、开发者工具与 CI 集成；每个示例都标注生产接口、请求参数、响应结构和安全边界，便于在不影响实时数据的情况下完成开发测试，并可在本地自动化测试与团队回归中重复使用。"
---

Meridian 提供沙盒环境，让智能体和集成方可以在**无需 API 密钥、不消耗配额、且完全不影响生产数据**的前提下进行开发和测试。沙盒由三部分组成:
...
- **符合 schema** — 每个 `response.body` 都通过 [openapi.json](https://meridian.app/openapi.json) 中对应操作的响应 schema 校验。
- **明确标注为合成数据** — 每个 fixture 都带有 `"sandbox": true`。切勿将 fixture 负载当作实时数据。

## 2. 生产 MCP 服务器上的匿名、免配额发现接口

生产 MCP 服务器 `https://meridian.app/mcp` 允许你在无需认证、不消耗每日配额的情况下探索完整的工具面:
...
[文档 MCP 服务器](/zh/mcp-overview)(`https://www.meridian.app/docs/mcp`)完全公开——无需任何密钥即可通过 MCP 搜索和阅读本文档。
...
1. 在 [meridian.app/pro](https://meridian.app/pro) 申请密钥，并通过 `X-Meridian-Key: wm_<40位十六进制>` 请求头发送(或使用 [OAuth 2.1](/zh/api-oauth)，`scope=mcp`)。
