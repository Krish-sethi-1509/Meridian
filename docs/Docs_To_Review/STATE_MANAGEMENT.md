...
sequenceDiagram
    ...
    setTheme->>localStorage: setItem('meridian-theme', 'light')
...
export async function initDB(): Promise<IDBDatabase> {
  // Opens or creates meridian_db v1
...
- **Toggles key:** `meridian-runtime-feature-toggles` in `localStorage`
...
| `meridian-theme` | Theme preference | `'dark' \| 'light'` | `src/utils/theme-manager.ts` |
| `panel-order` | Panel arrangement order | `string[]` (JSON) | `App.ts` |
| `meridian-panel-spans` | Panel grid sizes | `Record<string, number>` (JSON) | `src/components/Panel.ts` |
...
| `meridian-runtime-feature-toggles` | Desktop feature toggles | `Record<RuntimeFeatureId, boolean>` (JSON) | `src/services/runtime-config.ts` |
| `meridian-persistent-cache:{key}` | Persistent data cache entries | `CacheEnvelope<T>` (JSON) | `src/services/persistent-cache.ts` |
| `wm-update-dismissed-{version}` | Dismissed update notifications | `'1'` | `App.ts` |
| `meridian-panel-order-v1.9` | Panel order migration flag | `'done'` | `App.ts` (one-time migration) |
| `meridian-tech-insights-top-v1` | Tech variant migration flag | `'done'` | `App.ts` (one-time migration) |
