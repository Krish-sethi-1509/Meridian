...
4. [Full Variant Panels](#4-full-variant-panels-meridianio)
5. [Tech Variant Panels](#5-tech-variant-panels-techmeridianio)
6. [Finance Variant Panels](#6-finance-variant-panels-financemeridianio)
...
|---------|--------|-------|-------------|
| `full` | meridian.app | Geopolitical intelligence, OSINT, defense | 44 |
| `tech` | tech.meridian.app | Technology, AI/ML, startups, VC | Tech-focused |
| `finance` | finance.meridian.app | Markets, trading, macro, commodities | Finance-focused |
| `commodity` | commodity.meridian.app | Mining, metals, energy, supply chains | Commodity-focused |
| `happy` | happy.meridian.app | Positive news, breakthroughs, conservation | Happy-focused |
| `energy` | energy.meridian.app | Oil, gas, chokepoints, energy security | Energy-focused |
...
|-----|---------|------------|
| `meridian-panel-spans` | Panel height/span sizes (1–4) | `Panel.ts` |
| `panel-order` | Drag-and-drop panel ordering | `App.ts` |
| `meridian-variant` | Last-active variant (triggers reset on change) | `App.ts` |
| `meridian-panel-order-v1.9` | Migration flag for v1.9 panel layout | `App.ts` |
...
Span values are persisted per-panel in the `meridian-panel-spans` localStorage key as a JSON object `{ [panelId]: spanNumber }`.
...
## 4. Full Variant Panels (meridian.io)
...
## 5. Tech Variant Panels (tech.meridian.io)
...
## 6. Finance Variant Panels (finance.meridian.io)
...
| Panel sizes/spans | `meridian-panel-spans` | `Record<string, number>` JSON | All span-1 | ✅ |
...
This ensures users switching between variant domains (e.g. from meridian.io to tech.meridian.io) get a clean default experience for the new variant.
