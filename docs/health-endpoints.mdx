...
description: "Two health endpoints Meridian exposes for monitoring data pipeline integrity — Edge Runtime handlers querying Upstash Redis for system state."
---
...
| Condition | Status |
| --- | --- |
| Every decision-critical country inside the 72-hour content budget | `OK` |
...
Health pins both that set and the 72-hour budget in its own config rather than
...
refresh every run they are a cache miss. Without that reservation the rotation
period (`ceil(174 / 30)` runs at a 12-hour cadence ~ 72 hours) equals the
content budget, and the alarm would measure the rotation rather than an
incident.

The reservation only reaches a country that is a cache **miss**. A country stays
a cache hit while the upstream `max(date)` it already holds is unchanged, so
when the upstream feed itself freezes, the decision-critical countries never
enter the refresh queue and no amount of slot reservation makes their content
current. In that state the check is doing its job precisely by staying
`STALE_CONTENT`: the content is genuinely old, the seeder cannot fix it, and the
alarm is reporting an upstream outage rather than an internal one.
Fleet-wide content staleness is normal by construction: the seeder refreshes at
most 30 countries per 12-hour run, so a full sweep takes about 72 hours and the
tail of the rotation is routinely past budget. Alarming on that would produce a
warning that is always lit and never actionable. The fleet-wide counts
...
The 72-hour budget is deliberately the same one
...
The surfaces differ in exactly one input class, deliberately. This endpoint
cannot tell a marker it failed to read from one that is genuinely absent — both
land in the same bucket — so a read error still grants the grace. MCP treats an
unreadable marker as *unknown* and fails closed, because a grace granted on the
absence of evidence never expires: an evicted or renamed marker would otherwise
disable the alarm permanently. So for a pre-activation key whose marker read
errors, this endpoint reports `OK` while MCP reports `stale: true`. The
disagreement only ever runs in that direction — MCP over-alarming — and never
the direction that hides stale content. Aligning this endpoint is tracked in
[#6095](https://github.com/koala73/worldmonitor/issues/6095).
