...
description: "Bootstrap, health, version, cache-purge, and user-preference endpoints — the plumbing every Meridian client talks to on startup and cleanup."
---
...
- **Server-to-server**: call `https://api.worldmonitor.app/api/bootstrap` directly with `X-Meridian-Key: wm_...`. There is no separate gateway host, token exchange, activation step, or IP allow-list requirement for this endpoint.
...
Returns the latest **GitHub Release** of `koala73/meridian`. Used by the desktop app to detect a newer published release and prompt the user to update. It is **not** the currently-deployed Vercel commit.
...
- `X-Meridian-Desktop-Timestamp`: Unix epoch milliseconds, within 5 minutes of server time.
- `X-Meridian-Desktop-Signature`: `sha256=<hex HMAC-SHA256>`.
