...
## Findings
- **`server/meridian/intelligence/v1/get-country-intel-brief.ts`** — reads `req.framework`, truncates to 2000 chars, passes to `callLlm` as `systemAppend`. No entitlement check.
- **`server/meridian/intelligence/v1/deduct-situation.ts`** — same pattern, no entitlement check.
- **`server/meridian/news/v1/summarize-article.ts`** — reads `req.systemAppend`, no entitlement check.
...
- Flagged by: security-sentinel, architecture-strategist, learnings-researcher (meridian-pro-panel-gating skill)
...
## Technical Details
- Files: `server/meridian/intelligence/v1/get-country-intel-brief.ts`, `server/meridian/intelligence/v1/deduct-situation.ts`, `server/meridian/news/v1/summarize-article.ts`
- PR: koala73/meridian#2380
- Reference: meridian-pro-panel-gating skill (4-layer checklist)
