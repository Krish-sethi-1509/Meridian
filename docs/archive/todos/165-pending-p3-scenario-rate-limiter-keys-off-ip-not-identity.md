...
- See MEMORY: `feedback_is_caller_premium_trusted_origin.md` — the API key is extractable from `X-Meridian-Key`
...
```ts
const apiKey = req.headers.get('x-meridian-key');
...
## Acceptance Criteria
- [ ] Rate limit key uses `X-Meridian-Key` when present
