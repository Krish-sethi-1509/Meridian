...
- **`server/meridian/intelligence/v1/get-country-intel-brief.ts:38`** — `frameworkRaw = req.framework.slice(0, 2000)` — truncation only, no sanitization
- **`server/meridian/intelligence/v1/deduct-situation.ts`** — same
- **`server/meridian/news/v1/summarize-article.ts`** — same
...
## Technical Details
- Files: `server/_shared/llm.ts`, `server/meridian/intelligence/v1/get-country-intel-brief.ts`, `server/meridian/intelligence/v1/deduct-situation.ts`, `server/meridian/news/v1/summarize-article.ts`
