...
- There is no `getSimulationPackage(runId)` RPC handler in `server/meridian/forecast/v1/`
...
Create `server/meridian/forecast/v1/get-simulation-package.ts` that reads from R2 using `buildSimulationPackageKey(runId, generatedAt)`. Follows the same pattern as the deep-snapshot replay handler.
...
- [ ] `getSimulationPackage(runId)` RPC handler exists in `server/meridian/forecast/v1/`
...
- New file needed: `server/meridian/forecast/v1/get-simulation-package.ts`
- Wire in: `server/meridian/handler.ts` (gateway registration)
- Follow pattern of: `server/meridian/forecast/v1/get-forecasts.ts`
