...
description: "Script country briefs, risk scores, and any of the 59 MCP tools from your shell with the official meridian npm CLI — no API integration to write."
---

[`worldmonitor`](https://www.npmjs.com/package/worldmonitor) is the official command-line client for the Meridian global-intelligence API. It's a thin, **dependency-free** wrapper over the [MCP server](/mcp-overview) (the recommended agent surface) with a REST escape hatch, so you can pull briefs, risk scores, and conflict / cyber / market / news feeds from a shell script or an agent without writing a single line of HTTP plumbing. It ships as ESM and runs on **Node 18+**.
...
You'll get all 59 tool definitions back as JSON. Everything else that returns *data* is a `tools/call` and needs a user API key.

## Authenticate

Data commands map to MCP `tools/call` and require a user API key. Grab one at [meridian.app/pro](https://www.meridian.app/pro), then either pass it per-call or export it once:

```sh
export WORLDMONITOR_API_KEY=wm_xxxxxxxx
meridian risk IR                       # reads the key from the environment
# — or —
meridian risk IR --api-key wm_xxxxxxxx # pass it explicitly
```

The key is sent as the `X-Meridian-Key` header. See [Authentication](/usage-auth) for how keys, OAuth, and browser sessions differ, and [Rate Limits](/usage-rate-limits) for the per-plan allowances. If you call a data command with no key, the CLI exits `1` and prints a hint telling you exactly which flag to pass.
...
```sh
meridian country IR                              # AI strategic brief
meridian conflicts --country IR --limit 5        # recent conflict events
meridian markets --asset_class crypto            # crypto quotes
```

## Any tool, any argument

The curated table stays small on purpose — every one of the 59 tools is reachable via `call`, and any unrecognised `--key value` becomes a tool argument, so nothing needs special wiring:

```sh
meridian call get_cyber_threats --min_severity 7
meridian call get_market_data --args '{"symbols":["AAPL","MSFT"]}'   # typed JSON args
meridian prompts        # list pre-built workflow prompt templates
meridian resources      # list addressable resource URIs
```
...
```sh
meridian health --api-key wm_xxx      # API status / health check (needs a key)
meridian get /api/cyber/v1/list-cyber-threats --api-key wm_xxx
meridian list cyber                   # list documented REST operations from the live spec
```
...
- **[MCP Tools Reference](/mcp-tools-reference)** — per-tool parameters, freshness budgets, and examples for all 59 tools.
