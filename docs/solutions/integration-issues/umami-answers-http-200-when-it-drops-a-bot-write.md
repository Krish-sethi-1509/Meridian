...
PR #5964 added a browser-side Sentry warning for every Umami collector write failure. It fired 45 events across 24 users in its first 32 minutes and caught **zero** instances of the umami#4183 `session_data` race it was built to detect. Most of the "failures" were HTTP 200 successes, and most of those came from Meridian's own crawler.
...
**Umami's bot check answers `HTTP 200` with `{"beep":"boop"}` and stores nothing.** 20 of the 27 came from `Mozilla/5.0 (VISION-3.0-Meridian; +https://github.com/local/vision)` — our own agent — plus 3 HeadlessChrome. The remaining `network` failures are ad-blocked clients.
