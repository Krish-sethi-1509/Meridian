...
- **WORLDMONITOR-WR** was a Clerk SDK error loading Clerk’s own UI bundle from `clerk.meridian.app`. Although the stack named the locally bundled `clerk-*.js` asset, the message itself carried a stable, SDK-owned prefix: `[clerk] failed to load …`.
...
```text
Error: [clerk] failed to load https://clerk.meridian.app/npm/@clerk/ui@1/dist/ui.browser.js
```
