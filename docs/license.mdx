...
description: "Meridian licensing terms — AGPL-3.0-only source license, optional commercial licensing, and the separate trademark and branding policy."
---

Meridian source code is licensed under the [GNU Affero General Public License v3.0 (AGPL-3.0-only)](https://www.gnu.org/licenses/agpl-3.0.html).
...
This source-code license is separate from the Personal, Pro Business, API Starter, API Business, and Enterprise licenses that govern use of the hosted Service and its outputs. See the [Terms of Service](/terms#subscription-license-scope) for those plan-specific rights.

**Exception — official client packages are MIT.** The thin API clients — the [`worldmonitor` npm CLI](https://www.npmjs.com/package/worldmonitor) (`cli/`), and the [Python](https://pypi.org/project/worldmonitor-sdk/) (`sdk/python/`), [Ruby](https://rubygems.org/gems/worldmonitor) (`sdk/ruby/`), and [Go](https://pkg.go.dev/github.com/koala73/worldmonitor/sdk/go) (`sdk/go/`) SDKs — are licensed under MIT (see the `LICENSE` file in each directory) so they can be embedded in any application without copyleft obligations. This exception covers only those client packages: the Meridian platform, server, and dashboard remain AGPL-3.0-only.
...
- use Meridian for personal, educational, research, nonprofit, or commercial purposes
...
- combine Meridian code into proprietary products in ways the AGPL would not permit
...
The AGPL covers copyright permissions for the code. It does not grant rights to use the Meridian name, logo, visual identity, or official project branding.
...
**Using Meridian's public API in your own tool**: API terms of service may apply separately from the source-code license.
