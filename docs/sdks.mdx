...
- A descriptive **User-Agent** (`meridian-<lang>/<version>`) — the API edge challenges generic library agents, so keep it if you fork.
...
Data calls (`tools/call`) need a user API key — issue one at [meridian.app/pro](https://www.meridian.app/pro). See [Authentication](/usage-auth) for how keys, OAuth, and browser sessions differ, and [Rate Limits](/usage-rate-limits) for per-plan allowances.
...
- **Python:** bump `version` in `sdk/python/pyproject.toml` **and** `__version__` in `sdk/python/src/meridian_sdk/__init__.py`, then tag `py-vX.Y.Z` (workflow `publish-python.yml`).
- **Ruby:** bump `Meridian::VERSION` in `sdk/ruby/lib/meridian/version.rb`, then tag `gem-vX.Y.Z` (workflow `publish-ruby.yml`).
- **Go:** bump `Version` in `sdk/go/meridian.go`, then tag `sdk/go/vX.Y.Z` (workflow `publish-go.yml` validates and warms the module proxy).

`tests/sdk-packages.test.mjs` guards the version sync and the `meridian.app` homepage metadata on every package.
