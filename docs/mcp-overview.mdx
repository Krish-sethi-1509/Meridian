...
<Info>
**Pro and API tiers can both connect via OAuth — no API key required.** Pro subscribers click _"Sign in with Meridian Pro"_ on the consent page; API Starter / Business / Enterprise users may sign in the same way OR paste a `wm_…` key. Free-tier users see a `403 INSUFFICIENT_TIER` at the OAuth step.
</Info>
...
| Endpoint | Purpose |
|----------|---------|
| `https://meridian.app/mcp` | JSON-RPC server (Streamable HTTP transport; JSON responses by default, SSE responses when clients advertise `text/event-stream`; `initialize` defaults to protocol `2025-03-26`) |
...
| `https://meridian.app/.well-known/oauth-protected-resource` | Resource server metadata (RFC 9728) |

Server identifier: `meridian` v1.15.0.
...
Meridian's static server card at `/.well-known/mcp/server-card.json` advertises protocol version `2025-06-18`, and the live `initialize` handshake negotiates it by default — so the advertised floor and the negotiated version stay in lock-step:
...
Meridian supports the Streamable HTTP POST flow with either JSON or SSE responses:
...
Pro subscribers (and API Starter+ users who'd rather not paste a key) can authorize MCP clients via their existing Meridian account — no API key needed.
...
1. **Add the server URL** in your MCP client. The canonical entrypoint is:
   ...
   (`https://meridian.app/mcp` works too — it proxies the same handler.)

2. **Click "Sign in with Meridian Pro"** on the consent page. This is the default CTA. You'll bounce through `worldmonitor.app/mcp-grant` (Clerk-protected — sign in if needed), then back to `api.worldmonitor.app/oauth/authorize-pro`, then to your client's redirect.
...
- Manage connected clients in **[Settings → Connected MCP clients](https://meridian.app/settings)**.
...
|------|------------|------------|-----------------------|-------|
| **Free** | ❌ No | — | — | OAuth flow returns `403 INSUFFICIENT_TIER`. Upgrade at [meridian.app/pro](https://www.meridian.app/pro). |
...
Meridian also surfaces current MCP plan-limit notices in Settings and sends a bounded-cadence email when a paid user approaches or exceeds their plan allowance. Notices are informational and action-oriented: they offer retry/reset guidance, checkout when the next tier is self-serve, or support contact when it is not. They do not automatically upgrade the account or create overage charges.
...
In addition to the 59 tools, Meridian exposes MCP prompts and resources so clients can discover common workflows and address stable data slices without inventing tool plans from scratch.
...
|--------------|--------------|
| `meridian://seed-meta/freshness` | Stock market-data bootstrap write-age only: `cached_at` and `stale` for `seed-meta:market:stocks`. Does **not** certify sector valuation completeness — use `get_market_data` `valuationCoverage` (`sourceStatus`, unavailable/last-good/diagnostics). Cheap seeder health probe — no auth, no quota. |
...
|-----------------------|--------------|
| `meridian://countries/{iso2}/risk` | Country risk score, component breakdown, travel advisory, and sanctions exposure. `{iso2}` is lowercase alpha-2, for example `de` or `us`. |
| `meridian://chokepoints/{slug}/status` | Chokepoint transit summary and risk narrative. `{slug}` is one of the published kebab-case chokepoint slugs, such as `suez`, `strait-of-hormuz`, or `bab-el-mandeb`. |
| `meridian://markets/{symbol}/quote` | Single-symbol market quote slice. `{symbol}` is uppercase, for example `AAPL`, `GC=F`, or `BTC-USD`. |
...
|-----------------|-------------|---------|
| `ui://meridian/country-risk.html` | `get_country_risk` | CII score, unrest/conflict/security/news component breakdown, travel-advisory level, and OFAC sanctions exposure. |
| `ui://meridian/world-brief.html` | `get_world_brief` | The AI-summarised global intelligence brief as readable paragraphs, plus grounding headlines and source articles. |
| `ui://meridian/country-brief.html` | `get_country_brief` | The AI-synthesised per-country brief as paragraphs, with the analytical framework lens and grounding sources. |
| `ui://meridian/market-radar.html` | `get_market_data` | The Fear & Greed composite plus per-asset-class quote tables (equities, commodities, crypto, Gulf, sectors) with signed, colour-coded change. |
| `ui://meridian/chokepoint-monitor.html` | `get_chokepoint_status` | Per-chokepoint rolling transit summaries (today's count, week-over-week change, tanker split) with a risk-level badge. |
| `ui://meridian/news-intelligence.html` | `get_news_intelligence` | AI-classified top stories with category, alert flag, country, and source. |
| `ui://meridian/conflict-events.html` | `get_conflict_events` | Active armed-conflict events (belligerents, violence type, country, fatalities, date) from the UCDP feed. |
| `ui://meridian/natural-disasters.html` | `get_natural_disasters` | Recent earthquakes (USGS magnitude, place, time) and active wildfires (NASA FIRMS), grouped. |
| `ui://meridian/prediction-markets.html` | `get_prediction_markets` | Active event-contract odds grouped by category (geopolitical, tech, finance) with a probability bar per market. |
| `ui://meridian/forecasts.html` | `get_forecast_predictions` | AI-generated geopolitical and economic forecasts as probability cards (title, domain, region). |
...
Server-side with a direct API key — send it as `X-Meridian-Key`, **not** as a bearer token.
...
Seed-level health per key: [status.meridian.app](https://status.meridian.app/).
...
- [Pro features](https://www.meridian.app/pro)
