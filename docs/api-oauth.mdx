...
description: "Dynamic client registration, authorization, and token exchange endpoints that back the Meridian MCP server's OAuth 2.1 authentication flow."
---

Meridian runs a minimal OAuth 2.1 authorization server whose only client-facing purpose today is **granting access to the MCP server** at `/api/mcp`. It implements:
