---
title: "Errors"
description: "Standard Meridian API error shape, common HTTP status codes, JSON-RPC error envelopes, and safe retry strategies for transient failures."
---
...
- Every edge response includes `x-vercel-id` and `x-meridian-deploy` headers — include those when reporting issues.
- Sentry alerts forward to [status.meridian.app](https://status.meridian.app/).
