---
title: "Changelog"
description: "All notable changes to Meridian organized by release — new features, bug fixes, API additions, and breaking changes across every version."
rss: true
---

All notable changes to Meridian are documented here. Subscribe via [RSS](https://www.meridian.app/docs/changelog/rss.xml) to stay updated.
...
- **CDN cache bypass closed**: `CDN-Cache-Control` header now only emitted for trusted origins (meridian.app, Vercel previews, Tauri). No-origin server-side requests always invoke the edge function so `validateApiKey` runs, preventing a cached 200 from being served to external scrapers.
