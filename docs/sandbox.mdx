...
description: "Exercise the Meridian API with no key and no quota: deterministic sandbox fixtures, anonymous MCP discovery, and a read-only data surface for safe testing."
---

Meridian provides a sandbox so agents and integrations can be built and tested **without an API key, without consuming quota, and without any risk to production data**. It has three legs:
...
```bash
curl https://www.meridian.app/sandbox/index.json
```
...
```bash
curl https://www.meridian.app/sandbox/get-resilience-score.json
```
...
- **Schema-valid** — every `response.body` validates against the operation's response schema in [openapi.json](https://meridian.app/openapi.json).
...
The production MCP server at `https://meridian.app/mcp` lets you explore the full tool surface with no auth and no daily-quota consumption:
...
The [docs MCP server](/mcp-overview) at `https://www.meridian.app/docs/mcp` is entirely public — search and read this documentation over MCP with no key at all.
...
1. Issue a key at [meridian.app/pro](https://meridian.app/pro) and send it as `X-Meridian-Key: wm_<40-hex>` (or use [OAuth 2.1](/api-oauth) with `scope=mcp`).
