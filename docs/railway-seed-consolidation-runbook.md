...
After linking the CLI to the `meridian` production environment, audit the
live settings with:
...
- environment variable `RAILWAY_PROJECT_ID`: the `meridian` project ID.
...
`meridian-bootstrap`. Managed `r2.dev` access stays disabled and the bucket
...
|---|---|---|---|
| Shared routing and tier shape | `R2_ACCOUNT_ID`, optional `R2_ENDPOINT`, `R2_BOOTSTRAP_BUCKET=meridian-bootstrap`, `IRAN_EVENTS_ENABLED` | Railway production and Vercel production | Names plus the feature flag that controls `iranEvents` tier membership; values must match |
| Publisher | `R2_BOOTSTRAP_ACCESS_KEY_ID`, `R2_BOOTSTRAP_SECRET_ACCESS_KEY` | Railway production publisher only | Publisher can PUT and GET only in `meridian-bootstrap` |
| Edge reader | `R2_BOOTSTRAP_READ_KEY_ID`, `R2_BOOTSTRAP_READ_SECRET` | Vercel production only | Edge can GET; it cannot PUT or DELETE, and cannot read `meridian-data` |
...
5. Run the negative permission probes: publisher cannot access
   `meridian-data`; edge cannot write/delete in `meridian-bootstrap` and
   cannot read `meridian-data`.
...
| Service | ID | Type |
|---|---|---|
| meridian (ais-relay) | `a5f66d97-217f-44a0-a42d-5f3b67752223` | AIS relay + inline seeds |
...
The simplest approach: use Railway's "shared variables" or copy all env vars from the `meridian` (ais-relay) service, which has a superset of all API keys.
