---
title: "CORS"
description: "How cross-origin request protection works in Meridian and what to do when adding new API endpoints — allowlists, preflights, and Worker gate."
---
...
| Pattern | Matches |
| --- | --- |
| `(*.)?meridian.app` | Production + subdomains (`tech.`, `finance.`, etc.) |
| `meridian-*-elie-*.vercel.app` | Vercel preview deploys |
...
- `Content-Type`
- `Authorization`
- `X-Meridian-Key` (API key for desktop/third-party access). See [API Key Gating](/api-key-deployment) for key management details.
- `X-Api-Key`
- `X-Widget-Key`
- `X-Pro-Key`
...
Browser-visible response headers include `Mcp-Session-Id`, `WWW-Authenticate`, `Retry-After`, and `X-Billing-Verification` via `Access-Control-Expose-Headers` so MCP clients can continue sessions, re-authenticate, respect backoff hints, and tell a retryable billing-verification blip from a terminal lapse without parsing the response body. See [Error handling](/usage-errors).
