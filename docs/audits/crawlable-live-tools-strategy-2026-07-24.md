# Crawlable Live Tools for Meridian

## Decision

Meridian should build search-intent tools as progressively enhanced pages on the main domain, not as a disconnected SEO subdomain. The static page must be useful and source-labelled before JavaScript runs; live API data should add utility without becoming the only crawlable content.
...
- Use subfolders on `meridian.app`; do not split authority onto an SEO subdomain.
