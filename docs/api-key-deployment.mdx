...
- `api/_api-key.js` validates `X-Meridian-Key` header on sebuf routes
- **Origin-aware**: desktop origins (`tauri.localhost`, `tauri://`, `asset://`) require a key
- Web origins (`meridian.app`) pass through without a key
- Non-desktop origin with key header: key is still validated
- Invalid key returns `401 { error: "Invalid API key" }`

### CORS

`X-Meridian-Key` is allowed in both `server/cors.ts` and `api/_cors.js`.
...
| `server/cors.ts` / `api/_cors.js` | CORS headers with `X-Meridian-Key` |
| `src/components/MeridianTab.ts` | Settings UI for key + registration |
