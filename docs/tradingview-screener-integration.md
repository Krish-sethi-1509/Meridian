...
**Purpose:** Reference document for extending Meridian's finance hub with TradingView Screener data.
...
The TypeScript library is the one relevant for Meridian. It has full parity with the Python version (41/41 operations pass) plus a self-hosted REST server option.
...
Meridian's data flow: `Railway seeds Redis → Vercel reads Redis → Frontend RPC`
...
```typescript
// server/meridian/market/v1/list-tv-stock-screener.ts
...
| **No auth initially** | Delayed data is fine for Meridian's use case |
