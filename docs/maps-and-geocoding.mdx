...
description: "Static map assets, country geometry, forward and reverse geocoding services, and boundary overrides used across Meridian's map layers."
---

## R2 CDN — `maps.worldmonitor.app`

All large static map files are served from Cloudflare R2, **not** from Vercel. The R2 bucket `meridian-maps` is fronted by a CF-proxied custom domain:
...
```bash
# Single file
rclone copyto <local-path> r2:meridian-maps/<filename>
...
4. Upload to R2:
   ```bash
   rclone copyto public/data/country-boundary-overrides.geojson r2:meridian-maps/country-boundary-overrides.geojson
   ```
