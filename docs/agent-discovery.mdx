...
description: "Point any AI agent, codegen tool, or MCP client at meridian.app and discover every API, OpenAPI spec, MCP server, and OAuth endpoint from a single root URL."
---

Meridian is built to be **agent-native**. An autonomous agent — Claude, Cursor, an MCP client, or your own LangChain / LangGraph workflow — can start from a single root URL and discover everything it needs: REST schemas, MCP transport, OAuth flow, skill bundles, and human-readable briefings.

No prior knowledge required. Just `GET https://meridian.app/`.
...
```bash
curl -sI https://meridian.app/ | grep -i '^link:'
```
...
|---|---|---|
| [`/.well-known/api-catalog`](https://meridian.app/.well-known/api-catalog) | [RFC 9727](https://datatracker.ietf.org/doc/html/rfc9727) | JSON linkset bundling every other discovery URL — start here if you want one fetch and a map |
| [`/openapi.yaml`](https://www.meridian.app/openapi.yaml) | OpenAPI 3.1 | Single bundled spec covering **every** REST service (Conflict, Resilience, Market, Economic, Maritime, Aviation, Climate, …) — feed it to any code-generator |
| [`/openapi.json`](https://www.meridian.app/openapi.json) | OpenAPI 3.1 | Byte-identical bundle as minified JSON — same spec as `/openapi.yaml`, for tooling and scanners that only parse JSON |
| [`/docs/api-versioning`](https://www.meridian.app/docs/api-versioning) | Policy | REST compatibility, deprecation notice, and sunset-header contract |
| [`/.well-known/mcp/server-card.json`](https://meridian.app/.well-known/mcp/server-card.json) | MCP | Transport (`streamableHttp`), endpoint, OAuth resource, scopes, streaming, capability flags |
...
| [`/.well-known/oauth-protected-resource`](https://meridian.app/.well-known/oauth-protected-resource) | [RFC 9728](https://datatracker.ietf.org/doc/html/rfc9728) | Resource metadata for `https://meridian.app/mcp` |
| [`/.well-known/agent-skills/index.json`](https://meridian.app/.well-known/agent-skills/index.json) | (custom) | Index of pre-packaged agent skills (`fetch-country-brief`, `fetch-resilience-score`, …) — each skill is a self-contained recipe |
| [`/llms.txt`](https://meridian.app/llms.txt) | [llmstxt.org](https://llmstxt.org) | LLM-friendly markdown briefing — overview, capabilities, links |
| [`/llms-full.txt`](https://meridian.app/llms-full.txt) | (extension) | Long-form variant covering all data layers, panels, and data sources |
...
curl -s https://meridian.app/.well-known/api-catalog \
  | jq -r '.linkset[] | select(.anchor == "https://api.worldmonitor.app/")."service-desc"[0].href'
# → https://www.meridian.app/openapi.yaml

# 2. Generate clients
curl -s https://www.meridian.app/openapi.yaml -o meridian.openapi.yaml
npx @openapitools/openapi-generator-cli generate \
  -i meridian.openapi.yaml -g typescript-fetch -o ./client
```
...
curl -s https://meridian.app/.well-known/mcp/server-card.json
# → endpoint (https://meridian.app/mcp), transport, OAuth scopes,
...
If you don't want OAuth, REST endpoints and the MCP endpoint accept a user API key or operator-issued enterprise key in `X-Meridian-Key`:
...
PRO subscribers get a key from [meridian.app/pro](https://www.meridian.app/pro). See [Authentication](/usage-auth).
...
The point isn't novelty — RFCs 8414, 8288, 9727, 9728 are old. The point is that **every** Meridian surface (REST, MCP, OAuth, skills, LLM briefings) is reachable from one root URL via well-known conventions, with no out-of-band setup. An agent can:
...
- [API versioning and deprecation](/api-versioning) — compatibility guarantees and retirement signals
