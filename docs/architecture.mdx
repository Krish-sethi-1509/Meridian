...
description: "Design principles, intelligence tradecraft, and algorithmic decisions behind Meridian — how the map-first dashboard is engineered and why."
---
...
| **Type-safe data flow**             | Discriminated union markers (`_kind` field), proto-generated typed clients/servers, and exhaustive `switch` matching ensure compile-time safety across 15+ marker types, 35 service domains, and 56 map layers. Adding a new data type produces compiler errors at every unhandled site.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                |

## Internationalization and Search Indexing

Meridian currently uses **application localization**, not separate localized
search documents. A `?lang=` parameter remains supported as shareable user
preference state: the client loads the requested interface dictionary, updates
the rendered document language and direction, and preserves the base page
canonical. Raw apex, dashboard, and Pro HTML is English, so those documents
advertise only `x-default` and `en`, both pointing at the self-canonical URL.
Query-string locale URLs do not belong in hreflang, robots, or sitemap output.

Publishing any non-English hreflang entry requires a separately reviewed
locale-prefixed static or server-rendered architecture, such as `/fr/` and
`/fr/dashboard`. Every advertised page must meet the complete localized-page
contract:

- translated visible main content and metadata;
- a correct `<html lang>` declaration;
- a stable crawlable URL that returns HTTP 200 and remains indexable;
- a self-referencing canonical;
- self-referencing and reciprocal hreflang entries across the cluster; and
- sitemap inclusion for the localized canonical URL.

Genuinely localized pages must not canonicalize back to English. Until that
architecture exists, adding a client translation or a supported locale code
does not authorize publishing a new indexable alternate.
...
**Analysis of Competing Hypotheses (ACH)** — composite intelligence products reduce single-source bias by corroborating independent inputs before they move a score: the Country Instability Index blends unrest, conflict, security, and information-velocity signals; protest data merges ACLED and GDELT; disaster data fuses USGS, GDACS, and NASA EONET. The breaking-news banner is deliberately different: the current code wires two independent producers, `rss_alert` and `oref_siren`, without a corroboration-count or cross-origin convergence requirement. A qualifying RSS item may raise a high or critical alert after recency, source-tier, story-phase, importance, deduplication, cooldown, startup, and user-sensitivity gates. A non-empty OREF siren batch escalates directly to critical when alerts are enabled and the event is not a duplicate; it bypasses the RSS-only gates and global cooldown.
...
**Alert-channel separation** — keyword spikes, hotspot escalation, and military surges contribute to correlation and anomaly products, but they are not wired as breaking-news banner producers. The banner presents qualifying RSS items and OREF sirens as separate alerts; it does not imply that those channels were fused into a corroborated assessment.
...
Meridian is written in vanilla TypeScript — no frontend framework (React, Vue, Svelte, Angular) is used. This is a deliberate architectural decision, not an oversight.
...
**35 service domains** cover every data vertical:
...
| `seed-natural-events` | NHC + GDACS + NASA EONET + HKO warnings | 3 hours | `natural:events:v1` |
...
- **Corporate Intelligence** — `IntelligenceService.GetCompanyEnrichment` resolves a company through the SEC's ticker→CIK registry (seeded daily from `company_tickers.json`) and aggregates EDGAR identity plus recent filings (10-K, 10-Q, 8-K with item codes), Finnhub market profile and earnings surprises, and recent news mentions into one envelope. `IntelligenceService.ListCompanySignals` classifies timestamped signals from SEC 8-K material events (tier 1) and news mentions (tier 3); Finnhub fiscal period ends stay in enrichment rather than being mislabeled as report timestamps. `SearchSecFilings` proxies EDGAR full-text search; `ListMaterialEvents` serves a seeded market-wide stream of material 8-K filings. No guessed attribution: companies that do not resolve in the SEC registry return empty envelopes (see issues #3754/#3755 for the heuristic predecessor this replaced)
...
All six dashboard variants (Meridian, Tech Monitor, Finance Monitor, Commodity Monitor, Happy Monitor, Energy Monitor) serve from a **single Vercel deployment**. The variant is determined at runtime by hostname detection:
...
| `meridian.app` (default) | `full` |

On the desktop app, the variant is stored in `localStorage['meridian-variant']` and can be switched without rebuilding. The variant selector in the header bar navigates between deployed domains on the web or toggles the localStorage value on desktop.
...
Sentry captures unhandled exceptions and promise rejections in production, with environment-aware routing (production on `meridian.app`, preview on `*.vercel.app`, disabled on localhost and Tauri desktop).
...
Transactions are sampled at 10% to balance observability with cost. Release tracking (`meridian@{version}`) enables regression detection across deployments.
