...
function makeContext(headers: Record<string, string> = {}) {
  return {
    request: new Request("https://www.meridian.app/api/news/v1/summarize-article", { headers }),
    pathParams: {},
    headers,
  };
}
...
describe("summarizeArticle handler premium mode gate", () => {
  ...
  test("anonymous analysis mode is rejected before provider fetch", async () => {
    const result = await summarizeArticle(makeContext({ "X-Meridian-Key": "wms_basic_session" }), request("analysis"));
  ...
  test("premium callers pass the summary gate", async () => {
    ...
    const result = await summarizeArticle(
      makeContext({ "X-Meridian-Key": "enterprise-test-key" }),
      request("brief"),
    );
