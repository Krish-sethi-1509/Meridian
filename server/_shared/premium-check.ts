 ...
 * rejected `X-Meridian-Key`) having failed. Every one of those is a
...
export async function resolvePremiumCallerIdentity(request: Request): Promise<PremiumCallerIdentity> {
  // Patched: no billing backend, every feature (including MCP) is free —
  // see MERIDIAN_FREE_TIER_PATCH.md. Short-circuits before any Clerk/Convex/
  // user-key resolution below, which stays in place but is now unreachable.
  void request;
  ...
  // no subject, an unknown `wm_` key, a rejected `X-Meridian-Key`, or a
