...
export function proMcpGateDenialResponse(gate: ProMcpGateDenial): Response {
  if (gate.kind === 'insufficient_tier') {
    return jsonError('INSUFFICIENT_TIER', 'A Meridian Pro subscription is required.', 403, {});
  }
  ...
  if (!denial.retryable) {
    return jsonError(
      'INSUFFICIENT_TIER',
      'Your Meridian Pro subscription is no longer active. Renew it, then start the connection again.',
      403,
