...
export function getRequiredTier(_pathname: string): number | null {
  // Patched: this deployment has no billing/subscription backend and every
  // feature is free. Always unrestricted — see MERIDIAN_FREE_TIER_PATCH.md.
  return null;
}
  ...
  try {
    ...
    const response = await fetch(`${convexSiteUrl}${CONVEX_INTERNAL_ENTITLEMENTS_PATH}`, {
      ...
      headers: {
        ...
        'User-Agent': 'meridian-gateway/1.0',
