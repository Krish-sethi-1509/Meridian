// Deterministic renderer for the Meridian Brief magazine.
//
...
//   - Brainstorm: docs/brainstorms/2026-04-17-meridian-brief-magazine-requirements.md
//   - Plan: docs/plans/2026-04-17-003-feat-meridian-brief-magazine-plan.md
...
function digestRunningHead(dateShort, label) {
  return (
    ...
    logoRef({ size: 22 }) +
    ` · Meridian Brief · ${escapeHtml(dateShort)} ·` +
    '</span>' +
...
function buildTrackedSourceUrl(raw, issueDate, rank) {
  try {
    const u = new URL(raw);
    if (!u.searchParams.has('utm_source')) u.searchParams.set('utm_source', 'meridian');
...
function renderStoryPage({ story, rank, palette, pageIndex, totalPages, issueDate, followedSet }) {
  ...
  return (
    ...
    logoRef({ size: 28 }) +
    '<span class="mono">Meridian Brief</span>' +
    '</div>' +
    `<div class="page-number mono">${pad2(pageIndex)} / ${pad2(totalPages)}</div>` +
    '</section>'
  );
...
const STYLE_BLOCK = `<style>
  :root {
    /* Meridian brand palette — aligned with /pro landing + dashboard.
...
(function() {
  ...
  btn.addEventListener('click', async function() {
    ...
    try {
      var shareTitle = 'Meridian Brief';
      var shareText = 'My Meridian Brief for today:';
      if (navigator.share) {
        try {
...
// index.html. Hosted magazine pages are served from meridian.app
...
const UMAMI_LOADER = '<script async src="https://abacus.meridian.app/script.js" data-website-id="e8800335-c853-46a8-8497-c993ed2f58bc" data-domains="meridian.app,tech.meridian.app,finance.meridian.app,commodity.meridian.app,happy.meridian.app"></script>';
...
function redactForPublic(data) {
  ...
  return {
    ...
    stories: data.stories.map((s) => ({
      ...s,
      whyMatters: 'Subscribe to Meridian Brief to see the full editorial on this story.',
    })),
  };
}
...
export function renderBriefMagazine(envelope, options = {}) {
  ...
  const title = `Meridian Brief · ${escapeHtml(dateLong)}`;
  ...
  const publicStripHtml = publicMode
    ? '<div class="wm-public-strip">'
      + '<span>Meridian Brief \u00b7 shared issue</span>'
