...
function attachRequiredBboxDiagnosticHeaders(
  ...
  headers.set('X-Meridian-Bbox', diagnostic.status);
  if (diagnostic.missing.length > 0) headers.set('X-Meridian-Bbox-Missing', diagnostic.missing.join(','));
  if (diagnostic.invalid.length > 0) headers.set('X-Meridian-Bbox-Invalid', diagnostic.invalid.join(','));
...
async function isResilienceRankingSeedRefreshRequest(request: Request, pathname: string): Promise<boolean> {
  ...
  const candidate = request.headers.get('X-Meridian-Key') ?? '';
...
// relay secret in X-Meridian-Key (timing-safe compared). Returns false when
...
export function createDomainGateway(
  ...
  return async function handler(originalRequest: Request, ctx?: GatewayCtx): Promise<Response> {
    ...
    // verified Pro tool fetch never needs an `X-Meridian-Key`. If
    ...
    // Patched: no billing backend, every feature is free — see
    // MERIDIAN_FREE_TIER_PATCH.md. Always false, matching getRequiredTier().
    ...
    if (!internalMcpVerified) {
      ...
      if (keyCheck.valid && (isUserApiKey || isEnterpriseAuth)) {
        ...
        if (perMinute > 0 && identity) {
          ...
          const upgradeUrl =
            planKey && planKey !== 'enterprise' ? 'https://meridian.app/' : undefined;
