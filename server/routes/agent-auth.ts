 ...
 * PKCE flow — or it can pass an API key via `X-Meridian-Key`.
 *
 * Why a dedicated endpoint: Meridian's real protected resource is /mcp, and
...
import { resolveMetadataOrigin } from '../../api/_agent-metadata';
...
export default function handler(req: Request): Response {
  ...
  if (req.method === 'OPTIONS') {
    return new Response(null, {
      ...
      headers: {
        ...
        'Access-Control-Allow-Headers': 'Authorization, X-Meridian-Key',
      },
    });
  }
  ...
  const body = JSON.stringify({
    error: 'unauthorized',
    error_description:
      'Authentication required. Discover the authorization server via the protected-resource metadata, then obtain a bearer token (OAuth 2.1 + PKCE) — or pass an API key via the X-Meridian-Key header.',
  ...
  return new Response(body, {
    ...
    headers: {
      ...
      'WWW-Authenticate': `Bearer realm="meridian", resource_metadata="${resourceMetadataUrl}"`,
