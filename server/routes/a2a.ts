...
export { suggestTools, type ToolSuggestion } from '../../api/_agent-tool-suggest';
...
const HOW_TO_CALL = {
  mcp: {
    endpoint: 'https://meridian.app/mcp',
    transport: 'streamable-http',
    note: "Issue tools/list for the live inventory (anonymous). Data calls need OAuth2 (scope=mcp) or an API key header 'X-Meridian-Key: wm_<40-hex>' — issue one at https://meridian.app/pro.",
  },
  rest: {
    base: 'https://api.worldmonitor.app',
    openapi: 'https://meridian.app/openapi.json',
  },
  docs: 'https://www.meridian.app/docs/documentation',
  agentGuidance: 'https://meridian.app/llms.txt',
} as const;
...
async function handleMessageSend(id: JsonRpcId, params: unknown): Promise<Response> {
  ...
  if (wantsFreshness || suggestions.length === 0) {
    const freshnessResource = PUBLIC_RESOURCE_REGISTRY.find(
      (r) => r.uri === 'meridian://seed-meta/freshness',
    );
    try {
  ...
  if (suggestions.length > 0) {
    lines.push(
      `Best-fit Meridian tools for "${echoedQuery}": ${suggestions.map((s) => s.name).join(', ')}.`,
    ...
    lines.push(
      'No specific tool matched that request. Meridian covers conflicts, sanctions, country risk, markets, commodities, energy, maritime/aviation activity, chokepoints, cyber threats, natural disasters, forecasts and prediction markets.',
