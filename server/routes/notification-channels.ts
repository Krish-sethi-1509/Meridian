...
// @ts-expect-error — JS module, no declaration file
import { captureEdgeException, captureSilentError } from '../../api/_sentry-edge.js';
import {
  beginStandaloneIdempotency,
  completeStandaloneIdempotency,
  getIdempotencyKey,
} from '../../api/_idempotency.js';
import { assertNotificationWebhookRegistrationUrlSafe } from '../../api/_notification-webhook-ssrf';
...
async function publishFlushHeld(userId: string, variant: string): Promise<void> {
  ...
  try {
    await notificationChannelsDeps.fetch(`${UPSTASH_URL}/lpush/wm:events:queue/${encodeURIComponent(msg)}`, {
      method: 'POST',
      headers: { Authorization: `Bearer ${UPSTASH_TOKEN}`, 'User-Agent': 'meridian-edge/1.0' },
      signal: AbortSignal.timeout(5000),
    });
  } catch (err) {
