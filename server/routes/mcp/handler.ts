...
import { getPublicCorsHeaders } from '../../../api/_cors.js';
import {
...
function hasCredentials(req: Request): boolean {
  ...
  return (req.headers.get('X-Meridian-Key') ?? '') !== '';
}
...
const STATIC_ASSET_USER_AGENT = 'Meridian-MCP/1.0 (+https://meridian.app)';
...
async function serveMcpGuide(req: Request, corsHeaders: Record<string, string>, headOnly = false): Promise<Response> {
  ...
  return new Response(headOnly ? null : mcpGuideCache, {
    ...
    headers: {
      ...
      ...corsHeaders,
      Vary: DISCOVERY_VARY,
      Link: '<https://meridian.app/mcp>; rel="canonical"',
    },
  });
}
...
async function mcpHandlerInner(
  ...
  // DATA reads (a `meridian://…` template instantiation) stay fully gated +
  ...
  switch (method) {
    ...
    case 'resources/list':
      // Concrete DATA resources (meridian://…, the metadata-only PUBLIC
