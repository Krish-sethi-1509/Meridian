...
import { emitWmSessionUsage } from '../../api/_usage-telemetry.js';
...
function shouldUseSharedCookieDomain(req) {
  ...
  return host === 'meridian.app' || host.endsWith('.meridian.app');
}

function cookieDomainAttribute(req) {
  return shouldUseSharedCookieDomain(req) ? '; Domain=.meridian.app' : '';
}
...
function clearReadableCookie(name) {
  return `${name}=; Domain=.meridian.app; Path=/; Max-Age=0; Secure; SameSite=Lax`;
}
...
export default async function handler(req, ctx) {
  ...
  // cookie can use the existing X-Meridian-Key validation path. This does
