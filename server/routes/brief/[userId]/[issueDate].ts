...
import { renderBriefMagazine } from '../../../_shared/brief-render.js';
import { readRawJsonFromUpstash, redisPipeline } from '../../../../api/_upstash-json.js';
import { verifyBriefToken, BriefUrlError } from '../../../_shared/brief-url';
import {
  BRIEF_PUBLIC_POINTER_PREFIX,
  buildPublicBriefUrl,
  encodePublicPointer,
} from '../../../_shared/brief-share-url';
...
function renderErrorPage(heading: string, body: string): string {
  return (
    ...
    + `<title>${heading} · Meridian</title>`
    + `<style>${ERROR_PAGE_STYLES}</style>`
    + '</head><body><div>'
    + `<h1>${heading}</h1>`
    + `<p>${body}</p>`
    + '<p><a href="https://meridian.app">Return to Meridian</a></p>'
    + '</div></body></html>'
  );
}
...
const FORBIDDEN_PAGE = renderErrorPage(
  ...
  "The brief link you followed is incomplete or has been tampered with. Open the most recent notification from Meridian to read today's brief.",
);
  ...
  try {
    const res = await fetch(`${convexSiteUrl}/relay/followed-countries`, {
      ...
      headers: {
        ...
        'User-Agent': 'meridian-brief-render/1.0',
      },
