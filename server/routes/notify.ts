...
import { checkTierProEntitlement } from '../_shared/pro-entitlement';
...
export default async function handler(req: Request): Promise<Response> {
  ...
  if (!proAccess.allowed) {
    ...
    return jsonResponse({ error: 'pro_required', message: 'Event publishing is available on the Pro plan.', upgradeUrl: 'https://meridian.app/pro' }, 403, cors);
  }
  ...
  const res = await fetch(
    ...
    { method: 'POST', headers: { Authorization: `Bearer ${upstashToken}`, 'User-Agent': 'meridian-edge/1.0' } },
  );
