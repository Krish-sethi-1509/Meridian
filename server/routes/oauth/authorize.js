...
import { getClientIp } from '../../../api/_rate-limit.js';
// @ts-expect-error — JS module, no declaration file
import { timingSafeIncludes, sha256Hex } from '../../../api/_crypto.js';
...
// First-party origin allowlist for the consent POST: the meridian.app apex
...
// meridian.app.evil.example, evilmeridian.app, and any :port.
const WM_ORIGIN = /^https:\/\/(?:[a-z0-9-]+\.)?meridian\.app$/;
...
function htmlError(title, detail) {
  return new Response(`<!DOCTYPE html><html lang="en"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><title>Error &#x2014; Meridian MCP</title>
...
<body><a href="https://www.meridian.app" class="wm-logo" target="_blank" rel="noopener">${GLOBE_SVG}<span class="wm-logo-text">Meridian MCP</span></a>
<div class="card"><h1>${escapeHtml(title)}</h1><p>${escapeHtml(detail)}</p><a href="javascript:history.back()" class="back">&#8592; go back</a></div>
<p class="footer"><a href="https://www.meridian.app" target="_blank" rel="noopener">meridian.app</a></p>
...
export function consentPage(params, nonce, errorMsg = '') {
  ...
  const proCtaHref = `https://meridian.app/mcp-grant?nonce=${encodeURIComponent(nonce)}`;
...
<body>
<a href="https://www.meridian.app" class="wm-logo" target="_blank" rel="noopener">${GLOBE_SVG}<span class="wm-logo-text">Meridian MCP</span></a>
<div class="card">
<div class="client-hd">
...
<hr>
<a id="pc" class="pro-cta" href="${escapeHtml(proCtaHref)}">Sign in with Meridian Pro</a>
...
<p class="hint">No key? <a href="https://www.meridian.app/pro" target="_blank" rel="noopener">Get one at meridian.app/pro &#x2192;</a></p>
<p class="error" id="ke"${errorMsg ? '' : ' style="display:none"'}>${errorMsg ? escapeHtml(errorMsg) : ''}</p>
<button type="submit" id="ab">Authorize</button>
</form>
</div>
<p class="footer"><a href="https://www.meridian.app" target="_blank" rel="noopener">meridian.app</a> &middot; <a href="https://www.meridian.app/pro" target="_blank" rel="noopener">Get an API key &#x2192;</a></p>
...
export default async function handler(req) {
  ...
  if (method === 'POST') {
    // Origin validation: allow any first-party meridian.app host (the consent
