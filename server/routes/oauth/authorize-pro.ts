...
import { verifyGrant, GrantConfigError } from '../../../api/_mcp-grant-hmac';
import {
  getEntitlements,
  isEntitlementBackendConfigured,
  type BillingVerificationDenial,
} from '../../_shared/entitlement-check';
import { checkProMcpAccess, type ProMcpEntitlement } from '../../_shared/pro-mcp-gate';
import {
  issueProMcpTokenForUser,
  revokeProMcpToken,
  ProMcpIssueFailed,
} from '../../_shared/pro-mcp-token';
...
function htmlError(
  ...
  return new Response(
    `<!DOCTYPE html><html lang="en"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><title>Error &#x2014; Meridian MCP</title>
...
function billingVerificationPage(
  ...
  if (!denial.retryable) {
    return htmlError(
      'Subscription Lapsed',
      'Your Meridian Pro subscription is no longer active, so this connection '
      + 'cannot be authorized. Renew your subscription at meridian.app, then '
  ...
  return htmlError(
    ...
    `We could not confirm your Meridian Pro subscription just now — this is `
...
export async function authorizeProHandler(req: Request, deps: AuthorizeProDeps): Promise<Response> {
  ...
  if (gate) {
    ...
    return htmlError(
      ...
      'A Meridian Pro subscription is required for this connection. Please subscribe and try again.',
      403,
    );
  }
