...
// — natural-language queries over Meridian's public agent surface.
//
...
const SITE = 'meridian.app';
const TOOLS_DOC_URL = 'https://www.meridian.app/docs/mcp-tools-reference';
const MCP_ENDPOINT = 'https://meridian.app/mcp';
...
export function buildResults(query: string): NlwebResult[] {
  ...
  const results: NlwebResult[] = suggestions.map((s) => ({
    ...
    schema_object: {
      ...
      url: MCP_ENDPOINT,
      provider: { '@type': 'Organization', name: 'Meridian', url: 'https://www.meridian.app' },
    },
  }));
  if (results.length === 0) {
