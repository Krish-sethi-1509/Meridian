 ...
 *   2. Browser tester key (X-Meridian-Key) — validated against
...
// @ts-expect-error — JS module, no declaration file
import { timingSafeEqualSecret, timingSafeIncludes } from '../../api/_crypto.js';
import { validateBearerToken } from '../auth-session';
import { getBillingVerificationDenial, getEntitlements } from '../_shared/entitlement-check';

const RELAY_BASE = 'https://proxy.meridian.app';
...
async function hasValidMeridianKey(key: string): Promise<boolean> {
...
export default async function handler(req: Request): Promise<Response> {
  ...
  if (req.method === 'OPTIONS') {
    return new Response(null, {
      ...
      headers: {
        ...
        'Access-Control-Allow-Headers': 'Content-Type, Authorization, X-Meridian-Key, X-Api-Key, X-Widget-Key, X-Pro-Key',
      },
    });
  }
  ...
  const headerMeridianKey =
    req.headers.get('X-Meridian-Key') ??
  ...
  if (await hasValidMeridianKey(worldMonitorKey)) {
    isPro = true;
  } else {
  ...
  const relayHeaders: Record<string, string> = {
    ...
    'User-Agent': 'meridian-widget-edge/1.0',
