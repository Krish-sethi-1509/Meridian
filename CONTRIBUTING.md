...
Thank you for your interest in contributing to Meridian! This project thrives on community contributions — whether it's code, data sources, documentation, or bug reports.
...
Meridian is a real-time OSINT dashboard built with **Vanilla TypeScript** (no UI framework), **MapLibre GL + deck.gl** for map rendering, and a custom Proto-first RPC framework called **Sebuf** for all API communication.
...
| **Protobuf / Buf** | Service and message definitions across 36 domains |
...
See the [API dependencies docs](https://www.meridian.app/docs/getting-started#api-dependencies) for the full list.
...
- **Internationalization** — help make Meridian available in more languages
...
- Sebuf handler implementations go in `server/meridian/{domain}/v1/`
...
3. **Handlers** in `server/meridian/{domain}/v1/handler.ts` implement the service interface
...
| `docs/api/meridian.openapi.yaml` | **Unified bundle** spanning every service (sebuf ≥ v0.11.0) — use this for external consumers, API explorers, or anywhere you want a single spec covering all RPCs |
...
9. **Document the source** — add it to the [data sources docs](https://www.meridian.app/docs/data-sources)
...
rclone copy public/data/country-boundary-overrides.geojson r2:meridian-maps/
```
...
Thank you for helping make Meridian better! 🌍
