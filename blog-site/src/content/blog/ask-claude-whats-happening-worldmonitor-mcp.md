...
description: "Connect Claude to Meridian's MCP server so non-developers can ask live questions about conflicts, markets, chokepoints, country risk, and breaking news."
metaTitle: "Ask Claude About Current Events with Meridian MCP"
keywords: "Claude connectors real-time news, ask AI about current events, Claude MCP server current events, Meridian MCP Claude, AI current events assistant"
audience: "Claude users, analysts, founders, journalists, operators, non-developer power users"
heroImage: "/blog/images/blog/meridian-mcp-server-ai-agents-real-time-intelligence.jpg"
pubDate: "2026-06-10"
...
Meridian's [MCP server](https://www.meridian.app/docs/mcp-overview) gives Claude a live tool connection. You add one connector URL, approve the OAuth flow, and Claude can call the same intelligence surfaces behind the [Meridian dashboard](/blog/posts/what-is-meridian-real-time-global-intelligence/): country risk, conflicts, markets, maritime chokepoints, aviation, cyber, energy, news intelligence, forecasts, and more.

You do not need to write an app. You do not need to understand JSON-RPC. You only need a paid Meridian tier and a Claude client that supports custom connectors.
...
Meridian's MCP server exists for this exact pattern. It exposes 59 tools, six prompt templates, and read-only resources through the [Model Context Protocol](https://modelcontextprotocol.io), so Claude can discover tools and call them during a conversation.
...
```text
https://meridian.app/mcp
```

Use the apex host exactly as shown. The dashboard and docs commonly live at `www.meridian.app`, but the MCP connector endpoint is intentionally published at `meridian.app/mcp`.

For Claude web, open Settings, go to Connectors, add a custom connector named `Meridian`, and paste that URL.
...
```json
{
  "mcpServers": {
    "meridian": {
      "url": "https://meridian.app/mcp"
    }
  }
}
```

Restart Claude Desktop after saving the file. On first use, Claude opens the Meridian OAuth consent flow. Pro users sign in with their Meridian account. API Starter, Business, and Enterprise users can use OAuth or an issued `wm_...` API key for server-side workflows.

Free accounts can still use the public dashboard, but the MCP server requires a paid tier. The [MCP quickstart](https://www.meridian.app/docs/mcp-quickstart) keeps the setup details current if a client changes its connector UI.

## Six prompts that work immediately

You can ask plain-English questions, but the fastest way to get reliable answers is to start from repeatable prompt shapes. These are the six templates Meridian exposes through MCP:
...
```text
Use Meridian before answering. Give me a 10-minute global brief for today.
...
If Claude answers without calling tools, ask it to retry with Meridian tools first.
...
```text
Use Meridian to brief Turkey for an operations team. Include country risk,
...
```text
Use Meridian to check route risk from the Gulf to Northwest Europe. Look at
...
```text
Use Meridian to evaluate this claim: "<paste claim>". Return an evidence
...
```text
Use Meridian for a market-open geopolitical prep. Include oil, gold, major
...
```text
Run a Meridian freshness audit for the tools you just used. Which fields are
...
- Do not assume Meridian covers every source on earth.
...
**Can non-developers use Meridian MCP with Claude?**

Yes. Claude web users can add Meridian as a custom connector, and Claude Desktop users can add the server URL to the desktop config. The setup is mostly copy-paste; no application code is required.

**Does this give Claude real-time current events?**

It gives Claude access to Meridian's live and cache-backed intelligence tools. Claude still has to call the tools, preserve freshness fields, and explain uncertainty. Ask it to use Meridian before answering live-risk questions.

**Is MCP free?**

The public dashboard is free, but Meridian MCP access requires Pro or an API tier. The MCP docs list the current auth modes, quotas, and client setup details.

---

**Add `https://meridian.app/mcp` as a Claude connector, then ask one narrow operational question: what changed in the world that matters to me today?**
