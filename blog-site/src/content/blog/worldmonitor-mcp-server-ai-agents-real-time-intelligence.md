---
title: "Give Your AI Agent Eyes on the World: The Meridian MCP Server"
...
metaTitle: "Real-Time Intelligence MCP Server | Meridian"
...
The Model Context Protocol (MCP) fixes the plumbing problem: it gives AI assistants a standard way to call live tools. Meridian fixes the data problem: it exposes the entire intelligence platform, the same one behind the [free real-time dashboard](/blog/posts/what-is-meridian-real-time-global-intelligence/), as an MCP server with **59 live tools**.
...
It speaks streamable HTTP (JSON-RPC 2.0), handles OAuth automatically on first connection, and serves 59 tools across six domains. The flagship ones:
...
{
  "mcpServers": {
    "meridian": { "url": "https://meridian.app/mcp" }
  }
}
```

**Claude web (claude.ai):** Settings → Connectors → Add custom connector → name it Meridian, paste `https://meridian.app/mcp`.
...
```bash
npx @modelcontextprotocol/inspector https://meridian.app/mcp
```

On first use, the client walks you through "Sign in with Meridian Pro." MCP access requires a [Pro account](https://www.meridian.app/pro); API Starter and Enterprise keys (`X-Meridian-Key`) also work for server-to-server agents. OAuth clients do not need copied keys because the protocol handles token exchange and refresh.
...
Every Meridian tool accepts an optional `jmespath` argument. The server applies the expression server-side and returns only the projection:
...
Meridian treats autonomous agents as first-class clients. From the root URL, an agent can discover everything by itself:
...
So an agent that has never heard of Meridian can start from `https://meridian.app/`, read the Link headers, and wire itself up without a human in the loop. If you prefer raw REST over MCP, the same data is available through the [developer API](/blog/posts/build-on-meridian-developer-api-open-source/), and the two share authentication.
...
Meridian follows the official [Model Context Protocol specification](https://modelcontextprotocol.io/specification/latest). Client support and transport behavior can vary, so verify the client's current MCP documentation when configuring production access.
...
**Does the Meridian MCP server work with ChatGPT or other non-Claude clients?**
...
**Add `https://meridian.app/mcp` to your MCP client and ask your agent what changed in the world today. Setup docs at [meridian.app/docs/mcp-quickstart](https://www.meridian.app/docs/mcp-quickstart).**
