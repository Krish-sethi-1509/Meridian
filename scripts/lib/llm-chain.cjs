...
const SERVICE_UA = 'meridian-llm/1.0';
...
const LLM_PROVIDERS = [
  ...
  },
  {
    ...
    headers: (key) => ({ 'Authorization': `Bearer ${key}`, 'Content-Type': 'application/json', 'HTTP-Referer': 'https://meridian.app', 'X-Title': 'Meridian', 'User-Agent': SERVICE_UA }),
    timeout: 20_000,
  },
];
