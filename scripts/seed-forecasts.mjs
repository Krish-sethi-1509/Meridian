...
async function warmPingChokepoints() {
  ...
  // warm-ping ONLY when it carries WORLDMONITOR_RELAY_KEY in X-Meridian-Key
  ...
  const headers = { 'User-Agent': CHROME_UA, Origin: 'https://meridian.app' };
  if (relayKey) headers['X-Meridian-Key'] = relayKey;
  try {
          ...
          try {
            const response = await forecastFetch(provider.apiUrl, {
              ...
              headers: {
                ...
                ...(provider.name === 'openrouter' ? { 'HTTP-Referer': 'https://meridian.app', 'X-Title': 'Meridian' } : {}),
              },
              body: JSON.stringify({
