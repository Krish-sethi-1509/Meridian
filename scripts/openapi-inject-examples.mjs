...
function stringExample(name, schema = {}, context = {}) {
  ...
  if (key.includes('callbackurl')) return constrainedString('https://example.com/meridian-webhook', schema);
  if (key.includes('url') || key.includes('link')) return constrainedString('https://example.com/meridian', schema);
  ...
  if (key.includes('fullname')) return constrainedString('koala73/meridian', schema);
  if (key.includes('provider')) return constrainedString('meridian', schema);
  ...
  if (key.includes('name')) return constrainedString('Meridian Analyst', schema);
  if (key.includes('message') || key.includes('summary') || key.includes('description')) {
    return constrainedString('Example Meridian observation.', schema);
  }
