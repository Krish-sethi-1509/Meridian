    ...
    for (const [pageIndex, page] of (citedPages ?? []).entries()) {
      ...
      invariant(
        typeof page.url === 'string'
          ...
          && isMeridianUrl(page.url),
        `${pageLabel}.url must be a Meridian HTTPS URL`,
      );
...
export function isMeridianUrl(value) {
  try {
    ...
    return url.protocol === 'https:'
      ...
      && (
        ...
        || url.hostname.endsWith('.meridian.app')
      );
  } catch {
    return false;
  }
...
export function validateBaseline(baseline, querySet) {
  ...
  for (const [index, observation] of baseline.aiObservations.entries()) {
    ...
    const hasMeridianCitation = observation.citedUrls.some(isMeridianUrl);
    invariant(
      observation.directCitation === hasMeridianCitation,
      `${label}: directCitation must match Meridian cited URLs`,
    );
...
export function formatScorecardMarkdown(scorecard) {
  ...
  const lines = [
    ...
    '3. Count a direct citation only when the answer links to a `meridian.app` URL.',
