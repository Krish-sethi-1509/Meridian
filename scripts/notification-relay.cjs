...
const RESEND_FROM = process.env.RESEND_FROM_EMAIL ?? 'Meridian <alerts@meridian.app>';
...
async function fetchEnabledRules(enabled = true) {
  const res = await fetch(`${CONVEX_SITE_URL}/relay/enabled-rules?enabled=${enabled}`, {
    headers: { Authorization: `Bearer ${RELAY_SECRET}`, 'User-Agent': 'meridian-relay/1.0' },
...
async function upstashRest(...args) {
  const res = await fetch(`${UPSTASH_URL}/${args.map(encodeURIComponent).join('/')}`, {
    method: 'POST',
    headers: { Authorization: `Bearer ${UPSTASH_TOKEN}`, 'User-Agent': 'meridian-relay/1.0' },
  });
  if (!res.ok) {
...
async function isUserPro(userId) {
  ...
  try {
    const res = await fetch(`${CONVEX_SITE_URL}/relay/entitlement`, {
      method: 'POST',
      headers: { 'Content-Type': 'application/json', Authorization: `Bearer ${RELAY_SECRET}`, 'User-Agent': 'meridian-relay/1.0' },
...
async function drainHeldForUser(userId, variant, allowedChannelTypes) {
  ...
  const lines = [`Meridian — ${events.length} held alert${events.length !== 1 ? 's' : ''} from quiet hours`, ''];
  for (const ev of events) {
    lines.push(`[${(ev.severity ?? 'high').toUpperCase()}] ${ev.payload?.title ?? ev.eventType}`);
  }
  lines.push('', 'View full dashboard → meridian.app');
  const text = lines.join('\n');
  const subject = `Meridian — ${events.length} held alert${events.length !== 1 ? 's' : ''}`;
    ...
    try {
      ...
      if (ch.channelType === 'telegram' && ch.chatId) ok = await sendTelegram(userId, ch.chatId, text);
        ...
        ok = await sendWebPush(userId, ch, {
          title: `Meridian · ${events.length} held alert${events.length === 1 ? '' : 's'}`,
          body: subject,
...
async function sendTelegram(userId, chatId, text, _retryCount = 0) {
  ...
  const res = await fetch(`https://api.telegram.org/bot${TELEGRAM_BOT_TOKEN}/sendMessage`, {
    method: 'POST',
    headers: { 'Content-Type': 'application/json', 'User-Agent': 'meridian-relay/1.0' },
...
async function sendSlack(userId, webhookEnvelope, text) {
  ...
  try {
    res = await postJsonWithPinnedAddress(
      ...
      { 'Content-Type': 'application/json', 'User-Agent': 'meridian-relay/1.0' },
      resolvedAddresses,
    );
  } catch (err) {
...
async function sendWebPush(userId, subscription, payload) {
  ...
  const body = JSON.stringify({
    ...
    tag: payload.tag || 'meridian-generic',
...
async function processWelcome(event) {
  ...
  const text = `✅ Meridian connected! You'll receive breaking news alerts here.`;
  if (channelType === 'slack' && ch.webhookEnvelope) {
    ...
    await sendEmail(ch.email, 'Meridian Notifications Connected', text);
...
async function processEvent(event) {
  ...
  const subject = `Meridian Alert: ${event.payload?.title ?? event.eventType}`;
      ...
      try {
        if (ch.channelType === 'telegram' && ch.chatId) {
          ...
          await sendWebPush(rule.userId, ch, {
            title: event.payload?.title || event.eventType || 'Meridian',
            body: firstLine,
            url: eventUrl,
