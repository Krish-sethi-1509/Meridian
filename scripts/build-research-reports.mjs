...
const UMAMI_SCRIPT_TAG =
  '<script async defer src="https://abacus.meridian.app/script.js" '
  + 'data-website-id="e8800335-16bc-4241-a133-0eb28c07c832" '
  + 'data-domains="meridian.app,www.meridian.app,happy.meridian.app" '
  ...
  return `${JSON.stringify(
    {
      ...
      canonicalUrl,
      license:
        'Report text and derived figures: © Meridian, citation welcome with attribution. Underlying transit data: IMF PortWatch (portwatch.imf.org); consult upstream terms.',
...
${provenanceRows}
      ...
      case 'live-handoff': {
        ...
        parts.push(`        <p>This report is a dated snapshot. For the current picture: the ${trackedLink(`/chokepoints/${chokepointSlug}/`, 'live Strait of Hormuz status page', 'chokepoint-page', escapeHtml)} shows today's disruption pulse, and the ${trackedLink(dashboardUrl, 'Meridian dashboard', 'dashboard', escapeHtml)} adds map layers, alerts, and vessel context around it.</p>
        <p>Programmatic access: the same chokepoint status and transit history are available through the ${trackedLink('/docs/api-reference', 'Meridian REST API', 'developer', escapeHtml)} and the ${trackedLink('/docs/mcp-overview', 'MCP server', 'developer', escapeHtml)} for AI agents. Higher request limits and research briefings come with ${trackedLink(withUtmSource(absoluteUrl(baseUrl, '/pro'), 'research-report'), 'Meridian Pro', 'pricing', escapeHtml)}. The research itself stays free and ungated.</p>`);
        break;
      }
      default:
        break;
...
${justification}
  ...
  const jsonLd = {
    ...
    publisher: { '@type': 'Organization', name: 'Meridian', url: 'https://www.meridian.app/' },
  ...
  const html = pageDocument({
    baseUrl,
    path,
    title: `${report.metaTitle} | Meridian`,
    description,
    lastmod,
    jsonLd,
    ...
    ogImageAlt: `Chart of daily Strait of Hormuz transit calls collapsing from about ${formatMetric(m('febAvgTotal'))} per day in February 2026 to single digits in March, partially recovering to about ${formatMetric(m('julToDateAvgTotal'))} in July — ${report.title}, Meridian`,
  });
...
export function renderResearchIndex({ reports, tpl, baseUrl, lastmod }) {
  ...
  const path = '/research/';
  const description =
    'Original, source-backed Meridian research: dated, versioned reports built from committed data snapshots, with downloadable data, explicit methodology, and declared gaps.';
  const body = `      <p class="eyebrow">Research corpus</p>
      <h1>Meridian research reports</h1>
      <p class="lede">${escapeHtml(description)}</p>
      <div class="grid">
${reports.map((report) => `        <a class="card" href="/research/${escapeHtml(report.slug)}/"><strong>${escapeHtml(report.title)}</strong><br><span>Published ${escapeHtml(report.datePublished)} &middot; v${escapeHtml(report.version)}</span></a>`).join('\n')}
  ...
  const html = pageDocument({
    baseUrl,
    path,
    title: 'Research Reports | Meridian',
    description,
    lastmod,
    jsonLd: {
      '@context': 'https://schema.org',
      '@type': 'CollectionPage',
      name: 'Meridian research reports',
      description,
