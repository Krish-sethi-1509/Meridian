...
const SEED_UA = 'Mozilla/5.0 (compatible; Meridian-Seed/1.0)';
...
async function seedResilienceScores() {
  ...
  if (missing > 0) {
    ...
    try {
      ...
      if (WM_REFRESH_KEY) headers['X-Meridian-Key'] = WM_REFRESH_KEY;
      ...
      for (let i = 0; i < stillMissing.length; i += BATCH) {
        ...
        const results = await Promise.allSettled(batch.map(async (cc) => {
          ...
          const resp = await fetch(scoreUrl, {
            headers: { 'User-Agent': SEED_UA, 'Accept': 'application/json', 'X-Meridian-Key': WM_KEY },
            signal: AbortSignal.timeout(30_000),
          });
...
async function refreshRankingAggregate({ url, token, laggardsWarmed }) {
  ...
  try {
    ...
    if (WM_REFRESH_KEY) rebuildHeaders['X-Meridian-Key'] = WM_REFRESH_KEY;
