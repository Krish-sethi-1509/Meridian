...
// Cross-index benchmark: compares Meridian resilience scores against
...
export async function fetchInformGlobal() {
  ...
  try {
    ...
    const resp = await fetch(INFORM_JSON_URL, {
      headers: { 'User-Agent': 'Meridian-Benchmark/1.0', Accept: 'application/json' },
      signal: AbortSignal.timeout(30_000),
    });
    if (!resp.ok) throw new Error(`HTTP ${resp.status}`);
...
async function readWmScoresFromRedis() {
  ...
  const rankingResp = await fetch(`${url}/get/${encodeURIComponent('resilience:ranking:v25')}`, {
