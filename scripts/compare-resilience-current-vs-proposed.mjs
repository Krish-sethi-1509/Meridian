...
// from `server/meridian/resilience/v1/_pillar-membership.ts` (which
...
async function main() {
  const scorerMod = await import('../server/meridian/resilience/v1/_dimension-scorers.ts');
  const {
    scoreAllDimensions,
  ...
  } = await import('../server/meridian/resilience/v1/_shared.ts');

  const {
    buildPillarList,
    PILLAR_ORDER,
    PILLAR_WEIGHTS,
  } = await import('../server/meridian/resilience/v1/_pillar-membership.ts');

  const { INDICATOR_REGISTRY } = await import(
    '../server/meridian/resilience/v1/_indicator-registry.ts'
  );
