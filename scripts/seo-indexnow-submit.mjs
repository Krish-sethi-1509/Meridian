...
const USER_AGENT = 'Meridian-IndexNow/1.0 (+https://www.meridian.app)';
...
function getBlogPostUrls() {
  return readdirSync(BLOG_DIR)
    ...
    .map((file) => `https://www.meridian.app/blog/posts/${basename(file, '.md')}/`)
    .sort();
}
...
function getBlogAuthorUrls() {
  return readdirSync(BLOG_AUTHORS_DIR)
    ...
    .map((file) => `https://www.meridian.app/blog/authors/${basename(file, '.astro')}/`)
    .sort();
}
...
function getBlogGlossaryUrls() {
  ...
  return slugs.map((slug) => `https://www.meridian.app/blog/glossary/${slug}/`).sort();
}
...
const APEX_URLS = uniqueSorted(getSitemapUrlsForHost('meridian.app'));
const WWW_URLS = uniqueSorted([
  ...getSitemapUrlsForHost('www.meridian.app'),
  ...getBlogUrls(),
]);
...
export const INDEXNOW_BATCHES = Object.freeze([
  batch('meridian.app', APEX_URLS, APEX_INDEXNOW_KEY),
  batch('www.meridian.app', WWW_URLS),
  batch('tech.meridian.app', urlsForHost('tech.meridian.app', ['https://tech.meridian.app/'])),
  batch('finance.meridian.app', urlsForHost('finance.meridian.app', ['https://finance.meridian.app/'])),
  batch('happy.meridian.app', urlsForHost('happy.meridian.app', ['https://happy.meridian.app/'])),
]);
