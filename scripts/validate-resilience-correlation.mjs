...
async function fetchMeridianScores(url, token, countryCodes) {
...
async function run() {
  ...
  console.log(`Fetching Meridian resilience scores for ${SAMPLE_COUNTRIES.length} countries...`);
  const wmScores = await fetchMeridianScores(url, token, SAMPLE_COUNTRIES);
  ...
  console.log(`Meridian vs ND-GAIN Readiness:  rho = ${ndgainResult.rho.toFixed(3)} (n=${ndgainResult.n}, target > 0.65) ${ndgainPass ? 'PASS' : 'FAIL'}`);
  console.log(`Meridian vs INFORM Risk:        rho = ${informResult.rho.toFixed(3)} (n=${informResult.n}, target > 0.60, inverted) ${informPass ? 'PASS' : 'FAIL'}`);
  ...
  console.log('Meridian score ranking (sample):');
