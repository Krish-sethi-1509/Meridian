...
const mcpCountSurfaces = [
  ...
  'blog-site/src/content/blog/ask-claude-whats-happening-meridian-mcp.md',
  'blog-site/src/content/blog/build-geopolitical-risk-agent-meridian-mcp.md',
  ...
  'blog-site/src/content/blog/build-on-meridian-developer-api-open-source.md',
  'blog-site/src/content/blog/meridian-mcp-server-ai-agents-real-time-intelligence.md',
  'blog-site/src/content/blog/meridian-is-not-palantir.md',
  'docs/cli.mdx',
  ...
  return {
    ...
    plans: [
      ...
      },
      {
        name: 'Enterprise',
        price: 'Custom',
        contact: 'enterprise@meridian.app',
...
transform('public/pricing.md', (source) => {
  ...
  if (!next.includes(generatedNote)) {
    next = next.replace('# Pricing - Meridian\n', `# Pricing - Meridian\n\n${generatedNote}\n`);
  }
  return next.replace(
    /```json\n[\s\S]*?```/,
...
transform('docs/docs.json', (source) => {
  ...
  const visit = (value) => {
    ...
    if (value.href === 'https://www.meridian.app/pro#waitlist') {
