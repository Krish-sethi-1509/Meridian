...
const DEFAULT_ORIGIN = 'https://www.meridian.app';
...
export function classifySitemapUrl(value) {
  ...
  const { pathname } = url;
  if (url.hostname === 'meridian.app' && pathname === '/mcp') return 'mcp';
  if (pathname === '/blog' || pathname.startsWith('/blog/')) return 'blog';
  if (pathname === '/docs' || pathname.startsWith('/docs/')) return 'docs';
  if (url.hostname !== 'www.meridian.app' && pathname === '/dashboard') {
...
async function fetchDirect(url, { method = 'GET', fetchImpl = globalThis.fetch } = {}) {
  return fetchImpl(url, {
    ...
    headers: {
      ...
      'User-Agent': 'Meridian-Sitemap-Verifier/1.0 (+https://www.meridian.app)',
    },
...
function validatePageLocation(value) {
  ...
  if (
    ...
    return `sitemap page URL is not an allowed canonical Meridian URL: ${value}`;
  }
  if (url.hostname === 'meridian.app' && url.pathname !== '/mcp') {
    return `apex sitemap URL must be the canonical MCP endpoint: ${value}`;
  }
  if (
    url.hostname !== 'www.meridian.app'
    && url.hostname !== 'meridian.app'
