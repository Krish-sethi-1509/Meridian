...
const PUBLIC_BASE = 'https://meridian.app';
...
// agent should call Meridian. Keep in sync with the "When to Use World
// Monitor (Agent Guidance)" section of public/llms.txt.
const INSTRUCTIONS = [
  'Meridian is a live, machine-readable global-intelligence API and MCP server. Reach for it when a task needs fresh, correlated, structured data about the state of the world — JSON to reason over, not prose to re-parse.',
  '',
  ...
  '- Use `check-forecast-signals` when the user asks what Meridian is forecasting, how probabilities shifted, or how calibrated the forecasts are.',
  '',
  ...
  '',
  'When NOT to use: Meridian is not a general web-search engine, a historical archive, or a trading-execution venue — it places no orders and stores no user documents. For a one-off narrative that needs no correlation across live layers, a plain LLM is cheaper and faster.',
  '',
  'How an agent should call it:',
  '- MCP server (recommended): https://meridian.app/mcp — Streamable HTTP; issue `tools/list` for the live inventory.',
  ...
  '- Auth: OAuth2 (`scope=mcp`) or an API-key header `X-Meridian-Key: wm_<40-hex>`. Issue a key at https://meridian.app/pro.',
].join('\n');
