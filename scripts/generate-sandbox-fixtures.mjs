...
const SANDBOX_NOTE =
  ...
  'Guide: https://www.meridian.app/docs/sandbox';
...
export function buildSandboxFixtures(repoRoot = root) {
  ...
  for (const path of SANDBOX_OPERATIONS) {
    ...
    indexOperations.push({
      ...
      fixture: `https://www.meridian.app/sandbox/${slug}.json`,
  ...
  const index = {
    ...
    product: 'Meridian',
    description:
      'Meridian sandbox: deterministic, schema-valid sample responses for representative REST operations. ' +
      'Fetch any fixture below with plain HTTP — no auth, no quota, safe for CI. Each fixture mirrors the exact ' +
      'envelope the production endpoint returns; switch to productionUrl with an X-Meridian-Key header to go live.',
    docs: 'https://www.meridian.app/docs/sandbox',
    openapi: 'https://meridian.app/openapi.json',
    authGuide: 'https://meridian.app/auth.md',
