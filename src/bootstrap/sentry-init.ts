...
const THIRD_PARTY_FETCH_HOST_ALLOWLIST = new Set([
  ...
  // itself (`retryImmediately`); a `Failed to fetch (clerk.meridian.app)`
  ...
  // `abacus.meridian.app/script.js`, whose tracker POSTs events to
...
function buildSentryInitOptions(): Parameters<SentryNs['init']>[0] {
  ...
  return {
    ...
    environment: (location.hostname === 'meridian.app' || location.hostname.endsWith('.meridian.app')) ? 'production'
      : location.hostname.includes('vercel.app') ? 'preview'
      : 'development',
    enabled: Boolean(sentryDsn) && !location.hostname.startsWith('localhost') && !('__TAURI_INTERNALS__' in window),
    allowUrls: [
      /https?:\/\/(www\.|tech\.|finance\.|commodity\.|happy\.)?meridian\.app/,
      /https?:\/\/.*\.vercel\.app/,
    ],
    sendDefaultPii: true,
    ...
    ignoreErrors: [
      ...
      /\[clerk\] failed to load/, // Clerk SDK failed to load its own UI chunk from clerk.meridian.app — SDK-internal load failure, not our code (WORLDMONITOR-??: Yandex Browser 26.4).
      ...
      if (dynamicImportAssetUrlMatch) {
        ...
        if (assetUrl.startsWith('/')) {
          ...
          try {
            ...
            isOwnedDynamicImportAssetUrl = host === 'meridian.app'
              || host.endsWith('.meridian.app')
