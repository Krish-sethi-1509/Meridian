...
export function migrateDisabledFeedsV2(
  ...
  const raw = data['meridian-disabled-feeds'];
  ...
  return { ...data, 'meridian-disabled-feeds': JSON.stringify(cleaned) };
}
...
export function migrateRegionalFeedRolloutDefaultsV5(
  ...
  const raw = data['meridian-disabled-feeds'] as string;
  ...
  return { ...data, 'meridian-disabled-feeds': JSON.stringify(reconciled) };
}
...
export function migrateCanadaArcticOptInsV6(
  ...
  return { ...data, 'meridian-disabled-feeds': JSON.stringify(updated) };
}
