...
export function exportToJSON(data: ExportData, filename = 'meridian-export'): void {
...
export function exportToCSV(data: ExportData, filename = 'meridian-export'): void {
  const clean = sanitizeExportData(data);
  const lines: string[] = [];

  lines.push(`# Meridian Export — ${new Date(clean.timestamp).toISOString()}`);
...
export const COUNTRY_EVIDENCE_PROVENANCE_DISCLAIMER =
  'This Meridian evidence bundle packages user-visible context and source metadata for analyst handoff. It is not a legal evidentiary record; verify source availability, timestamps, and claims before reuse.';
...
export function renderCountryEvidenceMarkdown(bundle: CountryEvidenceBundle): string {
  const lines: string[] = [];
  lines.push(`# Meridian Evidence Bundle: ${bundle.country} (${bundle.code})`);
  lines.push('');
...
export class ExportPanel {
  ...
  private export(format: ExportFormat): void {
    ...
    const timestamp = new Date().toISOString().replace(/[:.]/g, '-');
    const filename = `meridian-${timestamp}`;
