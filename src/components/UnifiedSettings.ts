...
export class UnifiedSettings {
      ...
      if (target.closest('.upgrade-to-business-btn')) {
        ...
        void openBillingPortal(reservedWin).then((result) => {
          if (result.outcome === 'no-customer') {
            showToast(
              'Subscription is managed outside Dodo. Email support@meridian.app for help.',
            );
          }
        });
        return;
  ...
  private renderUpgradeSection(): string {
    ...
    if (!isEntitled() && hasPremiumAccess()) {
      return `
        <div class="upgrade-pro-section upgrade-pro-external" data-billing-state="external">
          ...
          <div class="upgrade-pro-desc">This access is not billed through your Meridian account. No payment method or invoices are available here.</div>
        </div>
      `;
    }
    const sub = getSubscription();
    ...
    return `
      <div class="upgrade-pro-section" data-billing-state="free">
        <div class="upgrade-pro-title">Meridian Free</div>
  ...
  private handleUpgradeClick(): void {
    ...
    if (this.config.isDesktopApp) {
      window.open('https://meridian.app/pro', '_blank', 'noopener,noreferrer');
      return;
    }
  ...
  private handlePlanLimitNoticeCta(noticeId: string): void {
    ...
    if (notice.ctaKind === 'billing_portal') {
      ...
      void openBillingPortal(reservedWin).then((result) => {
        if (result.outcome === 'no-customer') {
          showToast('Subscription is managed outside Dodo. Email support@meridian.app for help.');
        }
      });
      return;
    }
    ...
    if (notice.ctaKind === 'contact_support') {
      window.location.href = `mailto:support@meridian.app?subject=${encodeURIComponent('Meridian API plan limit upgrade')}`;
    }
  }
    ...
    if (!hasFeature('apiAccess')) {
      ...
      return `
        <div class="panel-locked-state">
          ...
          <div class="panel-locked-desc">Create and manage API keys to access Meridian data programmatically.</div>
    ...
    return `
      <div class="api-keys-section">
        ...
        <div class="api-keys-header">
          <p class="api-keys-desc">Create API keys to access Meridian data programmatically. Keys are shown once on creation — store them securely.</p>
        </div>
  ...
  private renderMcpClientsContent(): string {
    ...
    if (!hasFeature('mcpAccess')) {
      ...
      return `
        <div class="panel-locked-state">
          <div class="panel-locked-icon">${upgradeIcon}</div>
          <div class="panel-locked-desc">Connect Claude Desktop and other AI clients to your Meridian account.</div>
        </div>`;
    }
    ...
    return `
      <div class="mcp-clients-section">
        ...
        <div class="mcp-clients-header">
          <p class="mcp-clients-desc">Connect Claude Desktop, Cursor, and other AI clients to your Meridian account. Each client gets its own credential — revoke any time.</p>
        </div>
    ...
    if (active.length === 0 && revoked.length === 0) {
      ...
      setTrustedHtml(container, trustedHtml(`
        <div class="mcp-clients-empty">
          ...
          <div class="mcp-clients-empty-desc">To connect Claude Desktop or another AI client, paste this URL into the client's MCP server settings and sign in with your Meridian Pro account:</div>
