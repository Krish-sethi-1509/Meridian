...
async function bootEmbed(): Promise<void> {
  ...
  try {
    ...
    attribution.href = buildMeridianAttributionUrl(new URL('/dashboard', window.location.origin).toString(), getReferrerHost());
    attribution.target = '_blank';
    attribution.rel = 'noopener noreferrer';
    attribution.textContent = 'Live map by Meridian';
    ...
    mountError(root, 'Meridian map embed could not load.');
