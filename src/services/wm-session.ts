...
// If a caller already set Authorization / X-Meridian-Key / X-Api-Key, we
...
export function installWmSessionFetchInterceptor(): void {
  ...
  window.fetch = async function wmSessionFetch(input, init) {
    ...
    // X-Meridian-Key=wms_... here, the premium injector sees the header
    ...
    const sendWith = (h: Headers, src: typeof input): Promise<Response> => {
      ...
      if (
        ...
        !requestHeaders.has('X-Meridian-Key') &&
        !requestHeaders.has('X-Api-Key')
      ) {
        requestHeaders.set('X-Meridian-Key', anonymousSessionHeaderToken);
      }
