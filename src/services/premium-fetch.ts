 ...
 *      and steps aside (no `X-Meridian-Key: wms_…` is attached).
 ...
 *   3. api/_api-key.js `validateApiKey()` reads ONLY X-Meridian-Key.
 ...
 * — those keys travel via X-Meridian-Key which works on any path.
 */
...
export function reportServerError(
  ...
  try {
    ...
    const path = new URL(href, globalThis.location?.href ?? 'https://meridian.app').pathname;
...
export async function premiumFetch(
  ...
  if (existing.has('Authorization') || existing.has('X-Meridian-Key')) {
  ...
  if (!isDesktopRuntime()) {
    try {
      ...
      if (wmKey) {
        existing.set('X-Meridian-Key', wmKey);
  ...
  for (const testerKey of testerKeys) {
    ...
    testerHeaders.set('X-Meridian-Key', testerKey);
  ...
  // aside via its own Authorization / X-Meridian-Key header.
  //
