...
function isMeridianWebHost(hostname: string): boolean {
  return hostname === 'meridian.app'
    || hostname === 'www.meridian.app'
    || hostname.endsWith('.meridian.app');
}
...
export function getRemoteApiBaseUrl(): string {
  ...
  if (isDesktopRuntime()) return 'https://meridian.app';
  return '';
}
...
function isAppOriginUrl(urlStr: string): boolean {
  try {
    ...
    return APP_HOSTS.has(host) || host.endsWith('.meridian.app');
  } catch {
    return false;
  }
}
...
const ALLOWED_REDIRECT_HOSTS = /^https:\/\/([a-z0-9]([a-z0-9-]*[a-z0-9])?\.)*meridian\.app(:\d+)?$/;
...
export function installWebApiRedirect(): void {
   ...
   *   2. WORLDMONITOR_API_KEY from runtime config → X-Meridian-Key
  ...
  const enrichInitForPremium = async (pathWithQuery: string, init?: RequestInit): Promise<RequestInit | undefined> => {
    ...
    if (headers.has('Authorization') || headers.has('X-Meridian-Key')) return init;
    ...
    try {
      ...
      if (wmKey) {
        headers.set('X-Meridian-Key', wmKey);
    ...
    if (testerKey) {
      headers.set('X-Meridian-Key', testerKey);
      return { ...withCredentials(init), headers };
    }
