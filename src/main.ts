...
import './styles/watchlist-panel.css';
...
function shouldSuppressCspViolation(
  ...
  // this default-src fallback. Preserve first-party meridian.app http blocks
  ...
  if (directive === 'default-src') {
    try {
      ...
      if (u.protocol === 'http:'
          && u.hostname !== 'meridian.app'
          && !u.hostname.endsWith('.meridian.app')) return true;
    } catch { /* scheme-only values fall through */ }
  }
  ...
  // them. Scope to `meridian.app` and its subdomains — img-src blocks to foreign
  ...
  // `meridian.app.evil.com` do NOT match.
  //
  // REQUIRE https: protocol — our CSP only allows https: for img-src, so a real
  // mixed-content regression (`<img src="http://meridian.app/...">`) would be
  ...
  if (directive === 'img-src') {
    try {
      ...
      if (url.protocol === 'https:'
          && (url.hostname === 'meridian.app' || url.hostname.endsWith('.meridian.app'))) return true;
    } catch { /* scheme-only values fall through */ }
  }
...
if (urlParams.get('settings') === '1') {
  ...
  app
    ...
    .then(() => {
      ...
      // Personal addition: local watchlist + instability alerts panel.
      // Self-contained — safe to skip if anything upstream changes shape.
      try {
        watchlistAlertsPanel.mount();
      } catch (err) {
        console.error('[watchlist-alerts] failed to mount', err);
      }
    })
    .catch(console.error);
}
...
Object.defineProperty(window, 'beta', {
  get() {
    const on = localStorage.getItem('meridian-beta-mode') === 'true';
    console.log(`[Beta] ${on ? 'ON' : 'OFF'}`);
    return on;
  },
  set(v: boolean) {
    if (v) localStorage.setItem('meridian-beta-mode', 'true');
    else localStorage.removeItem('meridian-beta-mode');
    location.reload();
  },
});
