...
// Variants that are served from their own meridian.app subdomain by the
// single web deployment (vercel.json host-based rewrites map
// <variant>.meridian.app/dashboard → /dashboard-<variant>.html).
...
// the Meridian identity (each variant isPartOf Meridian — same
...
export function renderVariantDashboardHtml(fullDashboardHtml: string, variant: string): string {
  ...
  html = replaceCounted(
    html,
    /(<link rel="alternate" hreflang="[^"]+" href=")https:\/\/www\.meridian\.app\/dashboard((?:\?[^"]*)?" \/>)/g,
