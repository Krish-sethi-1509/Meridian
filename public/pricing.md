...
Live tier/price/product-ID data (JSON): `GET https://www.meridian.app/api/product-catalog` — public, no key required. Send a descriptive `User-Agent` (for example `mytool/1.0 (+https://yoursite.example)`); default HTTP-library user agents may be challenged by the edge firewall.

## License scope

- **Pro — Personal license:** one named subscriber may use Meridian for their own research, learning, investing and decisions. It does not cover employer, business or client use.
...
- **API Business — Commercial license for your customers:** the subscribing organization may display, embed or deliver API outputs as part of its own customer-facing product or service. It does not permit raw-data resale, shared API access or a standalone substitute for Meridian.
...
These subscription licenses cover the hosted Service and its outputs. They are separate from the AGPL-3.0-only platform source-code license and the MIT licenses on the official thin clients. Full scope, examples and restrictions: https://meridian.app/docs/terms#subscription-license-scope
...
- License: Commercial license included — use Meridian for client work, internal tools and reporting
...
- Best for: Developers and teams that want programmatic access to Meridian intelligence data
...
- License: Commercial license for your customers — embed Meridian data in what you sell; the 5 bundled Pro seats all require the same company email domain
- Limits: 300 requests/minute, 10,000 requests/day
- Upgrading from Starter: manage the switch from the billing portal (prorated immediately); new customers can subscribe directly at https://meridian.app/pro
...
- Per-endpoint request budgets are documented at https://www.meridian.app/docs/usage-rate-limits (also fetchable as markdown at https://www.meridian.app/docs/usage-rate-limits.md).
- Need a higher limit? Upgrade at https://meridian.app/pro or contact enterprise@meridian.app for custom quotas.
...
```json
{
  ...
  "pricing_url": "https://www.meridian.app/pro#pricing",
  "currency": "USD",
  "plans": [
    {
      "name": "Free",
