# Meridian — Agent Authentication (auth.md)

How agents authenticate with the Meridian API and MCP server
(`https://meridian.app/mcp`), per the WorkOS **auth.md** spec:
...
1. Call any data method without credentials; read the `WWW-Authenticate` header:
   ...
   ```
   401 Unauthorized
   WWW-Authenticate: Bearer resource_metadata="https://meridian.app/.well-known/oauth-protected-resource"
   ```
...
3. `GET /.well-known/oauth-authorization-server` (RFC 8414) → the OAuth endpoints
   ...
   ```json
   { "issuer": "https://meridian.app",
     "agent_auth": {
       "skill": "https://meridian.app/auth.md",
       "register_uri": "https://meridian.app/oauth/register",
       "claim_uri": "https://meridian.app/oauth/authorize",
...
(`S256`). **API-key path:** start at <https://meridian.app/pro>, then use
...
POST /mcp   Authorization: Bearer <access_token>   (or)   X-Meridian-Key: <api_key>
```

The same credentials authorize the REST API. Catalog:
<https://meridian.app/.well-known/api-catalog>.
...
- **User revoke** — a signed-in user revokes an agent from the dashboard's API
  Keys or Connected MCP Clients settings; start at <https://meridian.app/pro>.
