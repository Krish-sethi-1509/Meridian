...
No API key is required for this public RPC. If your agent already sends a Meridian key for quota attribution or a shared integration wrapper, use the `X-Meridian-Key` header. `Authorization: Bearer ...` is for MCP/OAuth or Clerk JWTs - **not** raw API keys.

```
X-Meridian-Key: wm_0123456789abcdef0123456789abcdef01234567
```

Issue a key at https://www.meridian.app/pro for authenticated and Pro-gated endpoints.
...
- Via MCP, use the unrest/conflict intelligence tools on `https://meridian.app/mcp`.

## References

- OpenAPI: https://meridian.app/openapi.json - operation `ListUnrestEvents`.
