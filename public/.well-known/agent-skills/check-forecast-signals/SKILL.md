...
version: 1
description: Retrieve probabilistic forecasts and their scorecard context. Use when the user asks what Meridian is forecasting, how probabilities shifted, or how calibrated the forecasts are.
---
...
curl -s --get -H "X-Meridian-Key: $WM_API_KEY" \
  -H "User-Agent: meridian-agent-skill/1.0" \
...
- Via MCP, use forecast-generation and prediction-market tools on `https://meridian.app/mcp`.

## References

- OpenAPI: https://meridian.app/openapi.json - operations `GetForecasts` and `GetForecastScorecard`.
