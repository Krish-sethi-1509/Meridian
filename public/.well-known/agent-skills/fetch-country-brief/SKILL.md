...
Use this skill when the user asks for a summary of the current geopolitical, economic, or security situation in a specific country. The endpoint returns a fresh AI-generated brief composed from the latest news, market, conflict, and infrastructure signals Meridian tracks for that country.
...
Browser requests from `meridian.app` get a free pass via CORS Origin trust, but agents will never hit that path. Issue a key at https://www.meridian.app/pro.
...
```bash
curl -s -H "X-Meridian-Key: $WM_API_KEY" \
...
- `401` — missing `X-Meridian-Key` (server-to-server callers).
...
- OpenAPI: [IntelligenceService.openapi.yaml](https://www.meridian.app/openapi.yaml) — operation `GetCountryIntelBrief`.
