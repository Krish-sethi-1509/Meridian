...
`/api/resilience/v1/get-resilience-score` is Pro-tier. Agents and other server-to-server callers MUST present an API key in the `X-Meridian-Key` header. `Authorization: Bearer …` is for MCP/OAuth or Clerk JWTs — **not** raw API keys.

```
X-Meridian-Key: wm_0123456789abcdef0123456789abcdef01234567
```

The key must be attached to a Pro subscription. Unauthenticated or free-tier requests return `401` / `403`. Issue a key at https://www.meridian.app/pro.
...
- OpenAPI: [ResilienceService.openapi.yaml](https://www.meridian.app/openapi.yaml) — operation `GetResilienceScore`.
- Auth matrix: https://www.meridian.app/docs/usage-auth
- Methodology: https://www.meridian.app/docs/documentation
