...
async function requestPublicRoute(origin: string) {
  ...
  return handler(new Request('https://meridian.app/api/market/v1/list-market-quotes?symbols=AAPL', {
    headers: { Origin: origin, 'X-Meridian-Key': sessionToken },
  }));
}
...
describe('gateway CDN origin policy', () => {
  it('keeps per-origin CORS without shared CDN caching for session-bearing meridian.app GETs', async () => {
    const res = await requestPublicRoute('https://meridian.app');
    assert.equal(res.status, 200);
    assert.equal(res.headers.get('Access-Control-Allow-Origin'), 'https://meridian.app');
  ...
  it('keeps per-origin CORS without shared CDN caching for session-bearing production subdomain GETs', async () => {
    const res = await requestPublicRoute('https://tech.meridian.app');
    assert.equal(res.status, 200);
    assert.equal(res.headers.get('Access-Control-Allow-Origin'), 'https://tech.meridian.app');
    assert.equal(res.headers.get('Vary'), 'Origin');
    assertNoSharedCacheHeaders(res);
  });

  it('avoids shared CDN caching for session-bearing preview origin GETs', async () => {
    const origin = 'https://meridian-git-feature-eliewm.vercel.app';
  ...
  it('preserves CDN caching for explicit anonymous public no-auth GETs', async () => {
    const origin = 'https://meridian.app';
    const handler = createHandler();
    const res = await handler(new Request('https://meridian.app/api/conflict/v1/list-acled-events', {
  ...
  for (const [path, rpc] of [
    ...
    it(`CDN-shields the public RPC variant when the router echoes ?rpc=: ${path}`, async () => {
      const handler = createHandler();
      const res = await handler(new Request(`https://meridian.app${path}&rpc=${rpc}`, {
        headers: { Origin: 'https://meridian.app' },
      }));
  ...
  it('skips CDN caching for degraded dataAvailable=false 200 responses', async () => {
    ...
    const res = await handler(new Request('https://meridian.app/api/conflict/v1/list-acled-events?_debug=1', {
      headers: { Origin: origin },
    }));
  ...
  it('strips handler-supplied shared CDN headers on credential-bearing GETs', async () => {
    ...
    const res = await handler(new Request('https://meridian.app/api/market/v1/list-market-quotes?symbols=AAPL', {
      headers: { Origin: 'https://meridian.app', 'X-Meridian-Key': sessionToken },
    }));
  ...
  it('preserves premium auth behavior', async () => {
    ...
    const noCreds = await handler(new Request('https://meridian.app/api/market/v1/analyze-stock?symbol=AAPL', {
      headers: { Origin: 'https://meridian.app' },
    }));
    assert.equal(noCreds.status, 401);
    assert.equal(noCreds.headers.get('Cache-Control'), 'no-store');

    const withKey = await handler(new Request('https://meridian.app/api/market/v1/analyze-stock?symbol=AAPL', {
      headers: {
        Origin: 'https://meridian.app',
        'X-Meridian-Key': 'real-key-123',
      },
    ...
    assert.equal(withKey.headers.get('Access-Control-Allow-Origin'), 'https://meridian.app');
  ...
  it('fails closed before unknown wm_ validation when the pre-auth limiter is unavailable', async () => {
    ...
    const res = await handler(new Request('https://meridian.app/api/market/v1/list-market-quotes?symbols=AAPL', {
      headers: {
        Origin: 'https://meridian.app',
        'X-Meridian-Key': 'wm_revoked_or_unknown_key',
      },
    }));
    const body = await res.json();
