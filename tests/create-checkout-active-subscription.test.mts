...
function makeCheckoutRequest(): Request {
  return new Request('https://meridian.app/api/create-checkout', {
    method: 'POST',
    headers: {
    ...
    body: JSON.stringify({
      ...
      returnUrl: 'https://meridian.app/?wm_checkout=return',
    }),
  });
}
...
describe('/api/create-checkout ACTIVE_SUBSCRIPTION_EXISTS relay handling', () => {
  it('forwards the typed duplicate-subscription response without logging a production error', async () => {
    ...
    assert.equal((relayInit.headers as Record<string, string>)['User-Agent'], 'meridian-checkout-edge/1.0');
  });
