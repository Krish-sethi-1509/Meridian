...
function makePostReq(body) {
  return new Request('https://meridian.app/api/internal/mcp-grant-mint', {
    method: 'POST',
...
function makeGetReq(nonce) {
  const url = nonce !== undefined
    ? `https://meridian.app/api/internal/mcp-grant-context?nonce=${encodeURIComponent(nonce)}`
    : `https://meridian.app/api/internal/mcp-grant-context`;
...
describe('mintGrantHandler', () => {
  ...
  it('returns 405 on non-POST', async () => {
    ...
    const req = new Request('https://meridian.app/api/internal/mcp-grant-mint', { method: 'GET' });
  ...
  it('returns 400 INVALID_REQUEST on non-JSON body', async () => {
    const { deps } = makeMintDeps();
    const req = new Request('https://meridian.app/api/internal/mcp-grant-mint', {
      method: 'POST',
      headers: { 'Content-Type': 'application/json', Authorization: 'Bearer fake-jwt' },
      body: 'not json {',
    });
...
describe('grantContextHandler', () => {
  ...
  it('returns 405 on non-GET', async () => {
    const { deps } = makeContextDeps();
    const req = new Request('https://meridian.app/api/internal/mcp-grant-context?nonce=x', {
      method: 'POST',
      headers: { Authorization: 'Bearer fake-jwt' },
    });
 ...
 * "A Meridian Pro subscription is required to authorize MCP clients." — a
