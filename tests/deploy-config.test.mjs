...
const APP_ROOT_HOST_PATTERN = '^(?:(?:www|tech|finance|commodity|happy|energy)\\.)?meridian\\.app$';
...
describe('crawlable content corpus deployment contracts', () => {
  ...
  it('keeps robots.txt advertising root, blog, and Mintlify docs sitemaps', () => {
    assert.match(robotsSource, /^Sitemap: https:\/\/www\.meridian\.app\/sitemap\.xml$/m);
    assert.match(robotsSource, /^Sitemap: https:\/\/www\.meridian\.app\/blog\/sitemap-index\.xml$/m);
    assert.match(robotsSource, /^Sitemap: https:\/\/www\.meridian\.app\/docs\/sitemap\.xml$/m);
  });
  ...
  it('keeps the root sitemap generated while delegating blog and docs inventories', () => {
    ...
    assert.ok(!sitemapSource.includes('https://www.meridian.app/blog/'));
    assert.ok(!sitemapSource.includes('https://www.meridian.app/docs/'));
  ...
  it('discovers canonical generated corpus pages and validates changelog pagination links', () => {
    ...
    try {
      writeFixturePage(
        ...
        '<link rel="canonical" href="https://www.meridian.app/countries/ukraine/" /><meta name="lastmod" content="2026-07-08" />'
      );
      writeFixturePage(
        publicDir,
        'chokepoints/suez-canal/index.html',
        '<link rel="canonical" href="https://www.meridian.app/chokepoints/suez-canal/" />'
      );
      writeFixturePage(
        publicDir,
        'crises/ukraine-war/index.html',
        '<link rel="canonical" href="https://www.meridian.app/crises/ukraine-war/" />'
      );
      writeFixturePage(
        publicDir,
        'tools/natural-hazard-pulse/index.html',
        '<link rel="canonical" href="https://www.meridian.app/tools/natural-hazard-pulse/" />'
      );
      writeFixturePage(
        publicDir,
        'reference/changelog/page/1/index.html',
        '<link rel="canonical" href="https://www.meridian.app/reference/changelog/page/1/" /><link rel="next" href="https://www.meridian.app/reference/changelog/page/2/" />'
      );
      writeFixturePage(
        publicDir,
        'reference/changelog/page/2/index.html',
        '<link rel="canonical" href="https://www.meridian.app/reference/changelog/page/2/" /><link rel="prev" href="https://www.meridian.app/reference/changelog/page/1/" />'
      );

      const pages = discoverContentCorpusPages({ publicDir });
      const locations = pages.map((page) => page.loc).sort();
      assert.deepEqual(locations, [
        'https://www.meridian.app/reference/changelog/page/1/',
        'https://www.meridian.app/reference/changelog/page/2/',
        'https://www.meridian.app/chokepoints/suez-canal/',
        'https://www.meridian.app/crises/ukraine-war/',
        'https://www.meridian.app/countries/ukraine/',
        'https://www.meridian.app/tools/natural-hazard-pulse/',
      ].sort());

      writeFixturePage(
        publicDir,
        'reference/changelog/page/3/index.html',
        '<link rel="canonical" href="https://www.meridian.app/reference/changelog/page/3/" />'
      );
      assert.throws(
...
describe('deploy/cache configuration guardrails', () => {
  ...
  it('serves /pro hashed assets immutable — broader /pro rules must not override', () => {
    ...
    assert.equal(
      effectiveCacheControl('/pro/assets/meridian-7-mar-2026-abc.jpg'),
      'public, max-age=31536000, immutable'
    );
...
// welcome route is consistent across meridian.app, finance.meridian.app,
// tech.meridian.app, commodity.meridian.app, happy.meridian.app, and
// energy.meridian.app.
...
describe('welcome landing page routing', () => {
  ...
  it('routes app roots to welcome and leaves non-app roots on the dashboard catch-all', () => {
    assert.equal(rootDestinationForHost('meridian.app'), '/pro/welcome.html');
    assert.equal(rootDestinationForHost('www.meridian.app'), '/pro/welcome.html');
    assert.equal(rootDestinationForHost('meridian.app.evil.example'), DASHBOARD_HTML_DESTINATION);

    const variantHosts = getVariantHosts().filter((host) => host !== 'www.meridian.app');
  ...
  it('keeps variant canonicals aligned with the /dashboard routing strategy', () => {
    ...
    assert.equal(variantUrls.full, 'https://www.meridian.app/dashboard');
  ...
  it('sitemap lists dashboard routes and does not list legacy /welcome', () => {
    ...
    assert.ok(
      sitemap.includes('<loc>https://www.meridian.app/dashboard</loc>'),
      'public/sitemap.xml must list https://www.meridian.app/dashboard'
    );
    for (const host of ['tech', 'finance', 'commodity', 'happy', 'energy']) {
      assert.ok(
        sitemap.includes(`<loc>https://${host}.meridian.app/dashboard</loc>`),
        `public/sitemap.xml must list https://${host}.meridian.app/dashboard`
      );
    }
    assert.ok(
      !sitemap.includes('<loc>https://www.meridian.app/welcome</loc>'),
      'public/sitemap.xml must not list legacy https://www.meridian.app/welcome'
    );
  });
  ...
  it('pins welcome and dashboard SEO canonicals to their new routes', () => {
    ...
    assert.ok(
      welcomeHtml.includes('<link rel="canonical" href="https://www.meridian.app/" />'),
      'welcome source must canonicalize to root'
    );
    assert.ok(
      !welcomeHtml.includes('https://www.meridian.app/welcome'),
      'welcome source must not emit legacy /welcome SEO URLs'
    );
    assert.ok(
      generatedWelcomeHtml.includes('<link rel="canonical" href="https://www.meridian.app/" />'),
      'generated welcome HTML must canonicalize to root'
    );
    assert.ok(
      !generatedWelcomeHtml.includes('https://www.meridian.app/welcome'),
      'generated welcome HTML must not emit legacy /welcome SEO URLs'
    );
    assert.ok(
      generatedWelcomeHtml.includes('https://www.meridian.app/dashboard'),
      'generated welcome HTML must launch the dashboard at /dashboard'
    );
    assert.ok(
      dashboardHtml.includes('<link rel="canonical" href="https://www.meridian.app/dashboard" />'),
  ...
  it('keeps welcome dashboard launch CTAs off the root welcome route', () => {
    ...
    const variantRootWelcomeLaunchLink = /https:\/\/(?:tech|finance|commodity|happy|energy)\.meridian\.app\/\?ref=welcome-/;
    assert.doesNotMatch(
...
describe('security header guardrails', () => {
  ...
  it('CSP frame-src includes Clerk origin for auth modals', () => {
    ...
    assert.ok(
      frameSrc.includes('clerk.accounts.dev') || frameSrc.includes('clerk.meridian.app'),
...
describe('agent readiness: api-catalog + openapi build', () => {
  ...
  it('linkset[0] is the catalog anchor and enumerates each API via RFC 9727 item links', () => {
    assert.equal(catalogEntry.anchor, 'https://meridian.app/.well-known/api-catalog');
    ...
    assert.ok(itemHrefs.includes('https://meridian.app/mcp'), 'item list must enumerate the MCP server');
    assert.ok(
      itemHrefs.includes('https://www.meridian.app/docs/mcp'),
  ...
  it('advertises the docs MCP server on every discovery surface', () => {
    const docsMcpUrl = 'https://www.meridian.app/docs/mcp';
  ...
  it('the docs MCP anchor describes itself with the first-party server-card (service-desc parity with product MCP)', () => {
    const docsAnchor = apiCatalog.linkset.find((e) => e.anchor === 'https://www.meridian.app/docs/mcp');
    ...
    assert.ok(
      desc.some((d) => d.href === 'https://www.meridian.app/.well-known/mcp/docs-server-card.json'),
  ...
  it('the first-party docs server-card advertises the working /docs/mcp endpoint, not the dead mintlify url', () => {
    ...
    // endpoint that actually initializes. meridian.mintlify.dev/mcp 404s;
    // www.meridian.app/docs/mcp returns 200. The committed card must carry
    ...
    const WORKING = 'https://www.meridian.app/docs/mcp';
  ...
  it('service-meta advertises the machine-readable pricing + support surfaces', () => {
    ...
    assert.ok(hrefs.includes('https://meridian.app/pricing.md'), 'service-meta must advertise pricing.md');
    assert.ok(
      hrefs.includes('https://www.meridian.app/api/product-catalog'),
      'service-meta must advertise the live product-catalog JSON endpoint'
    );
    assert.ok(hrefs.includes('https://meridian.app/support.md'), 'service-meta must advertise support.md');
    assert.ok(hrefs.includes('https://meridian.app/agents.md'), 'service-meta must advertise agents.md (#4952)');
    ...
    const commerceSpec = meta.find(
      (entry) => entry.href === 'https://www.meridian.app/docs/openapi/CommerceService.openapi.yaml'
    );
  ...
  it('has a second anchor for the MCP server-card', () => {
    const mcpEntry = apiCatalog.linkset.find((entry) => entry.anchor === 'https://meridian.app/mcp');
    assert.ok(mcpEntry, 'linkset must contain an anchor for https://meridian.app/mcp');
...
describe('agent readiness: MCP/OAuth origin alignment', () => {
  ...
  it('api/mcp.ts resource_metadata is host-derived, not hardcoded', () => {
    ...
    assert.ok(
      !/resource_metadata="https:\/\/(?:api\.)?meridian\.app\/\.well-known\//.test(source),
  ...
  it('discovery handlers reject spoofed Host (apex fallback) and non-GET methods', async () => {
    ...
    for (const host of ['evil.com', 'meridian.app.evil.com', 'evilmeridian.app', 'x.y.meridian.app']) {
      const prmRes = await prm(new Request('https://meridian.app/.well-known/oauth-protected-resource', { headers: { host } }));
      const prmJson = await prmRes.json();
      assert.equal(prmJson.resource, 'https://meridian.app', `PRM must not reflect spoofed host ${host}`);
      assert.deepEqual(prmJson.authorization_servers, ['https://meridian.app']);

      const asRes = await as(new Request('https://meridian.app/.well-known/oauth-authorization-server', { headers: { host } }));
      const asJson = await asRes.json();
      assert.equal(asJson.issuer, 'https://meridian.app', `AS must not reflect spoofed host ${host}`);
      assert.equal(asJson.token_endpoint, 'https://meridian.app/oauth/token', `AS token_endpoint must not carry spoofed host ${host}`);
      assert.equal(asJson.agent_auth.register_uri, 'https://meridian.app/oauth/register');
      assert.equal(asJson.agent_auth.claim_uri, 'https://meridian.app/oauth/authorize', `AS claim_uri must not carry spoofed host ${host}`);
      assert.equal(asJson.agent_auth.anonymous.claim_uri, 'https://meridian.app/oauth/authorize');
    }

    // Legit subdomain still self-describes.
    const variant = await as(new Request('https://tech.meridian.app/.well-known/oauth-authorization-server', { headers: { host: 'tech.meridian.app' } }));
    assert.equal((await variant.json()).issuer, 'https://tech.meridian.app');

    // Method guard: OPTIONS → 204 preflight, other verbs → 405 + Allow, GET → 200.
    for (const handler of [prm, as]) {
      const opt = await handler(new Request('https://meridian.app/x', { method: 'OPTIONS', headers: { host: 'meridian.app' } }));
      ...
      const post = await handler(new Request('https://meridian.app/x', { method: 'POST', headers: { host: 'meridian.app' } }));
      assert.equal(post.status, 405, 'non-GET/HEAD is rejected');
      assert.equal(post.headers.get('allow'), 'GET, HEAD, OPTIONS');

      const get = await handler(new Request('https://meridian.app/x', { headers: { host: 'meridian.app' } }));
...
describe('agent readiness: auth.md walkthrough', () => {
  ...
  it('advertises a register endpoint that resolves (matches the agent_auth register_uri path)', () => {
    assert.match(
      authMd,
      /https:\/\/(?:api\.)?meridian\.app\/oauth\/register/,
  ...
  it('serves /agent.txt as plain text and keeps it off the SPA catch-all', () => {
    ...
    assert.ok(agentTxt.includes('https://meridian.app/mcp'), 'agent.txt must point at the MCP server');
  });
});
...
describe('agent readiness: homepage Link headers', () => {
  ...
  for (const source of ['/', '/dashboard', '/dashboard.html']) {
    it(`${source} emits a Link header`, () => {
      ...
      // card's url points at meridian.mintlify.dev/mcp which 404s on
...
describe('agent readiness: registry branding + ARD catalog', () => {
  ...
  it('server-card carries the full branding trio and the icon asset exists', () => {
    ...
    assert.match(
      ...
      'server-card icon must be an absolute meridian.app URL'
    );
  ...
  it('ai-catalog.json declares the Meridian host identity', () => {
    assert.strictEqual(aiCatalog.specVersion, '1.0');
    assert.strictEqual(aiCatalog.host?.displayName, 'Meridian');
    assert.strictEqual(aiCatalog.host?.identifier, 'did:web:meridian.app');
  ...
  it('every ai-catalog entry is domain-anchored and complete', () => {
    for (const entry of aiCatalog.entries) {
      ...
      assert.match(
        entry.identifier ?? '',
        /^urn:air:meridian\.app:[a-z-]+:[a-z0-9-]+$/,
...
describe('variant subdomain dashboard SEO (#4996)', () => {
  ...
  const rewriteVariants = dashboardRewrites
    ...
    .map((r) => {
      ...
      return host.replace('.meridian.app', '');
    })
    .sort();

  const middlewareVariants = [...middlewareSource.matchAll(/'([a-z]+)\.meridian\.app': '([a-z]+)'/g)]
    .map((m) => m[2])
    .sort();
  ...
  it('each variant host rewrite targets its generated variant file', () => {
    for (const rule of dashboardRewrites.filter((r) => r.has)) {
      ...
      const variant = host.replace('.meridian.app', '');
      assert.match(host, /^[a-z]+\.meridian\.app$/, `unexpected host condition shape: ${host}`);
      assert.strictEqual(
        rule.destination,
...
describe('docs host scoping — Mintlify proxy is www-only (#5345)', () => {
  // The /docs rewrite proxies meridian.mintlify.dev with no host condition,
  ...
  it('redirects subdomain /docs/* to www permanently', () => {
    ...
    assert.equal(docsHostRedirect.destination, 'https://www.meridian.app/docs/:match*');
  ...
  it('the host condition covers every variant subdomain plus api., and never www.', () => {
    ...
    assert.ok(!hostRe.test('www.meridian.app'), 'www must keep serving the docs proxy');
  });
  ...
  it('redirects prefix-less /api-reference/* (Googlebot fetches Mintlify RSC route strings) into /docs on www', () => {
    ...
    assert.equal(redirect.destination, 'https://www.meridian.app/docs/api-reference/:match*');
    assert.equal(redirect.permanent, true);
  ...
  it('the www docs proxy rewrite itself is untouched', () => {
    ...
    assert.equal(proxy.destination, 'https://meridian.mintlify.dev/docs/:match*');
  });
});
...
describe('markdown canonical Link headers (#4999)', () => {
  ...
  for (const page of MD_PAGES) {
    it(`${page} declares a self-referencing canonical Link header`, () => {
      assert.strictEqual(
        ...
        `<https://www.meridian.app${page}>; rel="canonical"`,
  ...
  it('every sitemap-listed .md URL has the canonical Link header rule', () => {
    const sitemap = readFileSync(resolve(__dirname, '../public/sitemap.xml'), 'utf-8');
    const mdUrls = [...sitemap.matchAll(/<loc>https:\/\/www\.meridian\.app(\/[^<]+\.md)<\/loc>/g)].map((m) => m[1]);
...
// Monitor MCP server", "Meridian OpenAPI", "Meridian developer
// portal", or "Meridian SDK" must land on a crawlable page whose H1 names
...
describe('agent readiness: named developer-resource pages (#4953)', () => {
  const DEV_PAGES = [
    { file: 'developers.md', path: '/developers.md', h1: '# Meridian Developer Portal' },
    { file: 'mcp-server.md', path: '/mcp-server.md', h1: '# Meridian MCP Server' },
    { file: 'openapi.md', path: '/openapi.md', h1: '# Meridian OpenAPI Specification' },
    { file: 'sdks.md', path: '/sdks.md', h1: '# Meridian SDKs' },
  ];
  ...
  it('advertises the developer portal + resource pages across the discovery chain', () => {
    ...
    // The sitemap and the indexed "Build on Meridian" blog post are the two
    ...
    const blogPost = readFileSync(
      resolve(__dirname, '../blog-site/src/content/blog/build-on-meridian-developer-api-open-source.md'),
      'utf-8'
    );
    for (const page of DEV_PAGES) {
      const url = `https://meridian.app${page.path}`;
      ...
      assert.ok(
        sitemap.includes(`https://www.meridian.app${page.path}`),
...
describe('NLWeb schemamap (/schemamap.xml)', () => {
  ...
  it('every advertised <loc> resolves to a tracked file or a live route', () => {
    ...
    const resolvable = {
      ...
      'https://www.meridian.app/blog/rss.xml': () =>
        existsSync(resolve(__dirname, '../blog-site/src/pages/rss.xml.ts')),
      'https://www.meridian.app/blog/glossary/': () =>
...
describe('docs MCP facade (/docs/mcp)', () => {
  ...
  it('the first-party docs server card still points at the facade URL', () => {
    ...
    assert.equal(card.url, 'https://www.meridian.app/docs/mcp');
    assert.equal(card.serverUrl, 'https://www.meridian.app/docs/mcp');
  });
});
...
describe('section-scoped llms.txt files', () => {
  ...
  it('the site-wide llms.txt cross-links every section file and the sandbox', () => {
    ...
    for (const url of [
      'https://meridian.app/api/llms.txt',
      'https://www.meridian.app/docs/llms.txt',
      'https://meridian.app/developers/llms.txt',
      'https://www.meridian.app/blog/llms.txt',
      'https://www.meridian.app/sandbox/index.json',
      'https://www.meridian.app/schemamap.xml',
    ]) {
...
describe('skeleton brand text extraction (#5541)', () => {
  ...
  it('.skeleton-brand raw textContent does not contain "WWorld"', () => {
    ...
    assert.doesNotMatch(rawText, /WWorld/, 'skeleton-brand raw text must not concatenate as "WMeridian"');
    assert.match(rawText, /Meridian/, 'skeleton-brand raw text must contain "Meridian"');
  });
