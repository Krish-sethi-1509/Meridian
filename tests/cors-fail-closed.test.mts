...
describe('cors helper', () => {
  it('returns headers for a well-formed request', () => {
    const req = new Request('https://meridian.app/x', {
      headers: { Origin: 'https://meridian.app' },
    });
    const headers = getCorsHeaders(req);
    assert.equal(headers['Access-Control-Allow-Origin'], 'https://meridian.app');
    assert.match(
...
// The Vercel project moved from the personal scope (meridian-*-elie-<hash>)
...
describe('isAllowedOrigin — Vercel preview allowlist (eliewm team scope)', () => {
  ...
  const allowedByJsTwin = (origin: string) =>
    !isDisallowedOriginJs(new Request('https://meridian.app/x', { headers: { Origin: origin } }));

  const ALLOWED = [
    ['git-branch alias URL', 'https://meridian-git-feature-eliewm.vercel.app'],
    ['hash deployment URL', 'https://meridian-abc123def456-eliewm.vercel.app'],
    ['apex production origin', 'https://meridian.app'],
    ['production subdomain', 'https://tech.meridian.app'],
  ];

  const REJECTED = [
    ['non-meridian vercel.app origin', 'https://some-other-app-eliewm.vercel.app'],
    ['foreign team scope', 'https://meridian-git-feature-attacker.vercel.app'],
    ['bare meridian vercel.app (no scope segment)', 'https://meridian.vercel.app'],
    ['suffix-spoofed eliewm origin', 'https://meridian-git-feature-eliewm.vercel.app.evil.com'],
    ['dead personal-scope preview (post-migration)', 'https://meridian-feature-elie-abc123.vercel.app'],
  ];
...
describe('X-Billing-Verification is readable cross-origin (#5622)', () => {
  ...
  const SURFACES: Array<[string, () => Record<string, string>]> = [
    ...
    ['workers/api-cors-preflight buildCorsHeaders', () => buildCorsHeadersWorker('https://meridian.app')],
  ];
  ...
  for (const rel of TWINS) {
    it(`${rel} scopes Vercel previews to the eliewm team`, async () => {
      ...
      assert.ok(
        ...
        `${rel} must allow meridian-*-eliewm.vercel.app previews`,
      );
      assert.ok(
        !source.includes('meridian-[a-z0-9-]+\\.vercel\\.app'),
...
describe('CORS Worker superset invariant — edge allowlist ⊇ function allowlist', () => {
  ...
  // the preflight echoes the canonical meridian.app fallback and the
  ...
  const PROD_ORIGINS = [
    ...
    'https://meridian-git-feature-eliewm.vercel.app',
    'https://meridian-abc123def456-eliewm.vercel.app',
    'tauri://localhost',
    'asset://localhost',
    ...
    'https://meridian-git-feature-attacker.vercel.app',
    'https://meridian-feature-elie-abc123.vercel.app',
    'https://evil.com',
  ];
    ...
    it(`Worker allows everything the function allows: ${origin}`, () => {
      if (fnAllows(origin)) {
        assert.equal(
          ...
          true,
          `Worker rejects ${origin} that api/_cors.js accepts — its OPTIONS preflight will echo the meridian.app fallback and the browser will block it`,
        );
      }
    });
  }
