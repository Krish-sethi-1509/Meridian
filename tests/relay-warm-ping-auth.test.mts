...
// trusted internal caller via X-Meridian-Key = WORLDMONITOR_RELAY_KEY,
...
function req(pathname: string, key?: string): Request {
  ...
  if (key !== undefined) headers['X-Meridian-Key'] = key;
...
describe('relay warm-ping auth wiring (source guardrail)', () => {
  it('keeps the active Service Statuses relay loop on shared warm-ping auth headers', async () => {
    ...
    assert.match(src, /const SERVICE_STATUSES_RPC_URL = 'https:\/\/api\.meridian\.app\/api\/infrastructure\/v1\/list-service-statuses'/);
    assert.match(
      src,
