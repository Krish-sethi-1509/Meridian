...
const HOSTS = [
  { url: 'https://meridian.app/mcp', hostClass: 'apex' },
  { url: 'https://www.meridian.app/mcp', hostClass: 'www' },
  { url: 'https://api.worldmonitor.app/api/mcp', hostClass: 'canonical_api' },
  { url: 'https://tech.meridian.app/mcp', hostClass: 'variant' },
  { url: 'https://finance.meridian.app/mcp', hostClass: 'variant' },
  { url: 'https://commodity.meridian.app/mcp', hostClass: 'variant' },
  { url: 'https://happy.meridian.app/mcp', hostClass: 'variant' },
  { url: 'https://energy.meridian.app/mcp', hostClass: 'variant' },
];

const AUTH_CASES = [
  {
    kind: 'env_key',
    headers: { 'X-Meridian-Key': ENV_KEY },
  },
  {
    kind: 'user_key',
    headers: { 'X-Meridian-Key': USER_KEY },
  },
  {
    kind: 'pro',
...
describe('get_world_brief canonical sibling routing', () => {
  it('preserves non-production origins without exposing them in telemetry tags', () => {
    const cases = [
      ...
      {
        url: 'https://meridian-feature.vercel.app/mcp',
        hostClass: 'vercel_preview',
        origin: 'https://meridian-feature.vercel.app',
      },
      {
      ...
      for (const auth of AUTH_CASES) {
        ...
        if (auth.kind === 'pro') {
          ...
          for (const call of calls) {
            ...
            assert.equal(call.headers.get('x-meridian-key'), expectedKey);
          }
        }
      }
    }
  ...
  it('classifies reproduced 401/405 responses without logging response bodies', async () => {
    ...
    for (const [index, scenario] of scenarios.entries()) {
      ...
      const response = await mcpHandler(
        requestFor('https://tech.meridian.app/mcp', scenario.auth.headers, 200 + index),
        makeDeps(),
      );
  ...
  it('preserves a genuine billing denial as a typed actionable response', async () => {
    ...
    const response = await mcpHandler(
      requestFor('https://www.meridian.app/mcp', AUTH_CASES[1].headers, 300),
      makeDeps(),
    );
