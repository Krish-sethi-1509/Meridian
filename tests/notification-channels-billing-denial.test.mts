...
function makeSetChannelRequest(): Request {
  return new Request('https://meridian.app/api/notification-channels', {
    method: 'POST',
    headers: {
...
describe('/api/notification-channels POST billing-verification contract', () => {
  ...
  it('GET stays ungated, so the timed context read can never meet the retry path', async () => {
    ...
    const res = await mod.default(
      new Request('https://meridian.app/api/notification-channels', {
        method: 'GET',
        headers: { Origin: 'https://meridian.app', Authorization: 'Bearer clerk-token' },
      }),
      ctx,
    );
