...
function openApiArtifacts() {
  return [
    ...
    ...yamlSpecs.map((file) => ({
      family: file === 'meridian.openapi.yaml' ? 'bundle' : 'yaml',
      file,
...
describe('OpenAPI deprecated + operation-description contract', () => {
  ...
  it('propagates deprecated corporate-intelligence fields to every generated contract', () => {
    ...
    for (const file of ['IntelligenceService.openapi.yaml', 'meridian.openapi.yaml']) {
    ...
    for (const family of ['client', 'server']) {
      const generated = readFileSync(
        resolve(root, `src/generated/${family}/meridian/intelligence/v1/service_${family}.ts`),
        'utf8',
      );
...
function countDeprecatedRpcDeclarations() {
  let count = 0;
  for (const file of walkProtoFiles(resolve(root, 'proto/meridian'))) {
