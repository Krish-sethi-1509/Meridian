...
describe('feed catalog drift', () => {
  ...
  it('migrates exact non-English pre-strategic default and cap states', () => {
    ...
    const blob = {
      'meridian-disabled-feeds': JSON.stringify([...plStates[0]!]),
    };
    ...
    const disabled = new Set(
      JSON.parse(migrated['meridian-disabled-feeds'] as string) as string[],
    );
    for (const name of STRATEGIC_DEFAULTS) {
  ...
  it('migrates dormant schema-1/2 EN rows from before #5949 through schema 6', () => {
    ...
    for (const fromVersion of [1, 2]) {
      const blob = {
        'meridian-disabled-feeds': JSON.stringify([...preFrontlineDefault]),
      };
  ...
  it('fails closed for locale-less cloud fingerprints with conflicting policies', () => {
    ...
    for (const matching of targetsByLegacyState.values()) {
      ...
      const blob = {
        'meridian-disabled-feeds': JSON.stringify([...matching[0]!.legacyDisabled]),
      };
      assert.equal(
