...
describe('premium gateway API key enforcement', () => {
  it('enforces premium credentials while allowing public market session auth', async () => {
    ...
    const browserNoKey = await handler(new Request('https://meridian.app/api/market/v1/analyze-stock?symbol=AAPL', {
    ...
    const resilienceScoreNoKey = await handler(new Request('https://meridian.app/api/resilience/v1/get-resilience-score?countryCode=US', {
      headers: { Origin: 'https://meridian.app' },
    }));
    assert.equal(resilienceScoreNoKey.status, 401);

    const resilienceRankingNoKey = await handler(new Request('https://meridian.app/api/resilience/v1/get-resilience-ranking', {
      headers: { Origin: 'https://meridian.app' },
    }));
    ...
    const browserWithKey = await handler(new Request('https://meridian.app/api/market/v1/analyze-stock?symbol=AAPL', {
      headers: {
        Origin: 'https://meridian.app',
        'X-Meridian-Key': 'real-key-123',
      },
    }));
    assert.equal(browserWithKey.status, 200);

    const resilienceScoreWithKey = await handler(new Request('https://meridian.app/api/resilience/v1/get-resilience-score?countryCode=US', {
      headers: {
        Origin: 'https://meridian.app',
        'X-Meridian-Key': 'real-key-123',
      },
    ...
    const resilienceRankingWithKey = await handler(new Request('https://meridian.app/api/resilience/v1/get-resilience-ranking', {
      headers: {
        Origin: 'https://meridian.app',
        'X-Meridian-Key': 'real-key-123',
      },
    ...
    const publicAllowed = await handler(new Request('https://meridian.app/api/market/v1/list-market-quotes?symbols=AAPL', {
      headers: { Origin: 'https://meridian.app', 'X-Meridian-Key': SESSION_TOKEN },
    }));
    assert.equal(publicAllowed.status, 200);

    const insiderTransactionsAllowed = await handler(new Request('https://meridian.app/api/market/v1/get-insider-transactions?symbol=AAPL', {
      headers: { Origin: 'https://meridian.app', 'X-Meridian-Key': SESSION_TOKEN },
    }));
  ...
  it('standardizes issue #4609 Pro RPCs behind the entitlement 403 gate', async () => {
    ...
    try {
      for (const { method, path } of ISSUE_4609_GATED_ROUTES) {
        const res = await handler(new Request(`https://meridian.app${path}`, {
          method,
          headers: {
            Origin: 'https://meridian.app',
  ...
  it('strips client-supplied x-user-id before an anonymous session reaches handlers', async () => {
    ...
    const res = await handler(new Request('https://meridian.app/api/market/v1/list-market-quotes?symbols=AAPL', {
      headers: {
        Origin: 'https://meridian.app',
        'X-Meridian-Key': SESSION_TOKEN,
  ...
  it('rewrites client-supplied x-user-id on wm_ user-API-key auth (#3548)', async () => {
    ...
    try {
      const res = await handler(
        new Request('https://meridian.app/api/market/v1/analyze-stock?symbol=AAPL', {
          headers: {
            Origin: 'https://meridian.app',
            'X-Meridian-Key': OWNER_PRO_USER_KEY,
...
describe('POST-to-GET compatibility hardening', () => {
  ...
  function compatPost(body: string, headers: Record<string, string> = {}) {
    return new Request('https://meridian.app/api/market/v1/list-market-quotes', {
      method: 'POST',
      headers: {
        Origin: 'https://meridian.app',
        'X-Meridian-Key': SESSION_TOKEN,
...
describe('premium gateway bearer token auth', () => {
  ...
  it('valid Pro bearer token unlocks tier-1 entitlement-gated endpoints without a Convex row', async () => {
    ...
    const res = await handler(new Request('https://meridian.app/api/market/v1/analyze-stock?symbol=AAPL', {
      headers: {
        Origin: 'https://meridian.app',
  ...
  it('rejects free bearer token on resilience premium endpoints → 403', async () => {
    ...
    const scoreRes = await handler(new Request('https://meridian.app/api/resilience/v1/get-resilience-score?countryCode=US', {
      headers: {
        Origin: 'https://meridian.app',
        Authorization: `Bearer ${token}`,
      },
    ...
    const rankingRes = await handler(new Request('https://meridian.app/api/resilience/v1/get-resilience-ranking', {
      headers: {
        Origin: 'https://meridian.app',
        Authorization: `Bearer ${token}`,
      },
  ...
  it('rewrites spoofed x-user-id from a verified legacy bearer before reaching handlers', async () => {
    ...
    const res = await headerEchoHandler(new Request('https://meridian.app/api/resilience/v1/get-resilience-score?countryCode=US', {
      headers: {
        Origin: 'https://meridian.app',
        Authorization: `Bearer ${token}`,
        'x-user-id': 'attacker-controlled-user',
  ...
  it('forwards POST body alongside trusted x-user-id on the legacy bearer path', async () => {
    ...
    const res = await echoHandler(new Request('https://meridian.app/api/intelligence/v1/deduct-situation', {
      method: 'POST',
      headers: {
        Origin: 'https://meridian.app',
        Authorization: `Bearer ${token}`,
