...
function makeReq(q?: string, method = 'GET'): Request {
  const url = q === undefined
    ? 'https://meridian.app/api/symbol-search'
    : `https://meridian.app/api/symbol-search?q=${encodeURIComponent(q)}`;
  return new Request(url, { method, headers: { 'X-Meridian-Key': TEST_KEY } });
}
...
describe('symbol-search handler', () => {
  it('returns mapped Finnhub results for a query, with our User-Agent', async () => {
    ...
    assert.equal(requestedUA, 'meridian-edge/1.0');
