...
describe('news digest methodology parity', () => {
  it('keeps SummarizeArticle headline limits aligned across implementation and API docs', () => {
    ...
    const meridianRequestYaml = extractYamlSchemaBlock(
      meridianOpenApiYaml,
      'meridian_news_v1_SummarizeArticleRequest',
    );
  ...
  it('documents importance-score boosts in the API contract', () => {
    ...
    const meridianNewsItemYaml = extractYamlSchemaBlock(
      meridianOpenApiYaml,
      'meridian_news_v1_NewsItem',
    );
    for (const surface of [
      newsItemProtoText,
      importanceDescription,
      newsServiceNewsItemYaml,
      meridianNewsItemYaml,
    ]) {
      assert.ok(
  ...
  it('documents emitted threat classification sources in the API contract', () => {
    ...
    const meridianThreatYaml = extractYamlSchemaBlock(
      meridianOpenApiYaml,
      'meridian_news_v1_ThreatClassification',
    );
    for (const surface of [
      newsItemProtoText,
      sourceDescription,
