...
describe('research report corpus (#5668)', () => {
  ...
  before(async () => {
    ...
    await buildCorpus({ rootDir: repoRoot, outDir, baseUrl: 'https://www.meridian.app' });
  ...
  it('agrees on title, author, and headline facts across page, structured data, and downloads', () => {
    ...
    assert.equal(reportLd.url, `https://www.meridian.app/research/${report.slug}/`);
  ...
  it('links the hub, methodology, live surfaces, and distributions correctly', () => {
    ...
    assert.match(html, new RegExp(`og:image" content="https://www\\.meridian\\.app/research-assets/${report.slug}-og\\.png"`));
    assert.doesNotMatch(
      html.match(/og:image:alt" content="([^"]+)"/)[1],
      /^Meridian — real-time global intelligence dashboard/,
  ...
  it('separates evidence layers and states failure modes in plain language', () => {
    ...
    assert.match(html, /abacus\.meridian\.app\/script\.js/, 'research pages must load analytics');
  ...
  it('regenerates byte-identically from the committed snapshot', async () => {
    ...
    try {
      await buildCorpus({ rootDir: repoRoot, outDir: secondDir, baseUrl: 'https://www.meridian.app' });
      for (const path of [
