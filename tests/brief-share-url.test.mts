...
describe('buildPublicBriefUrl', () => {
  it('returns a full URL under baseUrl with the derived hash in the path', async () => {
    ...
    assert.match(url, /^https:\/\/meridian\.app\/api\/brief\/public\/[A-Za-z0-9_-]{12}$/);
  ...
  it('trims trailing slashes from baseUrl', async () => {
    ...
    assert.ok(url.startsWith('https://meridian.app/api/brief/public/'));
    assert.ok(!url.startsWith('https://meridian.app///'));
  });
});
