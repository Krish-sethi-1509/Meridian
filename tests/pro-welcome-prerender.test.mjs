...
test('built welcome page ships the real hero in #root before JavaScript', () => {
  ...
  assert.match(rootContent, /href="\/blog\/posts\/meridian-is-not-palantir\/"/);
  assert.match(rootContent, /Meridian is not an open-source Palantir/);
  assert.match(rootContent, /Which Meridian license do I need\?/);
  assert.match(rootContent, /API Business lets that organization embed Meridian data/);
  assert.match(rootContent, /href="\/docs\/terms"[^>]*>meridian\.app\/docs\/terms<\/a>/);
  ...
  assert.match(rootContent, /<img[^>]+src="\/pro\/assets\/meridian-7-mar-2026-[^"]+\.jpg"[^>]+fetchPriority="high"/);
});
