...
function makeRequest(pathAndQuery: string): Request {
  ...
  return new Request(
    'https://meridian.app' + pathAndQuery,
    { headers: { Origin: 'https://meridian.app', 'X-Meridian-Key': TEST_KEY } },
  );
}
...
function makeJsonPostRequest(path: string, bodyObject: Record<string, unknown>): Request {
  ...
  return new Request('https://meridian.app' + path, {
    method: 'POST',
    headers: {
      Origin: 'https://meridian.app',
      'X-Meridian-Key': TEST_KEY,
...
async function getGeneratedMilitaryFlightsPathAndQuery(req: Parameters<MilitaryServiceClient['listMilitaryFlights']>[0]): Promise<string> {
  let requestedUrl = '';
  const client = new MilitaryServiceClient('https://meridian.app', {
    fetch: async (input) => {
...
function assertNoBboxDiagnostic(res: Response): void {
  assert.equal(res.headers.get('X-Meridian-Bbox'), null);
  assert.equal(res.headers.get('X-Meridian-Bbox-Missing'), null);
  assert.equal(res.headers.get('X-Meridian-Bbox-Invalid'), null);
...
function assertBboxDiagnostic(
  ...
  assert.equal(res.headers.get('X-Meridian-Bbox'), status);
  assert.equal(res.headers.get('X-Meridian-Bbox-Missing'), options.missing ?? null);
  assert.equal(res.headers.get('X-Meridian-Bbox-Invalid'), options.invalid ?? null);
  const exposedHeaders = res.headers.get('Access-Control-Expose-Headers') ?? '';
  assert.match(exposedHeaders, /X-Meridian-Bbox/);
  assert.match(exposedHeaders, /X-Meridian-Bbox-Missing/);
  assert.match(exposedHeaders, /X-Meridian-Bbox-Invalid/);
