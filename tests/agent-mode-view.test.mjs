...
describe('agent-mode view (/?mode=agent)', () => {
  it('agent-view.json carries the machine-readable essentials', () => {
    ...
    assert.ok(view.authentication.apiKey.header === 'X-Meridian-Key');
  ...
  it('advertises the sandbox, quickstart, and docs MCP endpoints', () => {
    assert.equal(view.endpoints.sandbox.url, 'https://www.meridian.app/sandbox/index.json');
    assert.doesNotThrow(
      () => readFileSync(join(ROOT, 'public/sandbox/index.json')),
      'sandbox index advertised but public/sandbox/index.json is missing',
    );
    assert.equal(view.endpoints.docsMcp.url, 'https://www.meridian.app/docs/mcp');
  ...
  it('advertises every official SDK ecosystem with an install command', () => {
    ...
    assert.match(sdks.go.install, /^go get github\.com\/koala73\/meridian\/sdk\/go$/);
  ...
  it('the marketing homepage points at the agent view via link rel=alternate', () => {
    ...
    const linkTag =
      '<link rel="alternate" type="application/json" href="https://www.meridian.app/?mode=agent"';
  ...
  it('advertises the schemamap and every section llms.txt', () => {
    assert.equal(view.discovery.schemamap, 'https://www.meridian.app/schemamap.xml');
    ...
    assert.equal(sections.docs, 'https://www.meridian.app/docs/llms.txt');
  });
  ...
  it('stays in parity with the MCP server card and A2A agent card', () => {
    ...
    assert.equal(view.endpoints.nlweb.url, 'https://www.meridian.app/ask');
  });
  ...
  it('every discovery URL it advertises resolves to a tracked file or a live rewrite', () => {
    ...
    const trackedPaths = {
      'https://meridian.app/.well-known/agent-skills/index.json':
        'public/.well-known/agent-skills/index.json',
      'https://meridian.app/.well-known/api-catalog': 'public/.well-known/api-catalog',
      'https://meridian.app/.well-known/ai-catalog.json': 'public/.well-known/ai-catalog.json',
      'https://meridian.app/llms.txt': 'public/llms.txt',
    };
