...
describe('Giving published-estimate contract', () => {
  it('keeps existing field numbers and adds the exact provenance contract', () => {
    const source = readFileSync(resolve(root, 'proto/meridian/giving/v1/giving.proto'), 'utf8');
  ...
  it('uses one eligibility rule for platform and headline flow', async () => {
    ...
    } = await import('../server/meridian/giving/v1/published-estimates.ts');
    ...
    const implementation = [
      readFileSync(resolve(root, 'server/meridian/giving/v1/get-giving-summary.ts'), 'utf8'),
      readFileSync(resolve(root, 'server/meridian/giving/v1/published-estimates.ts'), 'utf8'),
    ].join('\n');
