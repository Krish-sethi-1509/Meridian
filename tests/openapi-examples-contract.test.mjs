...
const CURATED = (() => {
  ...
  const scenarioSrc = readFileSync(
    resolve(root, 'server/meridian/supply-chain/v1/scenario-templates.ts'),
    'utf8',
  );
  ...
  const consumerSrc = readFileSync(resolve(root, 'server/meridian/consumer-prices/v1/get-consumer-price-basket-series.ts'), 'utf8');
...
function assertSummarizeProviderExamples(spec, label) {
  ...
  for (const { value, where } of examples) {
    assert.notEqual(value, 'meridian', `${where}: meridian is not accepted by getProviderCredentials()`);
...
function assertScenarioStatusPollExample(spec, label) {
  ...
  if (example.status === 'done') {
    ...
    assert.notEqual(example.result.template.name, 'Meridian Analyst', `${label}: worker writes a template key, not a display name`);
...
describe('OpenAPI examples contract', () => {
  ...
  it('audits the known service operation surface', () => {
    ...
    assert.equal(total, 204, `expected 204 OpenAPI operations, found ${total}`);
  });
  ...
  it('uses a real accepted baseline type in the record-baseline-snapshot sample', () => {
    ...
    const specs = [
      ...
      ['meridian.openapi.yaml', loadYaml(readFileSync(resolve(apiDir, 'meridian.openapi.yaml'), 'utf8'))],
    ];
  ...
  it('adds request and response examples to the unified OpenAPI bundle', () => {
    ...
    const result = assertOperationExamples(bundle, 'meridian.openapi.yaml');
    assert.equal(result.operations, 204);
    assert.equal(result.responseExpected, 204);
  });
  ...
  it('omits honeypot fields from every request example', () => {
    ...
    const bundle = loadYaml(readFileSync(resolve(apiDir, 'meridian.openapi.yaml'), 'utf8'));
    violations.push(...honeypotRequestViolations(bundle, 'meridian.openapi.yaml'));
...
describe('OpenAPI curated example values', () => {
  ...
  it('keeps prose-enumerated parameter examples inside their documented closed sets', () => {
    ...
    const bundle = loadYaml(readFileSync(resolve(apiDir, 'meridian.openapi.yaml'), 'utf8'));
    checked += assertClosedValueParamExamples(bundle, 'meridian.openapi.yaml');
