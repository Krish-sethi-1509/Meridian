...
describe('CSP violation filter (shouldSuppressCspViolation)', () => {
  ...
  describe('default-src HTTP mixed-content suppression — WORLDMONITOR-S0', () => {
    ...
    it('does NOT suppress http default-src block to our own host (real mixed-content regression)', () => {
      assert.ok(!suppress('enforce', 'default-src', 'http://www.meridian.app/asset.json', '', false));
      assert.ok(!suppress('enforce', 'default-src', 'http://meridian.app/asset.json', '', false));
    });
    ...
    it('does NOT let a meridian.app suffix-spoof lookalike bypass the first-party gate', () => {
      // meridian.app.evil.com is third-party → http block IS suppressed (it is not us).
      assert.ok(suppress('enforce', 'default-src', 'http://meridian.app.evil.com/x', '', false));
    });
  });
  ...
  describe('scheme-only and special values', () => {
    ...
    it('suppresses blob: URI', () => {
      assert.ok(suppress('enforce', 'worker-src', 'blob:https://www.meridian.app/abc', '', false));
    });
  ...
  describe('third-party noise', () => {
    ...
    it('suppresses manifest.webmanifest', () => {
      assert.ok(suppress('enforce', 'default-src', 'https://www.meridian.app/manifest.webmanifest', '', false));
    });
    ...
    it('suppresses third-party stylesheet injection from cdn.jsdelivr.net (style-src-elem)', () => {
      ...
      // finance.meridian.app — 270 events / 26 users. We never load
  ...
  describe('first-party infrastructure (mutated user CSP)', () => {
    ...
    it('suppresses img-src to apex meridian.app (favicon)', () => {
      assert.ok(suppress('enforce', 'img-src', 'https://meridian.app/favico/favicon-32x32.png', '', false, FIRST_PARTY_CONVEX));
    });

    it('suppresses img-src to www.meridian.app (production favicon, WORLDMONITOR-JP)', () => {
      assert.ok(suppress('enforce', 'img-src', 'https://www.meridian.app/favico/favicon-32x32.png', '', false, FIRST_PARTY_CONVEX));
    });

    it('suppresses img-src to finance.meridian.app subdomain', () => {
      assert.ok(suppress('enforce', 'img-src', 'https://finance.meridian.app/favico/finance/apple-touch-icon.png', '', false, FIRST_PARTY_CONVEX));
    });

    it('suppresses img-src to tech.meridian.app subdomain', () => {
      assert.ok(suppress('enforce', 'img-src', 'https://tech.meridian.app/favico/tech/favicon-32x32.png', '', false, FIRST_PARTY_CONVEX));
    });
    ...
    it('does NOT suppress img-src to suffix-spoof lookalike `meridian.app.evil.com`', () => {
      // Endswith check uses a leading `.` so attacker-controlled lookalikes
      // (`meridian.app.evil.com`, `not-meridian.app`) are not whitelisted.
      assert.ok(!suppress('enforce', 'img-src', 'https://meridian.app.evil.com/pixel.gif', '', false, FIRST_PARTY_CONVEX));
    });

    it('does NOT suppress img-src to prefix-spoof `not-meridian.app`', () => {
      assert.ok(!suppress('enforce', 'img-src', 'https://not-meridian.app/pixel.gif', '', false, FIRST_PARTY_CONVEX));
    });

    it('does NOT suppress connect-src to meridian.app (rule is scoped to img-src)', () => {
    ...
    it('does NOT suppress script-src to meridian.app (rule is scoped to img-src)', () => {
      ...
      assert.ok(!suppress('enforce', 'script-src', 'https://www.meridian.app/assets/main-abc.js', '', false, FIRST_PARTY_CONVEX));
    });
    ...
    it('does NOT suppress http:// img-src to our own apex (mixed-content regression must surface)', () => {
      assert.ok(!suppress('enforce', 'img-src', 'http://meridian.app/favico/favicon-32x32.png', '', false, FIRST_PARTY_CONVEX));
    });

    it('does NOT suppress http:// img-src to a meridian.app subdomain', () => {
      assert.ok(!suppress('enforce', 'img-src', 'http://www.meridian.app/favico/favicon-32x32.png', '', false, FIRST_PARTY_CONVEX));
    });

    it('does NOT suppress ws:// img-src to a meridian.app subdomain (only https: is whitelisted)', () => {
      ...
      assert.ok(!suppress('enforce', 'img-src', 'ws://www.meridian.app/socket', '', false, FIRST_PARTY_CONVEX));
    });
  });
