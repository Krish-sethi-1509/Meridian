 ...
 * now accepts any meridian.app apex/subdomain origin (foreign origins still
...
const postWithOrigin = (origin) => {
  ...
  return handler(new Request('https://www.meridian.app/oauth/authorize', {
    method: 'POST',
    headers,
...
describe('OAuth authorize — consent POST origin gate (P1)', () => {
  const firstPartyOrigins = [
    'https://meridian.app',          // apex — the scanned host, previously 403'd
    'https://www.meridian.app',      // canonical Vercel host
    'https://api.worldmonitor.app',      // the only host that worked before
    'https://tech.meridian.app',     // variant subdomain
  ...
  const foreignOrigins = [
    'https://evil.example',
    'https://meridian.app.evil.example', // suffix attack — must stay anchored
    'https://evilmeridian.app',          // prefix attack — no subdomain dot
    'http://meridian.app',               // non-https
    'https://meridian.app:8443',         // port smuggling
  ];
