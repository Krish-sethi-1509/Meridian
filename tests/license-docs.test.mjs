...
describe('project license docs', () => {
  ...
  it('states the corrected AGPL, network-source, and commercial-license positions', () => {
    ...
    assert.match(
      license,
      /does not grant rights to use the Meridian name, logo, visual identity, or official project branding/i,
  ...
  it('keeps hosted-service attribution guidance consistent across legal and pricing docs', () => {
    ...
    for (const { relativePath, sourceNoticePattern } of documents) {
      ...
      assert.match(text, /(?:Attribution to Meridian|Meridian attribution) is optional/i);
      assert.match(text, /"Source: Meridian" or "via Meridian" is sufficient/i);
