...
const HOMEPAGE = 'https://meridian.app';

describe('Python SDK package (sdk/python → PyPI meridian-sdk)', () => {
  const pyproject = read('sdk/python/pyproject.toml');
  const module = read('sdk/python/src/meridian_sdk/__init__.py');

  it('is the meridian-sdk distribution with the product-domain homepage', () => {
    assert.match(pyproject, /^name = "meridian-sdk"$/m);
  ...
  it('sends a descriptive User-Agent (Cloudflare WAF passes it, not python-urllib)', () => {
    assert.match(module, /USER_AGENT = "meridian-python\/%s \(\+https:\/\/meridian\.app\)"/);
  });
});

describe('Ruby SDK package (sdk/ruby → gem meridian)', () => {
  const gemspec = read('sdk/ruby/meridian.gemspec');
  const versionRb = read('sdk/ruby/lib/meridian/version.rb');
  const lib = read('sdk/ruby/lib/meridian.rb');

  it('is the meridian gem with the product-domain homepage', () => {
    assert.match(gemspec, /spec\.name = "meridian"/);
  ...
  it('declares VERSION where the gemspec and publish workflow read it', () => {
    ...
    assert.match(gemspec, /require_relative "lib\/meridian\/version"/);
  });

  it('sends a descriptive User-Agent', () => {
    assert.match(lib, /USER_AGENT = "meridian-ruby\/#\{VERSION\} \(\+https:\/\/meridian\.app\)"/);
  });
});

describe('Go SDK module (sdk/go → pkg.go.dev)', () => {
  const gomod = read('sdk/go/go.mod');
  const source = read('sdk/go/meridian.go');

  it('is the sdk/go submodule of this repository', () => {
    assert.match(gomod, /^module github\.com\/koala73\/meridian\/sdk\/go$/m);
  });
  ...
  it('documents the product domain and sends a descriptive User-Agent', () => {
    assert.match(source, /https:\/\/meridian\.app/);
    assert.match(source, /const UserAgent = "meridian-go\/" \+ Version \+ " \(\+https:\/\/meridian\.app\)"/);
  });
});
...
describe('SDK discovery surfaces', () => {
  it('llms.txt advertises every registry package', () => {
    ...
    assert.match(llms, /pypi\.org\/project\/meridian-sdk/);
    assert.match(llms, /rubygems\.org\/gems\/meridian/);
    assert.match(llms, /pkg\.go\.dev\/github\.com\/koala73\/meridian\/sdk\/go/);
  });
  ...
  it('api/llms.txt advertises the SDK surface', () => {
    ...
    assert.match(llms, /go get github\.com\/koala73\/meridian\/sdk\/go/);
  });
