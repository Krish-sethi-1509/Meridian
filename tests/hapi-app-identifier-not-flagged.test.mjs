...
// "meridian" (confirmed live -- see the HAPI_APP_IDENTIFIER comment in
...
// field back toward "meridian..." would silently reintroduce the exact
...
// is deliberately left containing "meridian" as the real contact address,
...
for (const relPath of ['shared/hapi-app-identifier.json', 'scripts/shared/hapi-app-identifier.json']) {
  test(`${relPath}: application field does not contain "meridian"`, () => {
    const cfg = loadConfig(relPath);
    assert.equal(typeof cfg.application, 'string');
    assert.doesNotMatch(
      cfg.application.toLowerCase(), /meridian/,
      `${relPath}'s application field ("${cfg.application}") contains "meridian" -- ` +
