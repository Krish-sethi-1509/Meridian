...
describe('resilience runtime manifest', () => {
  it('returns public formula, dataVersion, and ranking metadata without deploy internals', async () => {
    ...
    const request = new Request('https://meridian.app/api/resilience/v1/get-runtime-manifest');
  ...
  it('returns safe empty/zero metadata when Redis keys are absent', async () => {
    ...
    const response = await modules.getResilienceRuntimeManifest({
      request: new Request('https://meridian.app/api/resilience/v1/get-runtime-manifest'),
    } as never);
...
describe('resilience runtime manifest gateway auth', () => {
  it('allows no-key manifest access while score and ranking remain premium gated', async () => {
    ...
    const manifest = await gateway(new Request('https://meridian.app/api/resilience/v1/get-runtime-manifest?_debug=1'));
    ...
    const score = await gateway(new Request('https://meridian.app/api/resilience/v1/get-resilience-score?countryCode=US'));
    assert.equal(score.status, 401);

    const ranking = await gateway(new Request('https://meridian.app/api/resilience/v1/get-resilience-ranking'));
