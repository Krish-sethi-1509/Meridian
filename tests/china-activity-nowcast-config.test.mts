...
describe('China activity nowcast production registration (#5579)', () => {
  ...
  it('registers the generated public RPC and a bounded gateway cache tier', () => {
    const proto = read('proto/meridian/economic/v1/service.proto');
    const handler = read('server/meridian/economic/v1/handler.ts');
    const gateway = read('server/gateway.ts');
    const client = read('src/generated/client/meridian/economic/v1/service_client.ts');
    const server = read('src/generated/server/meridian/economic/v1/service_server.ts');
