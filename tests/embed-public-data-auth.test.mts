...
describe('embed public data auth', () => {
  ...
  it('lets anonymous iframe requests reach embed RPC handlers while nearby RPCs stay gated', async () => {
    ...
    for (const path of EMBED_PUBLIC_RPC_PATHS) {
      const res = await gateway(new Request(`https://meridian.app${path}`, {
        headers: { Origin: 'https://meridian.app' },
      }));
      assert.equal(res.status, 200, `${path} should not require wm-session in an embed iframe`);
    }

    const gated = await gateway(new Request('https://meridian.app/api/conflict/v1/list-ucdp-events', {
      headers: { Origin: 'https://meridian.app' },
    }));
  ...
  it('scopes anonymous bootstrap access to the weather embed key only', async () => {
    ...
    for (const url of [
      'https://meridian.app/api/bootstrap?keys=weatherAlerts&public=1',
      'https://meridian.app/api/bootstrap?keys=weatherAlerts',
    ]) {
      const publicReq = new Request(url, { headers: { Origin: 'https://meridian.app' } });
    ...
    const rejected = [
      'https://meridian.app/api/bootstrap',
      'https://meridian.app/api/bootstrap?tier=fast',
      'https://meridian.app/api/bootstrap?keys=weatherAlerts,marketQuotes',
      'https://meridian.app/api/bootstrap?keys=marketQuotes',
      'https://meridian.app/api/bootstrap?keys=weatherAlerts&debug=1',
      'https://meridian.app/api/bootstrap?keys=weatherAlerts&keys=marketQuotes',
      'https://meridian.app/api/bootstrap?keys=weatherAlerts&public=1&debug=1',
      'https://meridian.app/api/bootstrap?keys=weatherAlerts&public=0',
      'https://meridian.app/api/bootstrap?keys=marketQuotes&public=1',
    ];

    for (const url of rejected) {
      const req = new Request(url, { headers: { Origin: 'https://meridian.app' } });
