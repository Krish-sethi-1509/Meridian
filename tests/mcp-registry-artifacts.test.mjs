...
// (registry.modelcontextprotocol.io, namespace app.meridian):
...
//   ~/.config/meridian/mcp-registry-ed25519.pem).
describe('mcp registry publication artifacts', () => {
  ...
  it('server.json stays in the app.meridian namespace with the canonical remote', () => {
    assert.equal(serverJson.name, 'app.meridian/mcp');
    ...
    assert.equal(serverJson.websiteUrl, 'https://www.meridian.app');
  });
