...
describe('resilience seed-refresh auth', () => {
  it('uses timingSafeEqual for X-Meridian-Key and WORLDMONITOR_SEED_REFRESH_KEY comparisons', async () => {
    const headerGetter = String.raw`(?:ctx\.request\.headers|request\.headers)\.get\(\s*['"]X-Meridian-Key['"]\s*\)`;
