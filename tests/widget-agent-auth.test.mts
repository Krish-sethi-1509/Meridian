...
describe('widget-agent unified tester key auth', () => {
  ...
  it('accepts X-Meridian-Key and upgrades relay request to pro', async () => {
    const res = await handler(new Request('https://www.meridian.app/api/widget-agent', {
      method: 'POST',
      headers: {
        Origin: 'https://www.meridian.app',
        'Content-Type': 'application/json',
        'X-Meridian-Key': 'browser-test-key',
      },
    ...
    assert.equal(call.arguments[0], 'https://proxy.meridian.app/widget-agent');
    ...
    assert.equal(headers.get('X-Meridian-Key'), null);
  ...
  it('falls back to legacy tester keys when X-Meridian-Key is invalid', async () => {
    const res = await handler(new Request('https://www.meridian.app/api/widget-agent', {
      method: 'POST',
      headers: {
        Origin: 'https://www.meridian.app',
        'Content-Type': 'application/json',
        'X-Meridian-Key': 'wrong-key',
  ...
  it('rejects invalid X-Meridian-Key before relay fetch', async () => {
    const res = await handler(new Request('https://www.meridian.app/api/widget-agent', {
      method: 'POST',
      headers: {
        Origin: 'https://www.meridian.app',
  ...
  it('rejects prefix and length mismatches for browser and legacy tester keys', async () => {
    const browserPrefix = await handler(new Request('https://www.meridian.app/api/widget-agent', {
      method: 'POST',
      headers: {
        Origin: 'https://www.meridian.app',
        'Content-Type': 'application/json',
        'X-Meridian-Key': 'browser-test',
      },
      body: JSON.stringify({ prompt: 'Build a widget', mode: 'create', tier: 'pro' }),
    }));
    assert.equal(browserPrefix.status, 403);

    const proLonger = await handler(new Request('https://www.meridian.app/api/widget-agent', {
      method: 'POST',
      headers: {
        Origin: 'https://www.meridian.app',
        'Content-Type': 'application/json',
    ...
    const widgetLonger = await handler(new Request('https://www.meridian.app/api/widget-agent', {
      method: 'POST',
      headers: {
        Origin: 'https://www.meridian.app',
        'Content-Type': 'application/json',
