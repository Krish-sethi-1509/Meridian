...
describe('ignoreErrors filters', () => {
  it('suppresses Clerk SDK UI chunk load failure', () => {
    assert.ok(
      isIgnored('[clerk] failed to load https://clerk.meridian.app/npm/@clerk/ui@1/dist/ui.browser.js'),
...
describe('dynamic-module-import failures (stale chunk after deploy)', () => {
  ...
  const assetUrlImportErrors = [
    'Failed to fetch dynamically imported module: https://meridian.app/assets/panels-abc.js',
    'Failed to fetch dynamically imported module: https://www.meridian.app/assets/index-DSkSc57y.js',
    'error loading dynamically imported module: https://www.meridian.app/assets/Map-eKJvyIxN.js',
    'error loading dynamically imported module: https://www.meridian.app/assets/hls-jw_vZdHi.js',
  ];
  ...
  it('lets through off-origin /assets dynamic-import failures even with first-party stack', () => {
    ...
    assert.ok(beforeSend(event) !== null, 'off-origin asset URL must not be treated as Meridian deploy skew');
  });

  it('lets through non-hashed /assets dynamic-import failures even on owned origins', () => {
    const event = makeEvent(
      'Failed to fetch dynamically imported module: https://meridian.app/assets/runtime.js',
      'TypeError',
      [firstPartyFrame()],
    );
...
describe('existing beforeSend filters', () => {
  ...
  it('suppresses Clerk SDK "Failed to fetch (clerk.meridian.app)" even with a clerk first-party frame', () => {
    // WORLDMONITOR-SA/SB: the bundled Clerk SDK fetches its Frontend API
    // (clerk.meridian.app, a CNAME to Clerk's auth infra) for token
    // refresh and retries transient failures itself. A leaked
    // `Failed to fetch (clerk.meridian.app)` is a Clerk-SDK-internal
    ...
    const event = makeEvent('Failed to fetch (clerk.meridian.app)', 'TypeError', [
  ...
  it('does NOT suppress MapLibre AJAXError for a non-allowlisted host (mixed stack)', () => {
    ...
    const event = makeEvent('Failed to fetch (pmtiles.meridian.app)', 'TypeError', [
      { filename: '/assets/maplibre-A8Ca0ysS.js', lineno: 4, function: 'ajaxFetch' },
      { filename: '/assets/panels-wF5GXf0N.js', lineno: 24, function: 'window.fetch' },
    ]);
  ...
  it('suppresses "Failed to fetch (<host>)" when stack is extension-only (covered by generic extension rule)', () => {
    ...
    const event = makeEvent('Failed to fetch (abacus.meridian.app)', 'TypeError', [
...
// ─── WORLDMONITOR-WH/WJ: `Failed to fetch (abacus.meridian.app)` ──────────
//
// abacus.meridian.app is our SELF-HOSTED Umami analytics collector
// (src/services/analytics.ts → `https://abacus.meridian.app/script.js`, which
...
describe('`Failed to fetch (abacus.meridian.app)` — Umami beacon (WORLDMONITOR-WH/WJ)', () => {
  ...
  it('suppresses the exact WH stack (Umami beacon through an extension fetch wrapper)', () => {
    assert.equal(beforeSend(makeEvent('Failed to fetch (abacus.meridian.app)', 'TypeError', whStack)), null,
      'a dropped Umami analytics beacon is unactionable');
  });

  it('suppresses the Firefox host-suffixed phrasing for the same host', () => {
    const event = makeEvent('NetworkError when attempting to fetch resource. (abacus.meridian.app)', 'TypeError', []);
