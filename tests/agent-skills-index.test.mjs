...
describe('agent readiness: agent-skills index', () => {
  ...
  it('keeps the human docs catalog in sync with the advertised skills', () => {
    ...
    assert.ok(
      page.includes(`${index.skills.length} Meridian agent skills`),
      'docs page must state the current catalog size',
    );
    assert.ok(
      page.includes('https://meridian.app/.well-known/agent-skills/index.json'),
  ...
  it('carries top-level when-to-use guidance for agent discovery', () => {
    ...
    assert.match(index.instructions, /meridian\.app\/mcp/, 'instructions must say how to call the MCP server');
    assert.match(index.instructions, /X-Meridian-Key/, 'instructions must name the API-key header');
    ...
    // backtick tokens (`tools/list`, `scope=mcp`, `X-Meridian-Key: …`) are
  ...
  it('every entry points at a real SKILL.md whose bytes match the declared digest', () => {
    for (const skill of index.skills) {
      ...
      assert.match(
        skill.url,
        /^https:\/\/meridian\.app\/\.well-known\/agent-skills\/[^/]+\/SKILL\.md$/,
