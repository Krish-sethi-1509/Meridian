...
describe('Umami runtime remediation (#6024)', () => {
  it('builds from the exact v3.2.0 release plus only the upstream session-data fix', () => {
    ...
    assert.match(dockerfile, new RegExp(`meridian\\.umami\\.fix-commit=${UMAMI_FIX_COMMIT}`));
  ...
  it('runs the real migration and concurrent upserts in a path-filtered PostgreSQL CI job', () => {
    ...
    assert.match(umamiJob, /docker build --file Dockerfile\.umami --tag meridian\/umami:ci \./);
    assert.match(
      umamiJob,
      /docker build --file Dockerfile\.umami-retention --tag meridian\/umami-retention:ci \./,
    );
