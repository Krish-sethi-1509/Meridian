 ...
 * the hardcoded `Documents/GitHub/meridian/.env.local` the old candidate
 * list reached for.
 */
function makeFixtureCheckout({ withLocalEnvFile, nested = false, only = null }) {
  ...
  const escapedDir = join(fakeHome, 'Documents/GitHub/meridian');
...
describe('seeder env hermeticity (#5767)', () => {
  ...
  describe('loadEnvFile() against a fixture checkout', () => {
    ...
    it('never reaches into $HOME/Documents/GitHub/meridian for credentials', () => {
  ...
  describe('no script resolves credentials from outside its checkout', () => {
    it('findCheckoutEscapingEnvPaths flags a home path baked into the repo', () => {
      assert.deepEqual(
        findCheckoutEscapingEnvPaths(`envPath = join('/Users/someone/Documents/GitHub/meridian', '.env.local');`),
        ['/Users/someone/Documents/GitHub/meridian'],
      );
      assert.deepEqual(
        findCheckoutEscapingEnvPaths(`const p = '/home/ci/meridian/.env.local';`),
        ['/home/ci/meridian/.env.local'],
      );
    ...
    it('findHomeDirReferences survives every rewrite of the #5767 escape', () => {
      ...
      const disguises = [
        `join(process.env.HOME, 'Documents/GitHub/meridian', '.env.local')`,
        `join(process.env['HOME'], 'Documents/GitHub/meridian', '.env.local')`,
        `import { homedir as getHome } from 'node:os';\nconst p = join(getHome(), 'wm/.env.local');`,
        'const p = `${process.env.HOME}/Documents/GitHub/meridian/.env.local`;',
        `const p = process.env.HOME + '/Documents/GitHub/meridian/.env.local';`,
        `const p = join(process.env.USERPROFILE, 'wm', '.env.local');`,
        `const p = '~/Documents/GitHub/meridian/.env.local';`,
      ];
