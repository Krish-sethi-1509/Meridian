...
describe('widget-agent relay — security', () => {
  ...
  it('CORS reuses getCorsOrigin (not a narrow hardcoded origin list)', () => {
    ...
    assert.ok(
      !corsBlock.includes("['https://meridian.app'"),
...
describe('widget-store — constants and logic', () => {
  ...
  it('deleteWidget cleans meridian-panel-spans (aggregate map)', () => {
    assert.ok(
  ...
  it('deleteWidget cleans meridian-panel-col-spans (aggregate map)', () => {
    assert.ok(
...
describe('proxy routing — widgetAgentUrl', () => {
  ...
  it('widgetAgentUrl targets proxy.meridian.app (not toRuntimeUrl)', () => {
    // The URL may be in a constant above the function; search the whole file
    assert.ok(
      proxy.includes('proxy.meridian.app'),
      'Must target proxy.meridian.app directly (sidecar destroys SSE via arrayBuffer)',
    );
  ...
  it('vite.config.ts proxies /widget-agent to proxy.meridian.app', () => {
    assert.ok(
      vite.includes('/widget-agent'),
      'vite.config.ts must have proxy entry for /widget-agent',
    );
    assert.ok(
      vite.includes('proxy.meridian.app'),
      'Vite proxy target must be proxy.meridian.app',
    );
  });
...
describe('PRO widget — store and sanitizer', () => {
  ...
  it('widget sandbox allows approved Vercel previews and rejects lookalike origins', () => {
    assert.ok(
      sandbox.includes("url.hostname === 'meridian.app'")
        && sandbox.includes("url.hostname.endsWith('.meridian.app')"),
      'sandbox must parse hostname and allow the meridian.app apex/subdomains only',
    );
    assert.ok(
      !sandbox.includes("endsWith('meridian.app')") && !sandbox.includes('endsWith("meridian.app")'),
      'sandbox must not use raw suffix checks that allow evilmeridian.app',
    );
    ...
    const matchesAllowedTeam = (hostname) =>
      slugs.some((team) =>
        new RegExp('^meridian-[a-z0-9-]+-' + team + '\\.vercel\\.app$').test(hostname),
      );
    assert.equal(matchesAllowedTeam('meridian-git-feature-eliewm.vercel.app'), true);
    assert.equal(matchesAllowedTeam('meridian-abc123-eliewm.vercel.app'), true);
    assert.equal(matchesAllowedTeam('meridian-feature-attacker.vercel.app'), false);
    assert.equal(matchesAllowedTeam('meridian-git-feature-eliewm.vercel.app.evil.com'), false);
    assert.equal(matchesAllowedTeam('meridian-feature-xeliewm.vercel.app'), false);
    assert.equal(matchesAllowedTeam('evilmeridian.app'), false);
    // The retired personal scope (meridian-*-elie-<hash>) must no longer match.
    assert.equal(matchesAllowedTeam('meridian-feature-elie-abc123.vercel.app'), false);
    ...
    assert.equal(matchesWithTeammate('meridian-feature-kieran.vercel.app'), true);
    assert.equal(matchesWithTeammate('meridian-feature-kieranfake.vercel.app'), false);
  });
  ...
  it('widget sandbox behavior accepts Vercel previews and blocks spoofed parents', () => {
    ...
    const allowed = runSandbox('https://meridian-git-feature-eliewm.vercel.app/dashboard');
    assert.equal(allowed.readyMessages.length, 1);
    assert.equal(allowed.readyMessages[0].targetOrigin, 'https://meridian-git-feature-eliewm.vercel.app');
    ...
    allowed.message({
      ...
      origin: 'https://meridian-git-feature-eliewm.vercel.app',
    ...
    for (const invalidMessage of invalidMessages) {
      const rejected = runSandbox('https://meridian.app/dashboard');
      rejected.message({ ...invalidMessage, source: rejected.parent });
      assert.deepEqual(rejected.writes, []);
    }

    const wrongSource = runSandbox('https://meridian.app/dashboard');
    wrongSource.message({
    ...
    const spoofed = runSandbox('https://meridian-git-feature-eliewm.vercel.app.evil.com/');
    assert.deepEqual(spoofed.readyMessages, []);
    spoofed.message({
      data: { type: 'wm-html', id: 'wm-1', token: 'test-token', html: '<p>bad</p>' },
      origin: 'https://meridian-git-feature-eliewm.vercel.app.evil.com',
  ...
  it('widget sandbox blocks beforeunload without breaking normal events before widget execution', () => {
    ...
    let vmContext;
    const document = {
      referrer: 'https://meridian.app/dashboard',
      open() {
