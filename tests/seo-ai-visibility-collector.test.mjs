...
const bingPerformance = ({
  ...
  firstCitedUrl = 'https://www.meridian.app/docs/api-reference',
} = {}) => ({
  status: 'available',
  windows: [
    {
...
describe('SEO/AI visibility collector', () => {
      ...
      windows: [
        {
          ...
          citedPages: [
            { url: 'https://www.meridian.app/docs/api-reference', citationCount: 4 },
          ],
        },
        {
          label: '90d',
  ...
  it('rejects cited URL credentials and stores canonical credential-free URLs', () => {
    const canonical = normalizeBingAiPerformance(
      bingPerformance({ firstCitedUrl: 'https://WWW.meridian.app/docs/api-reference?view=1' }),
      { observedAt },
    );
    assert.equal(
      canonical.windows[0].citedPages[0].url,
      'https://www.meridian.app/docs/api-reference?view=1',
    );

    assert.throws(
      () => normalizeBingAiPerformance(
        bingPerformance({ firstCitedUrl: 'https://operator:secret@www.meridian.app/docs/api-reference' }),
        { observedAt },
      ),
