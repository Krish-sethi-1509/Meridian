...
function post(body, headers = {}) {
  return new Request('https://www.meridian.app/api/docs-mcp', {
    method: 'POST',
...
describe('docs-mcp handler', () => {
  ...
  it('lifts the upstream unknown-tool SSE envelope into a top-level JSON-RPC error', async () => {
    ...
    assert.equal(calls[0].url, 'https://meridian.mintlify.dev/docs/mcp');
  ...
  it('answers OPTIONS preflight locally with permissive CORS', async () => {
    const res = await handler(
      new Request('https://www.meridian.app/api/docs-mcp', { method: 'OPTIONS' }),
    );
