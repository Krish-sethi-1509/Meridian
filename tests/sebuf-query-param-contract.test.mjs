...
describe('sebuf query-param implementation contract', () => {
  ...
  it('documents the military flights cursor as implemented rather than a no-op', () => {
    ...
    assert.doesNotMatch(
      contractSource,
      /meridian\/military\/v1\/list_military_flights\.proto:cursor/,
  ...
  it('flags unannotated query params that handlers do not reference', () => {
    ...
    const { violations } = collectQueryParamContractViolations(root, { scopedProtoFiles: new Set(['meridian/demo/v1/list_things.proto']), forcedNoopQueryParams: new Set() });
  ...
  it('allows documented no-op query params marked with the proto field option', () => {
    ...
    const { violations, stats } = collectQueryParamContractViolations(root, { scopedProtoFiles: new Set(['meridian/demo/v1/list_things.proto']), forcedNoopQueryParams: new Set() });
    assert.deepEqual(violations, []);
  ...
  it('flags forced no-op registry entries missing the proto field option', () => {
    ...
    const { violations } = collectQueryParamContractViolations(root, {
      scopedProtoFiles: new Set(['meridian/demo/v1/list_things.proto']),
      forcedNoopQueryParams: new Set(['meridian/demo/v1/list_things.proto:cursor']),
    });
    assert.equal(violations.length, 1);
