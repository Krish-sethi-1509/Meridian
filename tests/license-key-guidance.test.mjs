...
test('desktop settings give blocked users an exact help path', () => {
  ...
  assert.match(fullCopy.register.description, /meridian\.app\/docs\/api-keys/);
...
test('every locale describes the launched API-key flow instead of a waitlist', () => {
  ...
  for (const file of localeFiles) {
    ...
    assert.match(copy.register.description, /meridian\.app\/docs\/api-keys/, `${file}: help copy must link the guide`);
