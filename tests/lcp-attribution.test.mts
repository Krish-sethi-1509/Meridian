...
describe('lcp attribution helpers', () => {
  ...
  it('does not throw when browser storage getters are blocked', () => {
    ...
    Object.defineProperty(globalThis, 'window', {
      configurable: true,
      value: {
        location: { href: 'https://meridian.app/dashboard' },
  ...
  it('redacts query strings from resource URLs', () => {
    ...
    assert.equal(
      __testing__.sanitizeResourceUrl('https://meridian.app/data/countries-110m.json'),
      'https://meridian.app/data/countries-110m.json',
    );
  });
  ...
  it('keeps raw LCP text capture opt-in behind a separate flag', () => {
    ...
    Object.defineProperty(globalThis, 'window', {
      configurable: true,
      value: {
        location: { href: 'https://meridian.app/dashboard?wm_lcp_debug=1' },
    ...
    Object.defineProperty(globalThis, 'window', {
      configurable: true,
      value: {
        location: { href: 'https://meridian.app/dashboard?wm_lcp_text=1' },
        sessionStorage: { getItem: () => null },
        localStorage: { getItem: () => null },
      },
    });
