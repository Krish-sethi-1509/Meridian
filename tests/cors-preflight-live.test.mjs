...
//     (the 2026-05-27 outage — see meridian-architecture-gotchas/reference/
//      cloudflare-worker-overrides-vercel-cors-for-preflight.md).
//   - Origin echo broken (preflight echoes `https://meridian.app` for an
...
// OAuth/MCP), not the Worker's meridian.app-only echo.
...
for (const { url, origin } of PUBLIC_CORS_PROBES) {
  test(`OPTIONS ${url} from ${origin} bypasses Worker → Vercel ACAO survives`, { skip: !SHOULD_RUN }, async () => {
    ...
    // Acceptance: the response must NOT echo the canonical meridian.app
...
for (const url of ENDPOINTS) {
  test(`OPTIONS ${url} returns ACAC: true for ${ORIGIN}`, { skip: !SHOULD_RUN }, async () => {
    ...
    assert.equal(
      ...
      ORIGIN,
      'ACAO must echo the request origin (NOT https://meridian.app fallback, NOT *)',
    );
    assert.equal(
    ...
    for (const required of ['Authorization', 'X-Meridian-Key', 'X-Api-Key', 'X-Pro-Key', 'X-Widget-Key']) {
      assert.ok(
