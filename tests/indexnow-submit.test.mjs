...
describe('IndexNow submission', () => {
  it('registers the canonical apex MCP endpoint in its own same-host batch', () => {
    ...
    const batch = indexNow.INDEXNOW_BATCHES.find(({ host }) => host === 'meridian.app');
    assert.equal(batch.host, 'meridian.app');
    assert.equal(batch.keyLocation, `https://meridian.app/${batch.key}.txt`);
    assert.deepEqual(batch.urls, ['https://meridian.app/mcp']);
    const wwwBatch = indexNow.INDEXNOW_BATCHES.find(({ host }) => host === 'www.meridian.app');
    assert.ok(
      wwwBatch.urls.includes('https://www.meridian.app/blog/authors/elie-habib/'),
  ...
  it('keeps IndexNow coverage aligned with the committed root sitemap and blog corpus', () => {
    ...
    const wwwSitemapUrls = sitemapUrls.filter((url) => new URL(url).hostname === 'www.meridian.app');
    const apexSitemapUrls = sitemapUrls.filter((url) => new URL(url).hostname === 'meridian.app');
    const wwwBatch = indexNow.INDEXNOW_BATCHES.find(({ host }) => host === 'www.meridian.app');
    const apexBatch = indexNow.INDEXNOW_BATCHES.find(({ host }) => host === 'meridian.app');
    ...
    for (const url of [
      ...
      'https://www.meridian.app/blog/glossary/ais/',
      'https://www.meridian.app/blog/authors/elie-habib/',
    ]) {
  ...
  it('requires a direct key response with the exact key body', async () => {
    ...
    const fetchImpl = async (url, init) => {
      ...
      return new Response(null, {
        status: 301,
        headers: { location: 'https://www.meridian.app/key.txt' },
      });
    };

    await assert.rejects(
      indexNow.verifyIndexNowKey(indexNow.INDEXNOW_BATCHES.find(({ host }) => host === 'meridian.app'), {
        fetchImpl,
      }),
      /direct 200/i,
    );
  ...
  it('submits the apex-specific key and canonical URL after ownership verification', async () => {
    ...
    assert.deepEqual(JSON.parse(requests[1].init.body), {
      ...
      urlList: ['https://meridian.app/mcp'],
    });
  });
  ...
  it('submits each host batch only after a relevant successful production deployment', () => {
    ...
    assert.match(workflow, /node scripts\/seo-indexnow-submit\.mjs --host meridian\.app/);
    assert.match(workflow, /node scripts\/seo-indexnow-submit\.mjs --host www\.meridian\.app/);
  });
});
