...
const TEST_RESOLVER_KEY = Symbol.for('meridian.shippingV2.resolveWebhookHostnameForTest');
...
function proCtx() {
  return makeCtx({ 'X-Meridian-Key': 'pro-test-key' });
}
...
describe('ShippingV2Service handlers', () => {
  ...
  describe('registerWebhook', () => {
    ...
    it('rejects callers without an API key with 401 (tenant-isolation gate)', async () => {
      // Without this gate, Clerk-authenticated pro callers with no X-Meridian-Key
  ...
  describe('deliverWebhook', () => {
    ...
    it('sends only after a public delivery-time DNS answer and includes delivery headers', async () => {
      ...
      assert.equal(seen.headers['user-agent'], 'Meridian-ShippingV2-Webhooks/1.0');
