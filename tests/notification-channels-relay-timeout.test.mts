...
describe('/api/notification-channels relay timeout recovery', () => {
  it('returns CORS-safe 500, releases idempotency, and processes the same-key retry', async () => {
    ...
    assert.equal(first.headers.get('Access-Control-Allow-Origin'), 'https://meridian.app');
    ...
    assert.equal((relayInit.headers as Record<string, string>)['User-Agent'], 'meridian-edge/1.0');
