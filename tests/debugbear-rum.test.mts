...
describe('DebugBear RUM loader', () => {
  it('enables only first-party production dashboard hosts', () => {
    assert.equal(shouldEnableDebugBearRum('www.meridian.app'), true);
    assert.equal(shouldEnableDebugBearRum('happy.meridian.app'), true);
    assert.equal(shouldEnableDebugBearRum('localhost'), false);
    assert.equal(shouldEnableDebugBearRum('meridian-git-codex-preview-eliewm.vercel.app'), false);
    assert.equal(shouldEnableDebugBearRum('evilmeridian.app'), false);
  });

  it('installs DebugBear RUM with presampling and pre-script error buffering', () => {
    const h = installDebugBearHarness('www.meridian.app');
    try {
      initDebugBearRum();
  ...
  it('does not append a duplicate script when one already exists', () => {
    ...
    const h = installDebugBearHarness('meridian.app', existing);
    try {
      initDebugBearRum();
  ...
  it('keeps the RUM sample rate at 10% and skips out-of-sample loads', () => {
    ...
    const h = installDebugBearHarness('meridian.app', null, () => 0.1);
    try {
      initDebugBearRum();
...
describe('DebugBear RUM marketing loader', () => {
  ...
  it('uses the same production-host gate as the dashboard loader', () => {
    for (const host of [
      ...
      'www.meridian.app',
      'tech.meridian.app',
      'finance.meridian.app',
      'commodity.meridian.app',
      'happy.meridian.app',
      'energy.meridian.app',
      'localhost',
      'meridian-git-codex-preview-eliewm.vercel.app',
      'evilmeridian.app',
    ]) {
      assert.equal(
