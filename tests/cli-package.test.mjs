...
// Guards the published `meridian` CLI package (orank Access "CLI tool
...
describe('meridian CLI package wiring', () => {
  it('is the public unscoped `meridian` npm package', () => {
    assert.equal(pkg.name, 'meridian');
    assert.equal(pkg.type, 'module');
    assert.equal(pkg.publishConfig?.access, 'public');
    assert.match(pkg.license, /^MIT$/);
  });

  it('exposes the meridian bin pointing at the shipped entry', () => {
    assert.equal(pkg.bin?.meridian, 'bin/meridian.mjs');
    const bin = readFileSync(join(CLI_DIR, pkg.bin.meridian), 'utf-8');
...
describe('meridian CLI request construction', () => {
  ...
  it('always sends the CLI User-Agent (Cloudflare WAF passes it, not `node`)', () => {
    ...
    assert.match(USER_AGENT, /^meridian-cli\//);
  });
...
describe('meridian CLI run() with a mocked transport', () => {
