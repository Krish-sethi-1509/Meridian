...
<div style="font-family: -apple-system, BlinkMacSystemFont, 'Segoe UI', sans-serif; max-width: 600px; margin: 0 auto; background: #0a0a0a; color: #e0e0e0;">
  ...
  <div style="padding: 40px 32px;">
    ...
    <p style="font-size: 14px; color: #999; line-height: 1.5; margin: 0 0 24px;">
      Your Meridian Pro seat on a team Business plan has ended because the plan is no longer active.
      Your account has returned to the free tier.
    </p>
    <div style="text-align: center;">
      <a href="https://meridian.app/pro" style="display: inline-block; background: #ef4444; color: #fff; padding: 14px 36px; text-decoration: none; font-weight: 800; font-size: 13px; text-transform: uppercase; letter-spacing: 1.5px; border-radius: 2px;">View Plans</a>
    </div>
    <p style="font-size: 11px; color: #666; text-align: center; margin: 24px 0 0;">
      Questions? Reply to this email or contact support@meridian.app.
    </p>
  </div>
</div>`;
    ...
    await sendResendEmail({
      ...
      subject: "Your Meridian team access has ended",
      html,
...
export const sendBusinessInviteEmail = internalAction({
  ...
  handler: async (_ctx, args) => {
    const acceptUrl = `https://meridian.app/settings?accept-business-invite=${encodeURIComponent(args.grantId)}&token=${encodeURIComponent(args.token)}`;
    const html = `
<div style="font-family: -apple-system, BlinkMacSystemFont, 'Segoe UI', sans-serif; max-width: 600px; margin: 0 auto; background: #0a0a0a; color: #e0e0e0;">
  <div style="background: #4ade80; height: 4px;"></div>
  <div style="padding: 40px 32px;">
    <p style="font-size: 22px; font-weight: 700; color: #fff; margin: 0 0 12px;">You're invited to Meridian Pro</p>
    <p style="font-size: 14px; color: #999; line-height: 1.5; margin: 0 0 24px;">
      ${escapeHtml(args.ownerEmail)} has invited you to a Pro seat on their Meridian API Business plan.
    ...
    await sendResendEmail({
      to: args.inviteeEmail,
      subject: "You've been invited to Meridian Pro",
      html,
