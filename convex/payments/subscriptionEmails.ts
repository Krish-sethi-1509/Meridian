    ...
    <table cellpadding="0" cellspacing="0" border="0" style="margin: 0 auto 32px;">
      <tr>
        <td style="width: 40px; height: 40px; vertical-align: middle;">
          <img src="https://www.meridian.app/favico/android-chrome-192x192.png" width="40" height="40" alt="Meridian" style="border-radius: 50%; display: block;" />
        </td>
...
export const sendReactivationEmail = internalAction({
  ...
  handler: async (_ctx, args) => {
    ...
    await sendEmail(
      apiKey,
      args.userEmail,
      `Welcome back to Meridian ${planName}`,
      `<div style="font-family: -apple-system, BlinkMacSystemFont, 'Segoe UI', sans-serif; max-width: 600px; margin: 0 auto; background: #0a0a0a; color: #e0e0e0;">
        <div style="background: #4ade80; height: 4px;"></div>
        <div style="padding: 40px 32px;">
          ...
          <div style="text-align: center;">
            <a href="https://meridian.app/dashboard" style="display: inline-block; background: #4ade80; color: #0a0a0a; padding: 14px 36px; text-decoration: none; font-weight: 800; font-size: 13px; text-transform: uppercase; letter-spacing: 1.5px; border-radius: 2px;">Open Meridian</a>
          </div>
...
const DASHBOARD_URL = "https://www.meridian.app/dashboard";
const PRICING_URL = "https://www.meridian.app/pro#pricing";
...
<div style="font-family: -apple-system, BlinkMacSystemFont, 'Segoe UI', sans-serif; max-width: 600px; margin: 0 auto; background: #0a0a0a; color: #e0e0e0;">
  ...
  <div style="border-top: 1px solid #1a1a1a; padding: 24px 32px; text-align: center;">
    <p style="font-size: 11px; color: #444; margin: 0; line-height: 1.6;">
      ${footerNote}<br />
      <a href="https://meridian.app" style="color: #f59e0b; text-decoration: none;">meridian.app</a>
    </p>
  </div>
</div>`;
}
...
export function buildDunningEmail(
  ...
  switch (step) {
    case "dunning_day0":
      return {
        subject: `Your Meridian payment failed — access continues while you fix it`,
        html: dunningEmailShell(
          ...
          ctaUrl,
          "You're receiving this because a payment on your Meridian subscription failed.",
        ),
      };
    case "dunning_day3":
      return {
    ...
    case "dunning_day7":
      return {
        subject: `Final notice: your Meridian ${planName} subscription is paused`,
        html: dunningEmailShell(
    ...
    case "winback_day30":
      return {
        subject: `Your Meridian ${planName} access has ended — rejoin in one click`,
        html: dunningEmailShell(
