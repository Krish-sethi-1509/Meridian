...
const STUCK_PAYMENT_EMAIL_FROM = "Meridian <noreply@meridian.app>";
const STUCK_PAYMENT_SUPPORT_EMAIL = "support@meridian.app";
...
async function sendStuckPaymentEmail(
  ...
  const planName = planKey ? PRODUCT_CATALOG[planKey]?.displayName ?? "Meridian" : "Meridian";
  ...
  const html = `
    <div style="font-family: system-ui, -apple-system, BlinkMacSystemFont, 'Segoe UI', sans-serif; color: #111; line-height: 1.5;">
      <h1 style="font-size: 20px;">Your Meridian checkout still needs action</h1>
  ...
  const res = await fetch(RESEND_EMAILS_URL, {
    ...
    body: JSON.stringify({
      ...
      to: [email],
      subject: "Complete your Meridian checkout",
      html,
