...
Agent entry point for Meridian. Read this first, then follow links for depth.
...
│   └── meridian/       # Domain handlers (mirrors proto service structure)
├── proto/                  # Protobuf definitions (sebuf framework)
│   ├── buf.yaml            # Buf configuration
│   └── meridian/       # Service definitions with HTTP annotations
...
2. Run `npm run worktree:bootstrap` before typecheck/tests. The helper links ignored `.env.local` / `.env` from the main worktree when Git can infer it, and installs deps with `npm ci --cache /tmp/meridian-npm-cache`.
...
- Override env source discovery with `WM_ENV_SOURCE=/path/to/meridian npm run worktree:env` when the main worktree cannot be inferred.
- `.env*` files are ignored local state. Do not add, print, or summarize secret values.

Validation hygiene:

- Prefer `npm ci` over `npm install` in fresh worktrees. Use `npm_config_cache=/tmp/meridian-npm-cache` for `npx` or install commands if cache ownership errors appear.
...
- `server/meridian/<domain>/` has RPC handlers matching proto services
