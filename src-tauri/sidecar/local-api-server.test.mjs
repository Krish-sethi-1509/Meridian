...
test('signs desktop register-interest cloud fallback when shared secret is configured', async () => {
  ...
  try {
    const response = await postJsonViaHttp(`http://127.0.0.1:${port}/api/register-interest`, {
      ...
      'X-Meridian-Desktop-Timestamp': '1',
      'X-Meridian-Desktop-Signature': 'sha256=bad',
    });
    assert.equal(response.status, 200);
    ...
    const timestamp = request.headers['x-meridian-desktop-timestamp'];
    const signature = request.headers['x-meridian-desktop-signature'];
    ...
    'header-check.js': `
      export default async function handler(req) {
        return new Response(JSON.stringify({
          ...
          transportToken: req.headers.get('x-meridian-local-token'),
        }), {
          status: 200,
          headers: { 'content-type': 'application/json' },
        });
  ...
  try {
    const response = await fetch(`http://127.0.0.1:${port}/api/header-check`, {
      headers: {
        ...
        'X-Meridian-Local-Token': TEST_LOCAL_API_TOKEN,
      },
    });
    assert.equal(response.status, 200);
...
test('does not forward the sidecar transport token through Docker cloud proxy routes', async () => {
  ...
  try {
    const headers = { 'X-Meridian-Local-Token': TEST_LOCAL_API_TOKEN };
    ...
    for (const upstreamHeaders of remote.headers) {
      assert.equal(upstreamHeaders['x-meridian-local-token'], undefined);
    }
  } finally {
    await app.close();
    await localApi.cleanup();
...
test('uses canonical app origin when proxying to cloud fallback (cloudFallback enabled)', async () => {
  ...
  try {
    ...
    const body = await response.json();
    assert.equal(body.source, 'remote');
    assert.equal(body.origin, 'https://meridian.app');
    assert.equal(remote.origins[0], 'https://meridian.app');
  } finally {
    await app.close();
    await localApi.cleanup();
    await remote.close();
