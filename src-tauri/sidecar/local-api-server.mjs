...
const DESKTOP_AUTH_TIMESTAMP_HEADER = 'X-Meridian-Desktop-Timestamp';
const DESKTOP_AUTH_SIGNATURE_HEADER = 'X-Meridian-Desktop-Signature';
const LOCAL_API_TRANSPORT_HEADER = 'x-meridian-local-token';
...
const ALLOW_PRIVATE_NETWORK_FETCH = Symbol('meridian.allowPrivateNetworkFetch');
...
async function proxyToCloud(requestUrl, req, remoteBase) {
  ...
  headers.set('Origin', 'https://meridian.app');
  ...
  // supplied X-Meridian-Key (e.g. a wm_ user key from runtime config) wins
  // — don't clobber it.
  if (!headers.has('X-Meridian-Key')) {
    const wmKey = process.env.WORLDMONITOR_API_KEY;
    if (wmKey) headers.set('X-Meridian-Key', wmKey);
  }
  return fetch(target, {
    method: req.method,
    headers,
...
const SIDECAR_ALLOWED_ORIGINS = [
  ...
  // Only allow exact domain or single-level subdomains (e.g. preview-xyz.meridian.app).
  // The previous (.*\.)? pattern was overly broad. Anchored to prevent spoofing
  // via domains like meridianEVIL.vercel.app.
  /^https:\/\/([a-z0-9-]+\.)?meridian\.app$/,
];
...
function makeCorsHeaders(req) {
  return {
    ...
    'Access-Control-Allow-Headers': 'Content-Type, Authorization, X-Meridian-Desktop-Timestamp, X-Meridian-Desktop-Signature',
...
async function dispatch(requestUrl, req, routes, context) {
  ...
  try {
    ...
    // (OAuth bearer) and X-Meridian-Key is the caller's API key.
