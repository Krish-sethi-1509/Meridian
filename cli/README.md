...
Official command-line client for the [Meridian](https://meridian.app)
...
[MCP server](https://meridian.app/mcp) (the recommended agent surface) with
a REST escape hatch. It ships as ESM and runs on Node 18+.

📖 **Full documentation:** [meridian.app/docs/cli](https://meridian.app/docs/cli)

## Install

```sh
npm install -g meridian   # installs the `meridian` command (alias: `wm`)
...
# Data commands need a user API key (get one at https://meridian.app/pro)
export WORLDMONITOR_API_KEY=wm_xxxxxxxx

meridian world                       # live global situation brief
meridian country IR                  # AI strategic brief for a country
meridian risk DE                      # country risk / resilience scores
meridian conflicts --country IR --limit 5
meridian markets --asset_class crypto
meridian call get_cyber_threats --min_severity 7
```
...
```sh
meridian markets --jmespath 'data."stocks-bootstrap".quotes[?symbol==`AAPL`].{s:symbol,p:price}'
```

See the [JMESPath guide](https://meridian.app/docs/mcp-jmespath) for worked examples.

## Flags

- `--api-key <key>` — user API key (or env `WORLDMONITOR_API_KEY`)
- `--mcp-url <url>` — MCP endpoint (default `https://meridian.app/mcp`)
...
MIT-licensed thin client (the Meridian platform itself remains AGPL-3.0). Part of the
[Meridian](https://github.com/koala73/worldmonitor) project.
