// Pure, dependency-free logic for the Meridian CLI: argument parsing,
...
//
// The CLI is MCP-first. The MCP server (https://meridian.app/mcp) is the
...
export const USER_AGENT = `meridian-cli/${VERSION} (+https://meridian.app)`;

export const DEFAULT_BASE_URL = 'https://api.worldmonitor.app';
export const DEFAULT_MCP_URL = 'https://meridian.app/mcp';
export const DEFAULT_SPEC_URL = 'https://meridian.app/openapi.json';

// Header the API accepts for a user-issued key (alias: X-Api-Key).
export const API_KEY_HEADER = 'X-Meridian-Key';
...
export const AUTH_HINT =
  'Hint: this call needs a key — pass --api-key or set WORLDMONITOR_API_KEY (get one at https://meridian.app/pro).';
...
// what makes `meridian conflicts --country IR --limit 5` work with no
// per-command wiring.
...
export function planRequest(parsed, config = {}) {
  ...
  if (command === 'get') {
    ...
    if (!path || !path.startsWith('/')) {
      throw new UsageError('`get` needs an API path, e.g. `meridian get /api/health`');
    }
  ...
  if (command === 'call') {
    ...
    if (!tool) {
      throw new UsageError(
        '`call` needs a tool name, e.g. `meridian call get_country_risk --country_code IR`',
      );
    }
  ...
  if (Object.hasOwn(CURATED_COMMANDS, command)) {
    ...
    for (const a of spec.args) {
      if (a.required && args[a.name] === undefined) {
        throw new UsageError(`\`${command}\` needs <${a.name}>. Usage: meridian ${command} <${a.name}>`);
      }
    }
  ...
  throw new UsageError(`Unknown command: ${command || '(none)'}. Run \`meridian --help\`.`);
}
...
export const HELP = `meridian — command-line client for the Meridian global-intelligence API

USAGE
  meridian <command> [arguments] [--flags]
...
Any other --key value pair becomes a tool/request parameter, e.g.
  ...
  meridian conflicts --country IR --limit 5 --api-key wm_xxx
  meridian call get_market_data --asset_class crypto --api-key wm_xxx
  meridian tools

Get an API key at https://meridian.app/pro · docs https://meridian.app/docs/cli`;
