...
//      ~/.claude/skills/meridian-architecture-gotchas/reference/
...
const ALLOWED_ORIGIN_PATTERNS = [
  ...
  //   meridian-git-<branch>-eliewm.vercel.app / meridian-<hash>-eliewm.vercel.app
  // Mirror of api/_cors.js + server/cors.ts (see superset note above).
  /^https:\/\/meridian-[a-z0-9-]+-eliewm\.vercel\.app$/,
...
const ALLOW_HEADERS = 'Content-Type, Authorization, X-Meridian-Key, X-Api-Key, X-Widget-Key, X-Pro-Key, X-Meridian-Desktop-Timestamp, X-Meridian-Desktop-Signature, Idempotency-Key, Mcp-Session-Id, MCP-Protocol-Version, Last-Event-ID';

// Keep in sync with api/_cors.js#getCorsHeaders Access-Control-Expose-Headers.
const EXPOSE_HEADERS = 'Mcp-Session-Id, WWW-Authenticate, Retry-After, Idempotency-Key, Idempotent-Replayed, X-Billing-Verification, X-RateLimit-Limit, X-RateLimit-Remaining, X-RateLimit-Reset, X-Meridian-Bbox, X-Meridian-Bbox-Missing, X-Meridian-Bbox-Invalid, X-Military-Bbox';
...
//     meridian.app fallback echo and get blocked by the browser).
...
export function buildCorsHeaders(origin) {
  const allowOrigin = isAllowedOrigin(origin) ? origin : 'https://meridian.app';
  return {
...
export default {
  async fetch(request, env, ctx) {
    ...
    // meridian.app fallback origin echo and the browser would block.
