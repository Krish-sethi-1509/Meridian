...
//   - Disallowed origins fall back to the canonical https://meridian.app
...
const CANONICAL_FALLBACK = 'https://meridian.app';
const KNOWN_GOOD = 'https://www.meridian.app';
const ACAH_EXPECTED = 'Content-Type, Authorization, X-Meridian-Key, X-Api-Key, X-Widget-Key, X-Pro-Key, X-Meridian-Desktop-Timestamp, X-Meridian-Desktop-Signature, Idempotency-Key, Mcp-Session-Id, MCP-Protocol-Version, Last-Event-ID';
const ACEH_EXPECTED = 'Mcp-Session-Id, WWW-Authenticate, Retry-After, Idempotency-Key, Idempotent-Replayed, X-Billing-Verification, X-RateLimit-Limit, X-RateLimit-Remaining, X-RateLimit-Reset, X-Meridian-Bbox, X-Meridian-Bbox-Missing, X-Meridian-Bbox-Invalid, X-Military-Bbox';
...
test('isAllowedOrigin accepts apex meridian.app and subdomains', () => {
  assert.equal(isAllowedOrigin('https://meridian.app'), true);
  assert.equal(isAllowedOrigin('https://www.meridian.app'), true);
  assert.equal(isAllowedOrigin('https://tech.meridian.app'), true);
  assert.equal(isAllowedOrigin('https://commodity.meridian.app'), true);
});
...
test('isAllowedOrigin accepts Vercel preview deploys under the eliewm team scope (mirrors api/_cors.js)', () => {
  ...
  // preview preflights echo the canonical meridian.app fallback and the
  // browser blocks them before the request ever reaches Vercel.
  assert.equal(isAllowedOrigin('https://meridian-git-feat-x-eliewm.vercel.app'), true);
  assert.equal(isAllowedOrigin('https://meridian-r6q9o-eliewm.vercel.app'), true);
  // Tight allowlist: a foreign team scope, a non-meridian app, and the
  // retired personal scope (meridian-*-elie-<hash>, migration complete)
  // must all stay rejected. Never a bare *.vercel.app.
  assert.equal(isAllowedOrigin('https://meridian-feat-x-attacker.vercel.app'), false);
  assert.equal(isAllowedOrigin('https://some-other-app-eliewm.vercel.app'), false);
  assert.equal(isAllowedOrigin('https://meridian-abc-elie-habib.vercel.app'), false);
});
...
test('isAllowedOrigin rejects unrelated origins', () => {
  ...
  assert.equal(isAllowedOrigin('https://meridian.app.evil.com'), false);
  assert.equal(isAllowedOrigin('https://notmeridian.app'), false);
...
test('GET response preserves function-specific exposed headers (bootstrap U3a regression)', async () => {
  ...
  globalThis.fetch = async () => new Response(JSON.stringify({ ok: true }), {
    ...
    headers: {
      ...
      'Access-Control-Expose-Headers': [
        'Server-Timing',
        'X-Meridian-Bootstrap-Redis-Duration',
        'Age',
        'X-Vercel-Cache',
        'CF-Cache-Status',
  ...
  try {
    ...
    assert.equal(
      ...
      `${ACEH_EXPECTED}, Server-Timing, X-Meridian-Bootstrap-Redis-Duration, Age, X-Vercel-Cache, CF-Cache-Status`,
    );
  } finally {
    globalThis.fetch = original;
  }
...
test('OPTIONS preflight to /api/mcp from https://claude.ai passes through to Vercel (Worker does NOT short-circuit)', async () => {
  ...
  // preflights with the canonical meridian.app fallback origin echo,
  ...
  globalThis.fetch = async (req) => {
    ...
    return new Response(null, {
      ...
      headers: {
        ...
        'Access-Control-Allow-Headers': 'Content-Type, Authorization, X-Meridian-Key',
      },
    });
  };
  try {
