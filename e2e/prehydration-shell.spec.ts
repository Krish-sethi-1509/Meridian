...
test.describe('pre-hydration dashboard shell', () => {
  ...
  test('paints contentful HTML before the dashboard bundle hydrates', async ({ page }) => {
    ...
    try {
      ...
      expect(preHydration.appHeadingText).toContain('Meridian');
      expect(preHydration.badgeAriaLabel).toBeNull();
      expect(preHydration.focusableCount).toBe(0);
      expect(preHydration.shellText).toContain('Meridian');
      ...
      await expect(page.locator('body > h1.app-heading')).toContainText('Meridian');
      for (const href of [
        '/countries/',
        '/chokepoints/',
        '/crises/',
