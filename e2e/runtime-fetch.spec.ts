...
test.describe('desktop runtime routing guardrails', () => {
    ...
    const result = await page.evaluate(async () => {
      ...
      return {
        ...
        webHost: runtime.detectDesktopRuntime({
          ...
          userAgent: 'Mozilla/5.0',
          locationProtocol: 'https:',
          locationHost: 'meridian.app',
          locationOrigin: 'https://meridian.app',
        }),
      };
    });
  ...
  test('runtime fetch patch falls back to cloud for local failures', async ({ page }) => {
    ...
    const result = await page.evaluate(async () => {
      ...
      window.fetch = (async (input: RequestInfo | URL) => {
        ...
        if (url.includes('meridian.app/api/fred-data')) {
          return responseJson({ observations: [{ value: '321.5' }] }, 200);
        }
        if (url.includes('meridian.app/api/stablecoin-markets')) {
    ...
    expect(result.calls.some((url) => url.includes('meridian.app/api/fred-data'))).toBe(true);
    expect(result.calls.some((url) => url.includes('meridian.app/api/stablecoin-markets'))).toBe(true);
  ...
  test('runtime fetch patch never sends blocked local-control endpoints to cloud', async ({ page }) => {
    ...
    expect(result.calls.some((url) => url.includes('meridian.app/api/local-env-update'))).toBe(false);
    expect(result.calls.some((url) => url.includes('meridian.app/api/local-validate-secret'))).toBe(false);
  });
  ...
  test('update badge picks architecture-correct desktop download url', async ({ page }) => {
    ...
    expect(result.macArm).toBe('https://meridian.app/api/download?platform=macos-arm64&variant=full');
    expect(result.windowsX64).toBe('https://meridian.app/api/download?platform=windows-exe&variant=full');
  ...
  test('cloud fallback blocked without Meridian API key', async ({ page }) => {
    await page.goto('/tests/runtime-harness.html');
    ...
    const result = await page.evaluate(async () => {
      ...
      try {
        ...
        const cloudCalls = calls.filter(u => u.includes('meridian.app'));
  ...
  test('cloud fallback allowed with valid Meridian API key', async ({ page }) => {
    await page.goto('/tests/runtime-harness.html');
    ...
    const result = await page.evaluate(async () => {
      ...
      window.fetch = (async (input: RequestInfo | URL, init?: RequestInit) => {
        ...
        if (url.includes('meridian.app') && init?.headers) {
          const h = new Headers(init.headers);
          const wmKey = h.get('X-Meridian-Key');
          if (wmKey) capturedHeaders['X-Meridian-Key'] = wmKey;
        }

        if (url.includes('meridian.app/api/market/v1/test')) {
      ...
      try {
        ...
        return {
          ...
          cloudCalls: calls.filter(u => u.includes('meridian.app')).length,
          wmKeyHeader: capturedHeaders['X-Meridian-Key'] || null,
          proxiedPaths,
        };
      } finally {
        window.fetch = originalFetch;
