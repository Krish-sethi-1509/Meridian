...
// www.meridian.app/dashboard session via Resource Timing:
//
...
async function readPersistedDigest(
  ...
  return page.evaluate(async (exactKey) => {
    const db = await new Promise<IDBDatabase>((resolve, reject) => {
      ...
      const request = indexedDB.open('meridian_persistent_cache');
...
async function seedFreshAnonymousFullVariant(
  ...
  await page.addInitScript((panels: Record<string, unknown>) => {
    ...
    localStorage.setItem('meridian-variant', 'full');
    // Overlays that would otherwise steal focus/paint during the load window.
    localStorage.setItem('wm-layer-warning-dismissed', 'true');
    localStorage.setItem('wm-pro-banner-launched-dismissed', String(Date.now()));
    localStorage.setItem('meridian-mission-preset-dismissed-v1', '1');
    ...
    if (Object.keys(panels).length > 0) {
      localStorage.setItem('meridian-panels', JSON.stringify(panels));
    }
...
async function seedScrollableDashboard(page: Page): Promise<void> {
  ...
  await page.addInitScript((panelOrder: string[]) => {
    ...
    localStorage.setItem('meridian-panel-order-v1.9', 'done');
    localStorage.setItem('meridian-panel-prune-v1', 'done');
    localStorage.setItem('meridian-layout-reset-v2.5', 'done');
