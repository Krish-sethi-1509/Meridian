...
const SECONDARY_STARTUP_REQUEST =
  /(?:abacus\.meridian\.app\/script\.js|fonts\.googleapis\.com\/css2|fonts\.gstatic\.com\/s\/(?:nunito|tajawal)|clerk\.meridian\.app|ingest\.us\.sentry\.io|static\.cloudflareinsights\.com|\/_vercel\/insights\/script\.js|www\.youtube\.com\/iframe_api)/;
...
test.describe('secondary startup work', () => {
  ...
  test('defers secondary startup requests until idle startup runs', async ({ page }) => {
    ...
    await expect.poll(() => secondaryRequests.some((url) => url.includes('abacus.meridian.app/script.js'))).toBe(true);
