...
const installDashboardClsObserver = async (
  ...
  await page.addInitScript(({ dismissKey, legacyDismissKey, seedDeferredFootprint, panelId, panelOrder }) => {
    ...
    if (seedDeferredFootprint) {
      ...
      localStorage.setItem('meridian-panel-spans', JSON.stringify({ [panelId]: 3 }));
      localStorage.setItem('meridian-panel-col-spans', JSON.stringify({ [panelId]: 2 }));
      localStorage.removeItem('meridian-panel-collapsed');
    }
