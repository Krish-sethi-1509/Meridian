...
test.describe('AI widget builder', () => {
  ...
  test('supports modify, keeps session history, exposes touch-sized controls, and cleans storage on delete', async ({ page }) => {
    ...
    await page.evaluate((widgetId: string) => {
      localStorage.setItem('meridian-panel-spans', JSON.stringify({ [widgetId]: 2 }));
      localStorage.setItem('meridian-panel-col-spans', JSON.stringify({ [widgetId]: 3 }));
    }, storedWidgetMeta!.id);
    ...
    const cleanedStorage = await page.evaluate(() => {
      return {
        ...
        rowSpans: localStorage.getItem('meridian-panel-spans'),
        colSpans: localStorage.getItem('meridian-panel-col-spans'),
      };
    });
