...
async function installCleanLiveMediaPrefs(page: Page, webcamPrefs?: Record<string, unknown>): Promise<void> {
  await page.addInitScript((prefs) => {
    ...
    localStorage.removeItem('meridian-active-channel');
    if (prefs) {
      localStorage.setItem('meridian-webcam-prefs', JSON.stringify(prefs));
    } else {
      localStorage.removeItem('meridian-webcam-prefs');
    }
  }, webcamPrefs ?? null);
}
