# 🌍 Self-Hosting Meridian

Run the full Meridian stack locally with Docker/Podman.
...
*/30 * * * * cd /path/to/meridian && ./scripts/run-seeders.sh >> /tmp/wm-seeders.log 2>&1
```
...
|-----------|---------|------|
| `meridian` | nginx + Node.js API (supervisord) | 3000 → 8080 |
| `meridian-redis` | Data store | 6379 (internal) |
| `meridian-redis-rest` | Upstash-compatible REST proxy | 8079 |
| `meridian-ais-relay` | Live vessel tracking WebSocket | 3004 (internal) |
...
# Full Docker image
docker build -t meridian:latest -f Dockerfile .
...
| 🔴 nginx won't start | Check `podman logs meridian` — likely missing `gettext` package |
...
| 🚢 No vessel data | Set `AISSTREAM_API_KEY` in both `meridian` and `ais-relay` services |
